...
import { TEXT_OVERFLOW_ALLOWLIST, allowlistProblems, matchesAllowlist } from "./lib/text-overflow-allowlist.js";
    ...
    test(`${target.name} (${target.path})`, async ({ page }, testInfo) => {
      ...
      for (const viewport of VIEWPORTS) {
        ...
        for (const lang of LANGS) {
          ...
          const where = `${target.name} · ${lang} · ${viewport.name} (${viewport.size.width}x${viewport.size.height})`;
          for (const raw of await scanPageForTextDefects(page)) {
      ...
      // Rapporten vedhaeftes ALTID, ogsaa naar testen er groen: den er
      // raamaterialet til docs/audits/2026-09-19-5383-tekst-overflow-fund.md.
      ...
      expect(
        ...
        `${findings.length} tekst-fund paa ${target.path} (${byRule}).\n` +
