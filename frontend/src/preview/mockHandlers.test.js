...
import { normalizeHonours, championOf } from "../lib/seasonHonours.js";
...
test("#2863 rpcResponse svarer kun på seedede RPC'er", () => {
  assert.ok(rpcResponse("get_season_honours"), "kårings-RPC har seed");
  assert.equal(
...
test("#2863 kårings-seedet overlever normalizeHonours med begge fælder intakte", () => {
  ...
  const onPoints = championOf(points, "points");
  assert.equal(onPoints.champion.isAi, true);

  // Fælde 2: toppen af sejrs-listen er delt → tie-break-noten skal kunne vises.
  const onWins = championOf(wins, "wins");
  assert.equal(onWins.shared, true);
  assert.equal(onWins.champion.wins, onWins.runnersUp[0].wins);

  // Tallene er strenge i seedet (som PostgREST' bigint) og skal være tal bagefter.
  assert.equal(typeof onPoints.champion.points, "number");
