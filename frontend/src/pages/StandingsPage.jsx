  ...
  return (
    <div className="bg-[#0f0f18] border border-white/5 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5"
        ...
        {division > 1 && (
          <span className="ml-auto text-[10px] text-green-400 bg-green-500/10
            px-2 py-0.5 rounded-full uppercase tracking-wider">
            Top 2 rykker op
          </span>
        )}
        {division < 3 && (
          <span className={`${division > 1 ? "" : "ml-auto"} text-[10px] text-red-400
            bg-red-500/10 px-2 py-0.5 rounded-full uppercase tracking-wider`}>
            Bund 2 rykker ned
          </span>
        )}
      </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-white/5">
            <th className="px-4 py-2.5 text-left text-white/30 font-medium text-xs uppercase tracking-wider w-8">#</th>
            <th className="px-4 py-2.5 text-left text-white/30 font-medium text-xs uppercase tracking-wider">Hold</th>
            <th className="px-4 py-2.5 text-right text-white/30 font-medium text-xs uppercase tracking-wider">Point</th>
            <th className="px-4 py-2.5 text-right text-white/30 font-medium text-xs uppercase tracking-wider hidden sm:table-cell">Etapesejre</th>
            <th className="px-4 py-2.5 text-right text-white/30 font-medium text-xs uppercase tracking-wider hidden sm:table-cell">GC-sejre</th>
            <th className="px-4 py-2.5 text-right text-white/30 font-medium text-xs uppercase tracking-wider hidden md:table-cell">Løb</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((s, i) => {
            ...
            return (
              <tr key={s.id}
                className={`border-b border-white/4 transition-colors
                  ${isMe ? "bg-[#e8c547]/5" : "hover:bg-white/3"}`}>
                <td className="px-4 py-3">
                  <span className={`font-mono text-sm font-bold
                    ${isPromotion ? "text-green-400" :
                      isRelegation ? "text-red-400" : "text-white/40"}`}>
                    {i + 1}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    {s.team?.is_ai && (
                      <span className="text-[9px] uppercase text-white/20 bg-white/5
...
export default function StandingsPage() {
  ...
  useEffect(() => {
    if (selectedSeason) loadStandings(selectedSeason);
  }, [selectedSeason]);
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-white/30 text-sm">Divisionstabeller og sæsonresultater</p>
        </div>
        {seasons.length > 0 && (
          <select
            ...
            {seasons.map(s => (
              <option key={s.id} value={s.id}>
                Sæson {s.number} ({s.status === "active" ? "Aktiv" :
                  s.status === "completed" ? "Afsluttet" : "Kommende"})
              </option>
            ))}
          </select>
        )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-white/20">
          ...
          <p>Ingen rangliste data for denne sæson endnu</p>
        </div>
      ) : (
