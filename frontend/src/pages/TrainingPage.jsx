...
function FocusOpenButton({ rider, plan, busy, smartFocus, error, onOpen, t, dataTour }) {
  return (
    <div>
      ...
      >
        {/* min-w-[40px]-gulv på labelen — uden den kan `min-w-0` (som truncate
            kræver) skrumpe teksten til 0px i en smal container. Ingen effekt ved
            normal 184px-bredde. */}
...
export default function TrainingPage() {
  ...
  // #3643 — mobil har sin EGEN visning (mockup 2, tabel med dagens løbsdage),
  // ikke en indsnævret udgave af denne tabel. Derfor er #5124's D-047-gren for
  // rosteret VÆK: chip-bytteren (MobileTableChips), MOBILE_SCROLLER-tilstanden,
  // `showRosterCol`-gates, `align-top`-korrektionen og den dynamiske colSpan
  // var alle betingede på `isMobile` og kunne kun nås i en tilstand der ikke
  // findes længere. Tabellen nedenfor er nu udelukkende desktop-fladen og
  // renderes uændret (samme kolonner, samme sticky-navnekolonne, samme SCROLLER).
  //
  // `isMobile` bliver: den vælger mellem de to visninger, og den gate ligger
  // ÉT sted (TabPanel value="today").
  const isMobile = useIsMobileViewport();
  const rosterColSpan = ROSTER_COLS_TOTAL;
      ...
      <tr className={`${trClass(null)} ${isSelected ? "bg-cz-accent/5" : ""}`}>
        ...
        >
          {/* whitespace-nowrap: navnet er kolonnens naturlige bredde (DataTable-opskriften)
              ...
              #3643: mobil-tilstanden (som brød navnet i stedet) er væk — telefonen
              tegner TrainingMobileToday, ikke denne tabel. */}
        ...
        {/* Skift dag — sidens hovedhandling på desktop-fladen ("skift dagens
            træning"). Mobil har sin egen vej til den samme mutation: rytterens
            kort under tabellen (#3643). */}
        <td className={tdClass({})}>
          {plan?.focus ? (
            <div
              role="group"
  ...
  return (
    <div className="mx-auto max-w-[1600px]">
      ...
      <PageHeader
        ...
        actions={
          // #3643: på telefonen bor sidens ENE gold primary ("Kør dagens
          // træning") i fuld bredde over fanerne, og assistenten er et lukket
          // panel i indholdet — begge inde i mobil-visningen. Sidehovedets
          // actions-slot står derfor tomt dér, så der aldrig er to golde knapper.
          isMobile ? null : (
