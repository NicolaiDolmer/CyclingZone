...
export default function RacePointsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900">Point- og præmieoversigt</h1>
        <p className="text-slate-400 text-sm">Sådan beregnes dine løbspræmier fra UCI-placeringer</p>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {PRIZE_EXAMPLES.map(ex => (
            <div key={ex.label} className="bg-white rounded-lg px-3 py-2 border border-amber-100">
              <p className="text-xs text-slate-500 truncate">{ex.label}</p>
              <p className="font-mono font-bold text-amber-700 text-sm">{fmt(ex.points)} pt</p>
              <p className="text-xs text-slate-400">{fmtPrize(ex.points)}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap
              ${activeClass === c
                ...
                : "bg-white border border-slate-200 text-slate-700 hover:border-amber-200 hover:text-amber-700"
              }`}
          >
        ...
        <div className="space-y-4">
          {availableTypes.map(rType => {
            ...
            if (rType === "Forertroje" && rows.length === 1) {
              ...
              return (
                <div key={rType} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  ...
                  <div className="px-4 py-4 flex items-center justify-between">
                    <p className="text-slate-600 text-sm">Pr. dag i førerposition</p>
                    <div className="text-right">
                      <p className="font-mono font-bold text-amber-700">{fmt(pt)} pt</p>
                      <p className="text-xs text-slate-400">{fmtPrize(pt)}</p>
                    </div>
                  </div>
                </div>
              );
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 text-left">
                        <th className="px-4 py-2 font-medium text-slate-500 text-xs w-14">Plads</th>
                        <th className="px-4 py-2 font-medium text-slate-500 text-xs">UCI-point</th>
                        <th className="px-4 py-2 font-medium text-slate-500 text-xs text-right">Præmie</th>
                      </tr>
                    </thead>
