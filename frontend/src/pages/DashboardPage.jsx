    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="text-right">
          ...
          <p className="text-cz-3 text-xs">Balance</p>
        </div>
      </div>
        ...
        return (
          <div className="mb-4 px-4 py-3 bg-cz-danger-bg border border-cz-danger/30 rounded-xl
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-cz-danger font-bold text-sm">DEADLINE DAY</p>
                <p className="text-cz-danger/70 text-xs">Transfervinduet lukker om {h}t {m}m</p>
              </div>
            </div>
            <Link to="/transfers"
      ...
      {seasonInfo && (
        <div className="mb-5 bg-cz-card border border-cz-border rounded-xl px-5 py-3.5 flex flex-wrap items-center gap-x-5 gap-y-2">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-cz-1 text-sm">Sæson {seasonInfo.number}</span>
          ...
          {seasonInfo.end_date && (() => {
            ...
            if (daysLeft <= 0) return <span className="text-cz-3 text-xs">Sæsonslut passeret</span>;
            return (
              <div className="flex items-center gap-1.5">
                <span className="text-cz-1 font-mono font-bold text-sm">{daysLeft}</span>
                <span className="text-cz-3 text-xs">dage til sæsonslut</span>
              </div>
            );
          })()}
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-cz-card border border-cz-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-cz-1 text-sm">Aktive Auktioner</h2>
            <Link to="/auctions" className="text-xs text-cz-accent-t hover:underline">Se alle →</Link>
          </div>
          {allAuctions.length === 0 ? (
            <p className="text-cz-3 text-sm text-center py-4">Ingen aktive auktioner</p>
          ) : (
                    ...
                    <div key={a.id} onClick={() => navigate("/auctions")}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {isWinning && <span className="text-[9px] bg-cz-success-bg text-cz-success px-1.5 py-0.5 rounded-full">Vinder</span>}
                          {isSelling && !isWinning && <span className="text-[9px] bg-cz-info-bg text-cz-info px-1.5 py-0.5 rounded-full">Sælger</span>}
                        </div>
                      </div>
                      <div className="text-right ml-3">
                        <p className="text-cz-accent-t font-mono text-sm font-bold">{a.current_price?.toLocaleString("da-DK")} CZ$</p>
        ...
        <div className="bg-cz-card border border-cz-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-cz-1 text-sm">Transfers & Tilbud</h2>
            <Link to="/transfers" className="text-xs text-cz-accent-t hover:underline">Se alle →</Link>
          </div>
          {activeMarketOffers.length === 0 && pendingIncoming === 0 ? (
            <p className="text-cz-3 text-sm text-center py-4">Ingen ventende transfers</p>
          ) : (
            <div className="flex flex-col gap-2">
              {pendingIncoming > 0 && (
                <div className="flex items-center gap-3 py-2 border-b border-cz-border">
                  <span className="text-cz-success text-lg">↓</span>
                  <p className="text-cz-1 text-sm">{pendingIncoming} indgående transfer{pendingIncoming > 1 ? "s" : ""}</p>
                  <span className="ml-auto text-[9px] bg-cz-success-bg text-cz-success border border-cz-success/30 px-2 py-0.5 rounded-full">Afventer vindue</span>
                </div>
              )}
                ...
                return (
                  <div key={o.id} onClick={() => navigate("/transfers")}
                    ...
                    <div>
                      ...
                      <p className="text-cz-3 text-xs">{isReceived ? `Fra: ${o.buyer?.name}` : `Til: ${o.seller?.name}`}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-cz-accent-t font-mono text-sm">{(o.counter_amount || o.offer_amount)?.toLocaleString("da-DK")} CZ$</p>
        ...
        <div className="bg-cz-card border border-cz-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-cz-1 text-sm">Kommende Løb</h2>
            <Link to="/races" className="text-xs text-cz-accent-t hover:underline">Kalender →</Link>
          </div>
          {nextRaces.length === 0 ? (
            <p className="text-cz-3 text-sm text-center py-4">Ingen planlagte løb</p>
          ) : (
            <div className="flex flex-col gap-2">
              {nextRaces.map((race, i) => (
                <div key={race.id}
                  ...
                  <div className="text-right">
                    {race.start_date
                      ? <p className="text-cz-2 text-sm">{new Date(race.start_date).toLocaleDateString("da-DK", { day: "numeric", month: "short" })}</p>
                      : <p className="text-cz-3 text-sm">Dato TBD</p>}
                    <p className="text-cz-accent-t text-xs font-mono">{race.prize_pool?.toLocaleString("da-DK")} CZ$</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-cz-card border border-cz-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-cz-1 text-sm">Division {team?.division} — Stilling</h2>
            <Link to="/standings" className="text-xs text-cz-accent-t hover:underline">Fuld rangliste →</Link>
          </div>
          {divStandings.length === 0 ? (
            <p className="text-cz-3 text-sm text-center py-4">Ingen sæsondata endnu</p>
          ) : (
        ...
        <div className="bg-cz-card border border-cz-border rounded-xl p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-cz-1 text-sm">Bestyrelsens Status</h2>
            <Link to="/board" className="text-xs text-cz-accent-t hover:underline">Detaljer →</Link>
          </div>
          {!board ? (
            <p className="text-cz-3 text-sm text-center py-4">Ingen bestyrelsesdata</p>
          ) : (
            <div>
              <div className="grid sm:grid-cols-3 gap-4">
                <div>
                  <p className="text-cz-3 text-xs uppercase tracking-wider mb-2">Tilfredshed</p>
