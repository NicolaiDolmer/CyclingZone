...
// European Champion. Begge tal kommer fra rider_rankings_mv, altså PRÆCIS de
// samme kolonner rytter-ranglisten har vist hele sæsonen (points + #925's
// "Sejre i alt" = de seks sejrs-kategorier lagt sammen). Kåringen må ikke
// regne anderledes end den liste spillerne har fulgt.
...
function normalizeEntry(row, index) {
  ...
  return {
    ...
    // en tom celle uden forklaring i en kåring.
...
/**
 * Vinderen af en kategori plus de øvrige på listen, og om toppen er delt.
 *
 * `shared` er ikke hypotetisk: sæson 1 sluttede med Jakub Adamczyk og George
 * Whitfield på 28 sejre hver. Tie-breaket (flest point) afgør hvem der får
 * titlen, men UI'et skal kunne sige det højt i stedet for at lade nr. 2 undre
 * sig over hvorfor det samme tal gav to forskellige placeringer.
 *
 * @param {Array} entries normaliseret liste (allerede sorteret af serveren)
 * @param {"points"|"wins"} metric
 */
export function championOf(entries, metric) {
  const list = Array.isArray(entries) ? entries : [];
  if (list.length === 0) return { champion: null, runnersUp: [], shared: false };
  const [champion, ...runnersUp] = list;
  const shared = runnersUp.some((e) => e[metric] === champion[metric]);
  return { champion, runnersUp, shared };
}

/**
 * PostgREST svarer PGRST202 (og Postgres 42883) når funktionen ikke findes.
 * Migrationen applies EFTER merge, så i vinduet imellem skal siden bare undlade
 * at vise kårings-blokken. Enhver ANDEN fejl er ægte og skal vises som en
 * fejl-tilstand, ikke skjules: en tom kåring der ser bevidst ud er værre end en
 * synlig fejl (#1851-klassen).
 */
