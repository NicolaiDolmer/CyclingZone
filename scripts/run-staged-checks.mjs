...
if (!Object.keys(config).some(pattern => {
  const isMatch = picomatch(pattern, { cwd: process.cwd(), dot: true, matchBase: !pattern.includes('/'), posixSlashes: true, strictBrackets: true });
  return files.some(file => isMatch(file)); // Array index must not become picomatch's returnObject flag.
})) {
