...
Læs outputtet. Hvis dry-run rapporterer `Resultat: 0 worktree(s), 0 branch(es)` → der er intet at gøre. Skriv en kort chat-linje ("Worktree-cleanup: intet at fjerne denne uge") og exit normalt UDEN at køre `-Execute`.
...
Hvis filen ikke findes, start den med en H1-header (`# Worktree-cleanup log <MONTH>`). Append derefter én linje pr. kørsel med tidsstempel, PC, og resultat-linjen fra `-Execute` (fx `- 2026-06-21 20:00 (NICOLAIPC): 3 worktree(s), 4 branch(es) fjernet.`). Hvis kun dry-run kørte (0 kandidater), append `- <STAMP> (<PC>): intet at fjerne (dry-run rent).`.

### 5. Commit + push loggen
...
Skriv én chat-linje: "Worktree-cleanup kørt: <N> worktree(s) + <M> branch(es) fjernet, log i $LOG." (eller "intet at fjerne"). Exit 0.

## Fejlhåndtering

- PowerShell/script-fejl under dry-run: abort før `-Execute`, log til `~/.claude/cross-pc-sync.log`, exit non-zero.
...
- ❌ Brug ALDRIG `-Force` (uncommitted worktrees beholdes altid).
