#!/usr/bin/env node
/**
 * Sweep 2026-06-25. Dumper:
 *  1) NYE feedback-tråde i #samlet-feedback-features-og-bugs (parent 1501501095325732925)
 *     med snowflake-id > cutoff 1518740959020580955 (sidste behandlede 2026-06-22 22:42).
 *     Henter både aktive OG arkiverede public threads.
 *  2) Seneste beskeder i #cycling-zone (1386713292491194458),
 *     #spørgsmål-og-svar (1387129538315554867), #patch-notes (1389612026632077353)
 *     — filtreret til 2026-06-23T00:00 og frem.
 * Token fra env; aldrig printet. Output: scripts/discord/.new-threads-2026-06-25.md
 */
...
const CUTOFF_THREAD = 1518740959020580955n;
const SINCE_ISO = '2026-06-23T00:00:00';
const OTHER_CHANNELS = [
  ['1386713292491194458', '#cycling-zone (general)'],
  ['1387129538315554867', '#spørgsmål-og-svar'],
  ['1389612026632077353', '#patch-notes'],
];
...
async function main() {
  ...
  // collect candidate feedback threads (active + archived public)
  const found = new Map(); // id -> {name, ts}
  ...
  let out = `# Discord-sweep 2026-06-25 — siden cutoff ${CUTOFF_THREAD} (2026-06-22 22:42)\n\n`;
  ...
  const file = path.join(process.cwd(), 'scripts', 'discord', '.new-threads-2026-06-25.md');
