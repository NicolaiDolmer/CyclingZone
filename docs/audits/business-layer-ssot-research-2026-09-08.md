...
- **PR'er:** #5049 GROWTH_STACK · #5050 ANALYTICS_STACK + `docs/runbooks/GSC_SERVICE_ACCOUNT.md` · #5052 · #5053 EMAIL_STACK · #5054 COMMS_PLAYBOOK · #5056 integration (registrering, arkivering, pegepinde).
- **Nyt issue:** #5051 LTV-prisdrift (`PLAN_PRICE_CENTS.semiannual` 26500 mod faktisk 21200 øre), kode-fix uden for docs-bølgen.
- **Infrastruktur:** Vercel-projektet `cycling-zone-marketing` oprettet, marketing-sitet live på `cycling-zone-marketing.vercel.app` (rewrites udestår, separat PR med ejer-go).
- **Udestående ejer-trin:** `VITE_POSTHOG_KEY` sættes i Vercel · GSC-service-konto oprettes efter `docs/runbooks/GSC_SERVICE_ACCOUNT.md` · Resend-webhook-secret sættes (`RESEND_WEBHOOK_SECRET`) så delivered/bounce kan måles · EUR-testkøb gennemføres (#4616).
