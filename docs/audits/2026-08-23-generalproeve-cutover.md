...
| Entries til S3 (ikke i den oprindelige opgavebeskrivelse - se FUND) | `... node scripts/generateSeasonEntries.js --season=00000000-0000-0000-0000-000000000003 --execute` | **457,8 s (≈ 7 min 38 s)** - se FUND, staging-tallet er kunstigt langsomt | 34.219 enheder indsat, 0 fejlede. 315/471 løb har nu entries - de resterende 156 er alle i de 6 dormante D4-puljer (C/D/E/F/G/H, 0 ægte hold, 26 løb hver) og skal IKKE have entries. Dag 1 (25/8): 26/38 løb har entries (de manglende 12 er 2× de dormante puljers dagsandel) - korrekt, ikke en fejl |
...
| Entries for S3 dag 1 (25/8) | genereret | ❌ **0 `race_entries` for NOGEN af de 471 S3-løb** umiddelbart efter transitionen (se FUND) - **✅ rettet** ved at køre `generateSeasonEntries.js --execute` som ekstra skridt: 315/471 løb har nu entries (de resterende 156 er de 6 dormante D4-puljer, korrekt 0). Dag 1 (25/8): 26/38 løb har entries, resten er samme dormante puljer |
...
Bygget på de målte tider ovenfor. **Alle klokkeslæt er dansk tid og er STARTPUNKTER for et skridt - ikke garantier.** Hvert **EJER-GO** er en hård gate: intet skridt derefter starter uden ejerens eksplicitte ja på den LIVE tilstand (27/6-reglen). Samlet aktiv script-tid i selve vinduet: **≈ 44 minutter**. Med ejer-go-pauser mellem hvert skridt (budgetteret 2-3 min hver) lander hele kæden på **≈ 65-75 minutter**, dvs. der er **over halvanden times buffer** før 22:30-grænsen selv med generøse pauser - og god margen før det daglige tick kl. 22:00.

**Før vinduet (kl. ~19:05, lige efter sidste etape kl. 19:00):**

| Tid | Skridt | Kommando | Forventet varighed |
|---|---|---|---|
| 19:05 | D1-komprimering: dry-run mod PROD (regenerér listen - sidste chance for et løbsresultat der flytter cutlines) | `infisical run --env=prod -- node scripts/compressPyramidS3.js` | **≈ 11 min** - kør TIDLIGT, ikke inde i vinduet (se trin 4-FUND) |
| 19:16 | Ejeren ser + godkender top-24-listen, cutlines, pulje-fordeling | (læs `docs/snapshots/3901/dry-run-<dato>.md`) | ejer-tid |

**Vinduet (19:30-22:30):**

| Tid | Skridt | Kommando (prod) | Varighed | Ejer-gate |
|---|---|---|---|---|
| 19:30 | **EJER-GO 1**: start | - | - | ✅ ejeren har set live-status |
| 19:30 | Sæt `season_transition_planned_at` (#4129) | SQL, se trin 0 | < 5 s | - |
| 19:31 | Snapshot + backup-kæde (#3459 + #3645) | se trin 1's tabel | ≈ 45 s | STOP hvis snapshot ikke er læsbart |
| 19:32 | Pre-flip gate (**sæsonnummer 2**, se FUND trin 2) | `node scripts/dev/remeasureGate3459.mjs <json> 2` | < 1 s | STOP ved RØD (>1 % ændret eller p10 < −5) |
| 19:33 | **EJER-GO 2**: race-day-flip | `update app_config set value='"on"' where key='race_day_engine_enabled'` | øjeblikkeligt | ✅ spillerbesked postet FØR dette |
| 19:34 | Sæt `season_end_skip_division_movement='on'` - **FØR** "Afslut sæson", se trin 3-FUND | SQL | øjeblikkeligt | - |
| 19:35 | **EJER-GO 3**: "Afslut sæson" (S2) | UI-knap eller `POST /admin/seasons/:id/end` | **≈ 8 min** (målt 7 min 41 s + margin) | STOP ved fejl halvvejs - se trin 3-FUND, ALDRIG blind genkørsel |
| 19:43 | Verificér: `seasons.status='completed'`, notifikationer sendt, ingen division rørt endnu | SQL | < 1 min | - |
| 19:44 | **EJER-GO 4**: D1-komprimering apply | `node scripts/compressPyramidS3.js --apply --snapshot=<json fra 19:05>` | **≈ 70 s** (målt 67 s) | STOP hvis verifikation ≠ 214/214 placeringer korrekte |
| 19:46 | Window-wrap (**RETTET SQL**, se trin 3-FUND - husk `squad_enforcement_started_at`) | SQL | øjeblikkeligt | - |
| 19:47 | **EJER-GO 5**: "Udfør sæsonskifte" (transitionen) | UI-knap eller `executeSeasonTransition.js --from=<S2> --execute` | **≈ 14 min** (målt 13 min 29 s) | HÅRDT STOP kun ved `global_rank_decay`-fejl (ingen retry). Andre fase-fejl: ret + genkør (idempotent) |
| 20:01 | Sæt `season_end_skip_division_movement='off'` (#2164) | SQL | øjeblikkeligt | - |
| 20:02 | Verificér: `admin_log` 1 ny season_transition-række, S3 `active`, fatigue=0 | SQL | 1 min | - |
| 20:03 | **NYT SKRIDT (mangler i drejebogen, se trin 5-FUND): generér entries til S3** | `node scripts/generateSeasonEntries.js --season=<S3> --execute` | **≈ 8 min** (staging-målt 7 min 38 s, sandsynligvis hurtigere på PROD - se trin 5-FUND om manglende constraint) | STOP hvis `failed_units` > 0 |
| 20:11 | Verificér: 0 løb i aktive puljer uden felt | SQL | 1 min | - |
| 20:12 | **EJER-GO 6**: løn-genberegning | `CONFIRM_SALARY_RECOMPUTE=yes node scripts/dev/salaryRecompute3645.mjs --apply` | **≈ 5 min** (målt 4 min 36 s ved en afbrudt-og-genkørt sekvens - en ren kørsel bør være hurtigere) | - |
| 20:17 | Mandat-backup-tabel (manuel SQL, hvis ikke allerede oprettet) | `create table backup_board_profiles_3514_<dato> as select * from board_profiles;` | øjeblikkeligt | - |
| 20:18 | **EJER-GO 7**: mandat-migration (ejeren ser scorecardet LIVE) | `node scripts/dev/mandateMigration3514.mjs --apply --jeg-har-set-scorecardet` | **≈ 3 min** (målt 2 min 38 s) | STOP hvis > 0 hold krydser en NY konsekvens-tærskel |
| 20:21 | Sæson-achievements | `node scripts/backfillSeasonAchievements.js --season=<S2> --execute` | **≈ 30 s** (målt 27,5 s) | - |
| 20:22 | Slutkontrol (Discord 1 besked, Sentry, næste stage-scheduler-tick, matview-afstemning) | manuel | ≈ 5 min | - |
| **≈ 20:27** | **FÆRDIG** | | | **≈ 3 timer buffer til 22:30 / ≈ 1,5 time til det daglige tick kl. 22:00** |

**Buffer-anbefaling:** planlæg med afrundet **90 minutter** i stedet for de målte ≈ 65-75 (langsommere PROD-population, netværks-variabilitet, og ejer-gates tager typisk længere end 2-3 min i praksis). Selv med det er der rigelig margen i det 3-timers vindue.

**To skridt der IKKE er en del af den bindende rækkefølge, men skal tjekkes:**
- **#4131 (kalenderændring, S3 slutter søndag, 27 dage)** - hvis anvendt til PROD FØR 19:30 (ejer-beslutning i dag var "anvendes FØR cutover i aften"), skal `seasons.race_days_total` for S3 verificeres = **27**, ikke 28, FØR transitionen kører (påvirker dagslønnen). Denne generalprøve er kørt mod den GAMLE kalender (28 dage, 471 løb) - se "Hvad kunne IKKE måles".
- **Komponent 1 (markedsværdi-niveaukorrektion) og komponent 2 (#3393-løn)** flipper IKKE i aften (gate RØD / afventer design-session) - trin 10 er ren måling, ingen apply.

## FUND (samlet liste, prioriteret)

1. **Kritisk:** `SEASON_CUTOVER_RUNBOOK.md` modsiger #3901/`compressPyramidS3.js` om `season_end_skip_division_movement` (skal være `'on'` FØR "Afslut sæson" for S2→S3, IKKE `'off'` som runbook'en siger). Opdaget midt i den kørende `endSeasonS2.mjs` - rettet i tide i denne generalprøve, men **kun fordi jeg tilfældigvis fangede det undervejs**. **SKAL afklares/rettes i dokumentationen før i aften** (trin 3).
2. **Kritisk:** `generateSeasonEntries.js`-skridtet er IKKE en del af denne opgaves trinliste (KÆDEN, punkt 0-10), men UDEN det har S3 **0 entries i noget løb** efter transitionen. Skal tilføjes eksplicit til i aftens køreplan (trin 5).
3. **Kritisk:** D1-komprimeringens dry-run tager ~11 min mod PROD pga. en 571.614-rækkers ikke-batchet countback-forespørgsel - kør TIDLIGT (~19:05), ikke inde i selve 19:30-vinduet (trin 4).
4. **Kritisk (proces, ikke kode):** `processSeasonEnd` må ALDRIG genkøres blindt efter et afbrudt forsøg - sletning af `season_end_claims` + genkørsel dobbelt-tæller board-satisfaction (trin 3, selv-forårsaget i denne rehearsal ved at bryde drejebogens egen "diagnosticér først"-regel, men beviser risikoen er reel og konkret).
5. **Kritisk:** mandat-rollbackens `TRUNCATE`-SQL i `docs/2026-08-23-cutover-drejebog.md` (Komponent 4) fejler på en FK-constraint tilføjet af selve #3514-migrationen. Fix testet: brug `DELETE FROM` i stedet. **Ret drejebogen før den bliver nødvendig i en incident** (trin 9c).
6. **Vigtigt:** `remeasureGate3459.mjs`s sæsonnummer-argument har INGEN validering mod den faktisk aktive sæson - forkert tal (3 i stedet for 2) gav en falsk RØD gate på 54 % af AI-populationen. Brug **sæsonnummer 2** til pre-flip-gaten i aften (trin 2).
7. **Vigtigt:** window-wrap-SQL'en i den gamle runbook mangler `squad_enforcement_started_at` og fejler nu på en constraint der ikke fandtes da den blev skrevet (trin 3).
8. **Vigtigt:** `no_rider_double_booking`-constrainten findes på PROD men IKKE på staging-branchen - bekræfter at `status=MIGRATIONS_FAILED` er et REELT skema-hul, ikke kun kosmetisk. Enhver tidsmåling herfra der involverer entry-skrivning er en PESSIMISTISK (for langsom) proxy for PROD (trin 5).
9. `scripts/lib/Staging-Env.ps1` var aldrig committet selvom `scripts/with-staging.ps1` afhænger af den - rettet i denne PR (commit `fa8f99f1`).
10. `pwsh -File script.ps1 -- args` (den dokumenterede kommandoform for `with-staging.ps1`) fejler i denne miljøs PowerShell 7.6.5 - `--` bliver ikke stoppet af pwsh'ens egen parser og støder sammen med scriptets parameterbinding. Virker fra en LIVE pwsh-session med `& script.ps1 -Command @("node", "...")` (array-literal, IKKE `--`).
11. `backfillSeasonAchievements.js --season <uuid>` (mellemrum) fejler kryptisk - brug `--season=<uuid>`.
12. `generateSeasonEntries.js`s dry-run-rapport viser ALTID `+0 indsat · -0 fjernet · 0 rolle-opdateret`, uanset hvor mange enheder der reelt mangler (koden springer selve diff-tællingen over i dry-run - kun "Genereret (enheder)"-tallet er retvisende i dry-run). Kan let mistolkes som "intet at gøre".
13. Flere scripts (`compressPyramidS3.js` dry-run, `endSeasonS2.mjs --execute`, `mandateMigration3514.mjs --apply`, `salaryRecompute3645.mjs --apply`, `restoreCaps3459.mjs --apply`) tog alle **over 2 minutter** og blev dræbt af værktøjets synkrone timeout ved første forsøg - måtte alle genkøres i baggrunden. Ingen af disse er for langsomme til i aftens 3-timers vindue, men **brug ALDRIG en synkron/2-minutters-begrænset terminal til disse kommandoer i aften** - kør dem et sted uden en sådan grænse (Railway-shell, eller baggrundskørsel).
14. #4131 (kalenderændring: S3 slutter søndag, 27 løbsdage) er under aktiv behandling SAMME dag (ejer-beslutning kl. 12:38: anvendes FØR cutover i aften, dry-run-deadline 16:30) - staging blev branchet 12:57 og reflekterer derfor IKKE denne ændring. Se "Hvad kunne IKKE måles".
15. `board_satisfaction_events`-kvitteringerne fra mandat-migrationen var 374 i stedet for de forventede 217 - sandsynligvis fordi den afbrudte-og-genkørte apply indsatte kvitteringen som et rent INSERT i stedet for en upsert (ikke undersøgt til bunds).
...
- **Form-decay per rytter (drejebogens verifikationspunkt "stikprøve 20 ryttere"):** jeg tog ikke et `rider_condition`-snapshot FØR transitionen (et hul i min egen forberedelse - burde have været en del af trin 1's snapshot-kæde). Kun den AGGREGEREDE efter-tilstand er bekræftet (avg fatigue=0, avg form≈49,2, konsistent med decay-mod-50-formlen), ikke en per-rytter-verifikation af selve formlen.
- **Entry-genererings-tid på PROD:** staging manglede `no_rider_double_booking`-constrainten (se FUND #8) og tvang scriptet ned i en langsom fallback-sti. Det reelle PROD-tal kendes ikke - kun at det bør være HURTIGERE end de målte 457,8 sekunder.
- **D1-komprimeringens dry-run-tid på et skema med den korrekte constraint:** samme usikkerhed som ovenfor - de ~11 minutter er målt mod PROD selv (ikke staging), så DETTE tal ER retvisende for i aften, i modsætning til entry-generatoren.
- **#4131's virkning på selve kæden** (kortere sæson, andet `race_days_total`): kunne ikke rehearses, fordi #4131 endnu ikke var landet på PROD da staging blev branchet 12:57, og løses separat af en anden session i dag (deadline for dens egen dry-run: 16:30). Anbefaling: kør en hurtig sanity-SQL (`select race_days_total from seasons where number=3`) lige inden transitionen i aften, uanset hvad denne rapport siger.
