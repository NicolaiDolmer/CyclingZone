...
PreToolUse hard-block-hooks (`block-archived-edit`, `check-now-md-edit`, `block-dangerous-secret`) håndhæves **ikke** for tools der står i `permissions.allow`. På Claude Code ≥2.1.154 bypasser en allow-entry hookens `exit 2` ([anthropics/claude-code#18312](https://github.com/anthropics/claude-code/issues/18312)) — hooken fyrer, men blokeringen ignoreres. Verificeret på DolmerPC: både hook-`exit 2` (fix A) og JSON-`permissionDecision: deny` (fix B) bypasses i `acceptEdits`-mode når tool'et er allow-listet.

**LØSNING (fix D, verificeret 2026-05-29):** statiske `permissions.deny`-regler **overlever** allow-listen og `acceptEdits` — `deny > allow` i precedence håndhæves af permission-laget (ikke hooks). Modtest i frisk acceptEdits-session: `Write → docs/archive/_probe-deny.md` blev blokeret med *"File is in a directory that is denied by your permission settings"*, filen nåede aldrig disken. `.claude/settings.json` har nu `Write`/`Edit`/`NotebookEdit(docs/archive/**)` i `permissions.deny` (begge sti-former, med og uden `./`-præfiks). Derfor er `Write`/`Edit`/`NotebookEdit` igen i `permissions.allow` (autonomi for subagents) **samtidig med** at arkiverede docs er hard-beskyttede.

**Tilbageværende gap (upstream-blokeret, #18312):** guardrails der afhænger af *indhold* frem for *sti* kan ikke udtrykkes som statiske deny-globs — konkret NOW.md's 30-linjers-grænse (`check-now-md-edit`) og dynamisk secret-pattern-detektion (`block-dangerous-secret`). De forbliver hook-only og dermed afvæbnede for allow-listede tools indtil #18312 er fixet upstream. **Konsekvens for parallel-orchestration:** arkiv-skrivning er nu sikker, men en autonom subagent kan stadig sprænge NOW.md-grænsen eller dumpe secrets uden at hard-blocken griber. Hold disse to i menneske-review ved autonome runs.
...
| Reviewer | 30 min, **opus** | Praecis **eet** automatisk gen-spawn foer sporet meldes uden review. Et blokerende data-/skema-fund uden opslag i `database/schema-snapshot.json` eller en read-only `SELECT ... FROM` nedgraderes til bemaerkning (#5567) |
