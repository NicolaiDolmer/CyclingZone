...
| 5 | **Merge-protokol** (morgen) | Ejer-go pr. bølge; >5 PR'er = eksplicit bulk-go ([fleet-playbook](../.claude/learnings/) bølge 2). **Merge queue (4/8):** godkendte PRs sættes i kø med `gh pr merge --auto --squash` — køen batcher, kører required checks på den kombinerede kode og merger selv; ingen manuel update-branch (se GITHUB_WORKFLOW.md §Merge queue). Rækkefølge (= kø-rækkefølge): backend/lav-konflikt → store UI-PR'er → bredeste PR (med migration) sidst; snapshots refreshes i DENS worktree. `database/2026-*.sql` auto-applies på prod ved merge — review SQL FØR merge; post-merge må Claude selv apply/verificere via MCP under #2642-rammerne (AGENTS.md hard rule 9; destruktivt = ejer-gated). Mellemliggende deploy-verify-fails i en merge-salve er støj; kun SIDSTE merges deploy-verify + auto-migrate tæller. | Alle merges har ejer-go |
