...
- **Livstegn (6/9):** hver worker pusher inden 10 min (tom commit ok) og derefter hvert 15. min. Orkestratoren måler fremdrift hvert 15. min på BRANCH (`git worktree list` + `origin/<branch>`), ikke på en antaget mappe-sti. Tavs >45 min uden push = stop + recovery-worker i SAMME worktree med målt WIP-status (commits ahead, dirty filer, sidste fil). _6/9: tre agenter frøs (én i nat, to på samme lane); én blev fejlklassificeret som frossen fordi tjekket kiggede i en forkert mappe._
