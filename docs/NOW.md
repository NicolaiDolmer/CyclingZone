...
> **🎯 Næste action: MERGE-DAG 19/8** ([prompt](sessions/2026-08-19-merge-dag-session-prompt.md)) — åbner med ejer-test + merge-go på trin 7 (PR #3798, CI helt grøn), kører hele udrulningskæden i mål og tager derefter PR-køen (#3959 · rebase af #3393/#3449 · A/B på #3512). Overgangs-design bygget 18/8 aften (ejer-godkendt mockup): loft ved siden af prognosen (hero/scouting/tooltips) + engangspanel på dashboard m. holdets egne før/efter-tal. Test: lokal staging m. ægte branch-data (localhost:5173, egen konto virker) · Vercel-preview (mock) · delbart link https://web-production-aea1d.up.railway.app venter KUN på ejerens to `railway up`-kommandoer (chat 18/8, klassifikator blokerer agent-deploys). Efter go: merge → migration → #3803-backfill m. dry-run-tal til ejeren FØRST → ejeren poster besked (docs/discord/2026-08-16-trin7-potentiale-fart.md). Derefter LØN-SESSION ([prompt](sessions/2026-08-19-loen-design-session-prompt.md)) + ugen (kalender → race-UI PR B → cutover sø 23/8).
