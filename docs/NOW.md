...
> **🎯 Next action (24/6 — race-hub 0c SHIPPED + AKTIVERET):** Overlap er nu LIVE i sæson 1. **Næste: frontend Fase 1** (Lag 1 trup-fordeling + delt bånd) per race-hub-redesign-spec §9 — det er nu spillerne skal kunne SE/styre overlap-fordelingen i UI. Derefter Fase 2-5 (Holdstrategi → detalje/ruteprofil → taktik → andre divisioner). **Nyligt shippet/live:** præmiepenge ÷20 ([#1817](https://github.com/NicolaiDolmer/CyclingZone/pull/1817)) — `PRIZE_PER_POINT` 1500→75, recalc 12.675 ryttere, 0 hold i minus, backup `prize_rescale_backup_20260623`.
