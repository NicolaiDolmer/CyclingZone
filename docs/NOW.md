...
> **🎯 Next action:** (1) **#2080 kampagne-drafts før 4/7** (marketing; ejer-godkendt tekst). (2) **Ejer-clicks før TdF (~15 min):** **#2154** (opret #ops-webhook + sæt `DISCORD_OPS_WEBHOOK_URL`/`DISCORD_OPS_MENTION` i Railway — aktiverer #2077 ops-kanal → sig til, så verificerer Claude end-to-end) + **#2076** (Sentry uptime-monitor + Discord-action på alarm-regel 559456). (3) **#2170** Monuments binding-fri (game_day 100000+) — ejer-beslutning: behold vs. integrér i sæsonkalender (realisme). (4) **#2081-rest** (staged reveal · holdfilter · top-10) + luk #2072 efter GT-verify. (5) **Scouting-fane** (#2000-rest) + capstone/loft #2100. (6) #2082 trænings-scorecard (harness 5/7). (7) **#1972 backup-drop** ejer-gated. (8) **#1996 del 2** (`seasonTransition`/`squadEnforcement` window-kode) + Deadline Day EFTER TdF. **Shipped+merged 4/7 (fejl-triage-session, Supabase-log + Sentry):** **#2158 (PR #2166, v6.54)** — `race_result`+`emergency_loan_breach` manglede i `notifications_type_check` (2 tavst-døde notifikationer genoplivet) + #1464 auto-discovery forward-guard; **#2167 (PR #2168, v6.55)** — race-motor "No start list": autofill-binding (`loadFieldBindingContext`) selecter nu `game_day` (hentede kun `scheduled_at` → falsk all-løb-overlap efter kalender-rebuild → tomt startfelt), **verificeret prod-heal: SA-race 0→165 entries, etape 1 kørt**; **406-støj (PR #2169)** `.single()`→`.maybeSingle()` på watchlist/auktion/standing (~95% færre 406). **Shipped 3/7:** #1996 del 1 (PR #2151) transfervindue-oprydning; #1995 (PR #2159, v6.53) udskudt holdskifte midt i etapeløb (Model B, flush i raceRunner); #2077 stall-watchdog+ops (PR #2148); Discord DM per-type-prefs (PR #2157, v6.52). Railway-CLI logget ind → prod-logs læsbare (#2076-pkt 3 done). **Spøgelsesløb LØST 3/7 ([#2149](https://github.com/NicolaiDolmer/CyclingZone/issues/2149), PR #2155):** 192 tomme Division 4-løb ryddet, div 1-7 urørt, forward-guard på plads. Åbent i #2149: scheduler-latency ~103t. **Beslutnings-dag 6/7:** Alunta/CZ Pro + VMan + Discord-migration. Discord-posts kræver ALTID ejer-godkendt tekst.
