...
> - **[#1791](https://github.com/NicolaiDolmer/CyclingZone/issues/1791) ungdoms-rytter-rework — PR [#1809](https://github.com/NicolaiDolmer/CyclingZone/pull/1809) åben, afventer ejer:** Fase A-F done (svag talent-skaleret start + per-rytter start-held, potentiale-drevet loft+fart, ejer-godkendt sim-kalibrering). `migrateAcademyRiders.js` (dry-run default) ÆNDRER prod-data → **ejer kører migrering + merger** (ingen auto-merge). Næste-kandidat-pool: forever-relaunch-spor / #1152-rest.
> - **Race-hub Fase 0b — PR [#1810](https://github.com/NicolaiDolmer/CyclingZone/pull/1810) åben, afventer ejer:** proaktiv entry-generator (binding-bevidst, idempotent, flag-gated OFF) + afmeld-state (`race_withdrawals`). Migration → **ejer merger** (auto-applies). Subagent-drevet (impl + 2-trins-review/task, alle ✅), verify-local grøn (2156+683). 0a (#1808) merged. Næste: Fase 0c (bund-ryttere).
