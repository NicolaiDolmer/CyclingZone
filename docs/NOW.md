...
> **🎯 Næste action: EJER-TEST + MERGE-GO PÅ TRIN 7 (PR #3798, CI helt grøn).** Overgangs-design bygget 18/8 aften (ejer-godkendt mockup): loft ved siden af prognosen (hero/scouting/tooltips) + engangspanel på dashboard m. holdets egne før/efter-tal. Test: lokal staging m. ægte branch-data (localhost:5173, egen konto virker) · Vercel-preview (mock) · delbart link https://web-production-aea1d.up.railway.app venter KUN på ejerens to `railway up`-kommandoer (chat 18/8, klassifikator blokerer agent-deploys). Efter go: merge → migration (dev_transition_dismissed_at) → #3803-backfill m. dry-run-tal til ejeren FØRST → ejeren poster omskrevet besked (docs/discord/2026-08-16-trin7-potentiale-fart.md, omskrevet til loft-mod-loft). Derefter: LØN-DESIGN-SESSION ([prompt](sessions/2026-08-19-loen-design-session-prompt.md)) + resten af ugen (kalender → race-UI PR B → cutover søndag 23/8).
