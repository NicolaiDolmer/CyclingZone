...
>
> **⏳ Afventer ejer-merge+apply — PR #2396 (#2391):** stage-scheduler-flaskehals fixet. `updateStandings` streamede 166k race_results/kald (40-120s) efter hver etape → tick >35 min → backlog. Ny `recompute_season_standings`-RPC = ~190 ms (set-baseret i Postgres), bevist bit-identisk mod prod. Indeholder `database/*.sql` → **du merger + anvender migrationen** (graceful Node-fallback indtil da). Aktiverer sig selv efter apply (ingen redeploy). Lukker samtidig #2389's FK-vindue.
>
> **🎯 Next action:** **13/7: verificér første v3-løbsdag LIVE** — runs stempler `engine_version=2`, DNF-rate i bånd (`race_incidents`), hjælper-tab synligt, Sentry ren; fjern `race_incidents` (+`race_simulation_rider_scores`) fra liveness-whitelisten når tabellerne har rows (race_stage_roles ER fjernet 12/7). Post-deploy-verify #2394: Express-advarsel væk i Railway-log + nye cron-monitors checker ind (morgen-triagen 07:32 samler op). Næste Claude-session: **v3 S5 peak-planer** (spec §10). Ejer 13/7: Alunta+CZ Pro (#1903) · flip-bølge-verify (#2357) · /training-verify · Sentry-regression-klik · 📌 ovenfor.
