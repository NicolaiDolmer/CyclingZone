...
> 3. **Ryttertype-backfillen er FORBEREDT, ikke kørt.** Før-snapshot taget (`riders_type_backfill_snapshot_20260805`, 8.176 rækker verificeret). Dry-run: **51,9 %** af alle ryttere skifter type; climber+tt falder fra 89,7 % til 50,8 % af populationen. `valuation_type` er bevisligt urørt (payload = `{id, primary_type, secondary_type}`; værdisætningen læser `valuation_type` først). Én kommando: `cd backend && node scripts/backfillRiderTypes.js`. Tal + rollback-SQL: [night-wave-2026-08-05-aften.md](audits/night-wave-2026-08-05-aften.md).
