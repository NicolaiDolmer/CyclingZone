...
> **🎯 Næste action — EJER-ERKLÆRET VIGTIGST 14/8:** **design-session om rytterudvikling og træning** ([prompt](sessions/2026-08-14-traening-og-udvikling-designsession-prompt.md)). "Managers skal kunne stole på rytternes udvikling." Ingen kode før designet står; mange spørgsmål, visuelt, ét ad gangen. **Fund:** motoren regner ALLEREDE fart-baseret (`dailyAbilityDelta` ∝ afstand til loft) — muren er den sidste tomme (`gap === 0`) + teksten "will not rise again". **Målt:** lofterne binder 3× hårdere uden for rollen (22,1 % på loft, luft 10,4) end inden for (7,6 %, luft 27); rolle-faktoren 0,45 er hvor muren opleves. **[#3682](https://github.com/NicolaiDolmer/CyclingZone/issues/3682) er første forekomst af netop dét og bør VENTE på designet** — den er ikke dato-bundet til 23/8. **Landing 1 merged 13/8** (`6de072dd`); R1 landede på 7 mod ≤6 med ejer-go (årsag = evne-skalaen, #3668).
...
> **🤖 Working agent:** Ingen aktiv session. **Natbølge 14/8 (00:25-05:39) landede: 12/12 spor + 1 unblocker = 13 PR'er, 0 stoppet, 0 døde.** Fuld rapport: [`docs/audits/night-wave-2026-08-14.md`](audits/night-wave-2026-08-14.md). **Intet merged — venter ejer-go.** Merge #3704 (bundle-loft) FØRST: main lå selv 0,3 KB over loftet før bølgen, så fire frontend-PR'er står røde på `perf-gate` uden at have gjort noget forkert; de bliver grønne ved re-run bagefter. **Tre ejer-gates før merge:** #3691 leverer et tredje design hvor issuet bad dig vælge A/B · #3699 sender nu en notifikation på den hidtil tavse `pending`-sti (ny besked til spillere) · #3700 besvarer kun halvdelen af #3621. **Friktion målt:** Infisical-sessionen på denne PC er udløbet, så natbølge-agenter kan ikke læse Sentry-events — det kostede tid på #3597.
