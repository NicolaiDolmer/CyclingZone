...
> **🟢 2026-05-28 close-out ([#653](https://github.com/NicolaiDolmer/CyclingZone/issues/653)):** i18n e2e-smoke for raw keys og hardcoded DA var allerede leveret på `main` i `684508a` (`frontend/tests/e2e/core-smoke.spec.js`). Verification: targeted Playwright `translated manager pages do not leak raw i18n keys or hardcoded Danish in English` grøn på desktop Chromium. PatchNotes ikke opdateret: test-only hardening uden brugerrettet app-adfærd.
