...
> **Næste session starter med:** Læs `AGENTS.md`, `docs/GUARDRAILS_CORE.md`, `docs/NOW.md`, `docs/MASTER_PLAN.md` og `docs/decisions/session-c-discord-validation-loop.md`. Hvis Nicolai har postet Discord-loopet, opdatér `docs/SPRINT_DASHBOARD.md` Week 1 checkboxes + tidlige metrics. Ellers tag **#500 Apply Session B terminology** (priority:high — runtime strings stadig brudt) eller **#479 mobile performance fase 2** (i18n provider lazy-load for public routes + standalone /founder-supporter entry — fase 1 landet i v3.60 men nåede ikke 90+ target).
