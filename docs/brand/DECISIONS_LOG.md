...
**🟢 PR-3 (gold = the leader) SHIPPED 2026-06-06** (`feat/481-phase4-pr3-gold-leader`, PatchNotes 4.91). PF2 applied across every team-ranking surface so brand gold stops meaning two things. **Owner gate (NOW.md):** light+dark standings mockup shown before lock (`standings-gold-leader.html`) — owner picked **variant B (gold jersey-chip)** over A (gold left-bar) because the leader is almost always in the promotion zone, so A would sacrifice the green zone-bar every time; B lets the gold leader-signal and the green/red zone bars co-exist with zero collision. **What shipped:** new `LeaderBadge` component (gold `#e8c547` fill + navy `#1a1f38` text + jersey glyph + i18n `common.leaderBadge` = "Leader"/"Fører") on rank 1 of the 3 true competition standings (StandingsPage, SeasonEndPage, Dashboard mini-standings — chip omitted on the compact widget, leader shown via gold rank# + bar). "Your team" (`isMe`), previously gold on **5** surfaces (Standings, Teams, Dashboard, SeasonPreview, SeasonEnd), reassigned to a **neutral, non-gold** marker: a foreground ring + inverted badge via 3 new theme tokens (`--me-ring`/`--me-badge-bg`/`--me-badge-fg` — navy on the light Chalk canvas, neutral-light in dark since navy is invisible on the navy canvas). TeamsPage + SeasonPreviewPage get no leader chip (rank = catalog / strength, not a competition leader). Also fixed a latent invalid Tailwind class `bg-cz-accent/10/60` (Standings + SeasonEnd). Verified: unmasked Playwright-mock screenshots of `/standings` in **light + dark** (gold chip co-existing with green/red zone bars + neutral "you" confirmed), new forward-guard `standings-gold-leader.spec.js` (all 3 projects — injects a populated division since the default fixture's `season_standings` is empty), 175 frontend tests, build, i18n key-parity, core-smoke 18/18 (no snapshot diff — colour changes sit under the text-masked layout threshold). Next = PR-4 (raw-hex→token sweep).

**🟢 PR-2 (sidebar micro-interactions) SHIPPED 2026-06-04** (`feat/481-phase4-pr2-microinteractions`, PatchNotes 4.76). The two liked interactions from the `phase1-revalidation.html` mockup are now live in the real `NavItem` (the single shared sidebar nav-item in `Layout.jsx`): **(1) gold bullet** per item — active = solid gold `bg-cz-accent`, inactive = muted `bg-cz-sidebar-3` that lights up to `cz-sidebar-2` on hover; **(2) hover accent-dash** — the wordmark's short thick movement-line (20px wide, 2px tall, rounded, gold), absolutely positioned bottom-left, `scale-x-0 → scale-x-100` from `origin-left` on `group-hover` (inactive items only; active needs no affordance). Both decorative (`aria-hidden`); dash is `pointer-events-none` + `motion-reduce:transition-none`. Sidebar is option-A (always navy `#1a1f38`) so the gold renders identically in light + dark; verified via dedicated unmasked Playwright-mock screenshots in **light + dark + mobile-overlay** (gold bullet + dash confirmed in all three). Core-smoke: the bullet + ~16px label-shift sit *below* the masked-layout 0.05 threshold so a normal run stays green, but the 8 **desktop** baselines were force-refreshed (`--update-snapshots=all`) to keep the visual baseline honest; the 3 **mobile** projects are unchanged because the sidebar is `hidden` on mobile viewports (only the topbar renders in those snapshots). Next = PR-3. **Forward ideas** for carrying this micro-interaction / attention-to-detail polish across the rest of the UI (discussion backlog, not locked): [`MICROINTERACTION_IDEAS.md`](MICROINTERACTION_IDEAS.md) — captured 2026-06-04 after Nicolai liked the feel; revisit next UI-polish session.
