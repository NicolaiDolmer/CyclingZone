...
| Railway MCP `list-variables` | `mcp__railway__list-variables` — med en CLI-session returnerer den alle service-variabler i klartekst | Ingen kendt | ✅ Deny'et i `.claude/settings.json` (også v4-navnet `list_variables`), #2409. Brug `scripts/probe-railway-keys.ps1` (kun key-navne). Øvrige Railway-MCP-output (`get-logs`) dækkes af sanitizer. |
...
| Vector | Eksempel-command (FARLIG) | Sidste leak | Mitigation |
|---|---|---|---|
| HTTP-hentning af et Vite-modul | `curl http://localhost:5173/src/lib/supabase.ts`, `Invoke-WebRequest http://localhost:5174/src/main.jsx` | **2026-07-26 ([#3024](https://github.com/NicolaiDolmer/CyclingZone/issues/3024))** — agent curl'ede en Vite-dev-URL under [#3018](https://github.com/NicolaiDolmer/CyclingZone/issues/3018)-arbejde for at inspicere et modul. Sanitizer blokerede outputtet, incident logget. Ingen rotation (publishable anon-key). | **Hent ALDRIG et Vite-moduls INDHOLD over HTTP** (AGENTS.md hard rule 26) — hverken råt eller via en pipe der printer teksten. Læs kildefilen fra disk i stedet: `Read('frontend/src/lib/supabase.ts')` — på disk står `import.meta.env.VITE_*` som *navne*, ikke som værdier. **Eneste snævre undtagelse** (rører ikke "ALDRIG"-forbuddet, fordi intet indhold nogensinde forlader kommandoen): skal du kun vide OM et mønster findes i det serverede modul, og disk-læsning ikke er muligt, må du bruge `curl -s <url> | grep -c -- '<mønster>'` — outputtet er udelukkende et tal, aldrig en linje tekst. **Kopiér kommandoen som den står her, ikke med en `\` foran `|`:** en escaped pipe i en rigtig shell fjerner selve pipen (curl modtager `|`/`grep`/`-c` som rå argumenter i stedet for at sende output til grep), hvorved hele mitigationen falder væk og det rå modul-indhold kan lande i outputtet igen. `grep` uden `-c` (eller `-l`/`-o` der kan printe matchede linjer) er IKKE dækket af undtagelsen. |
...
| `STORT_VARIABELNAVN=<lowercase-hex-værdi>` (fx `COMMIT=<git-sha>`, `SHA=`, `COMMIT_SHA=`, container-digests, checksums, build-id'er) | ✅ Lukket — `=` bandt variabelnavnets STORE bogstaver sammen med værdiens små+cifre til ét token; `looks_like_uppercase_var_assignment` splitter kandidat-tokenet på `=` og kræver at mindst ét segment uafhængigt opfylder entropi-kravet, før fallbacken dømmer | [#4493](https://github.com/NicolaiDolmer/CyclingZone/issues/4493) |
