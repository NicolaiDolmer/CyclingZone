...
| Read/Grep tool på secret-fil | `Read('.env')`, `Read('.mcp.json')`, `Grep('TOKEN', 'backend/.env')` | **2026-05-29** — Discord bot-token dumpet til transcript via `.mcp.json` (ramt 2×: session-læsning + agent-`Grep`). | ✅ **LUKKET 2026-05-29 (#634 follow-up):** **Lag A (primær)** — `block-dangerous-secret-commands.sh` blokerer nu `Read`/`Grep` mod secret-fil-stier (`.mcp.json`, `*.env`, `*.env.*`, `*/secrets/*`; whitelist: `.example/.sample/.template`). Indholds-uafhængig, nul FP. **Lag B (backup)** — `Read\|Write\|Edit\|Grep` tilføjet til PostToolUse-sanitize-matcheren i `.claude/settings.json`, så kendte secret-*patterns* i fil-output også redactes. Begge verificeret i `scripts/test-sanitize-secrets.ps1` + live (Read+Grep mod `.mcp.json` → exit 2; normal kode → exit 0). Langsigtet root-cause: env-injection `${DISCORD_TOKEN}` ved næste rotation. |
