...
| Read tool på .env | `Read('.env')` (Claude Code's tool) | Ingen kendt | ⚠️ **GAP (opdaget 2026-05-29):** sanitizer-hook er KUN koblet på matcherne `Bash\|PowerShell\|mcp__.*` (se project `.claude/settings.json` PostToolUse). `Read`/`Grep` er IKKE dækket — denne rækkes påstand om at "sanitizer fanger output" var aspirationel, aldrig implementeret. Fix-optioner: tilføj `Read`+`Grep` til sanitize-matchere (koster scan på hver fil-read) ELLER PreToolUse-block af `Read`/`Grep` mod secret-fil-mønstre (kirurgisk). |
| Read/Grep tool på .mcp.json | `Read('.mcp.json')`, `Grep('DISCORD_TOKEN', '.mcp.json')` | **2026-05-29** — Discord bot-token dumpet til transcript (sanitize-hook dækkede ikke Read/Grep; se gap-rækken ovenfor). Ramt 2× samme dag: session-læsning + en agent-Grep under denne issue. | `.mcp.json` er gitignored = git-sikker, men indeholder råt token. Langsigtet: env-injection `${DISCORD_TOKEN}` så fil-læsning aldrig eksponerer value. Kortsigtet: luk Read/Grep-gappet (se ovenfor). |
