...
| 12 | **Afventer ejer-beslutning:** flere ryttere kan samtidig sættes til `hunter` på etape-taktikken (`race_stage_roles`) — `EXCLUSIVE_ROLES` og backend-guarden håndhæver kun `captain`/`sprint_captain`, mens holdudtagelsen (`race_entries`) håndhæver `hunter` unikt via DB-constraint. Målt 3/9: 119 af 760 hold-etape-hunter-grupper (15,7 %) har mere end én hunter, op til 6 samtidig. A/B-forslag + begrundelse i [decision-spec](superpowers/specs/2026-09-03-role-vs-teamorder-decision.md) §5 | [#2405](https://github.com/NicolaiDolmer/CyclingZone/issues/2405) |
