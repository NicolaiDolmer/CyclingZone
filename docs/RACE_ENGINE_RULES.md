...
**Grupettoen regner på grænsen ([#5581](https://github.com/NicolaiDolmer/CyclingZone/issues/5581), ejer 23/9: "følg realismen").** En rytter på `grupetto` kører så langsomt som muligt, men aldrig langsommere end grænsen tillader, så længe han har reserven til det — som i virkeligheden, hvor vejkaptajnen regner grænsen ud på forhånd. Motoren forudsiger vindertiden ud fra frontens tempo mod etapens profil og fordeler det resterende budget (lidt inden for grænsen) jævnt over resten af etapen; ligger grupetto-tempoet til at ryge ud, hæves det lige præcis nok, aldrig over rytterens egen CP. Sidder han i en blandet gruppe der er for langsom, går han selv frem, og de andre sidder på hans hjul. Med en tømt reserve kan han ikke holde det og ryger stadig ud (et knæk eller et uheld kan stadig sende ham hjem, og en svag rytter alene kan stadig ryge ud). Ingen rng, ingen tal til spilleren (fog). Grupettoen jager heller ikke op i finalen og placeres efter alle andre i kontendent-puljen: grupetto er aldrig et resultat. Regel: `mechanics/grupettoPace.ts`; kobling: `segmentLoop.applyGrupettoPaceFloor` og `finale.ts`.
...
**Juryen dækker også to tilfælde, genkørslen 26/9 fandt ([#5515](https://github.com/NicolaiDolmer/CyclingZone/issues/5515), PR [#5797](https://github.com/NicolaiDolmer/CyclingZone/pull/5797)).** Begge lukker et hul i §9 række 4 (et uheld må aldrig koste løbet):
- **Offeret i grupettoen.** Lander hans tid minus uheldets tid inde i en reddet grupetto (inden for ankomstvinduet fra en af dens ryttere), genindsættes han også. Uden uheldet var han kommet i mål med grupettoen og var blevet reddet med den. Før sammenlignede juryen kun med grænsen, så en punktering der skilte ham fra grupettoens kæde, kostede løbet. Regel: `juryReinstatements` i `mechanics/timeLimit.ts`.
- **Enkeltstarten.** En enkeltstart kører gennem tidskørsels-kernen (§2h), som kun havde holdgrænsen. Nu dømmer juryen hver rytter på sin tid minus uheldets tid, som på vejetapen. Der er ingen holdkammerat på vejen og intet jagt-tab. En genindsat rytter beholder `finished` med `reinstated_by: "jury"`. Regel: `applyTeamTimeLimit` med `jury` i `mechanics/teamTimeTrial.ts`, kaldt fra `mechanics/individualTimeTrial.ts`. Holdtidskørslen har stadig kun holdgrænsen (se §2h).
...
>
> **Hale-båndet er nu selv ejer-låst (7/9, §9 række 13, genåbn ikke):** bjerg/højbjerg 6-12 %, fladt 0-2 %, målt på p90 af den rene hale (uden uheldsramte/udgåede) pooled over 3 seeds. Håndhæves som en GATE, ikke kun en rapporteret række, af `backend/scripts/v4TailSpread.js --gate` (`LOCKED_TAIL_BANDS`/`evaluateTailGate`). Målt 7/9 mod denne branch (S3-kalenderen, felt 180, seeds s1-s3): bjerg 9,1 % (8,99-9,19), højbjerg 7,25 % (6,60-7,67), fladt 0,20 % — alle tre PASS.
...
> ⚠ **Kalibreringen er ikke i mål.** Holddominans-ankeret (`same_team_top10_share_4plus`) ligger på sit **gulv, 0,0 %, i både v3 og v4** og kan derfor hverken bekræfte eller afkræfte at holdspillet virker — det er en regressionsvagt mod det modsatte problem. Derfor måler harnessen nu **beskyttelses-gabet** direkte (`scripts/lib/headToHeadTeamPlay.js`): gennemsnitlig placering pr. rolle, korrigeret for rytterens egen evne-rang i feltet, målt på begge motorer over samme etaper og seeds. **Niveauet er siden 7/9 kalibreret til v3-paritet** (§9 punkt 14, [#4914](https://github.com/NicolaiDolmer/CyclingZone/issues/4914)). Målt på den pinnede 7/9-population over 5 seeds, felt 180, `--orders=ai`: v3's eget gab er **7,10 pladser** (3,67-9,99), v4 lå på **-0,06** (-2,01-2,10) før og står på **7,35** (4,38-10,19) efter. Det tidligere tal **19,4 pladser** for v3 stammer fra juli-populationen og er forældet — det er 7,10 der er referencen mod den re-eksporterede population ([#4936](https://github.com/NicolaiDolmer/CyclingZone/issues/4936)). Ingen anker skiftede dom ved kalibreringen, og hale-gaten (§9 punkt 13) er PASS før og efter. Holdspillet er dermed ikke længere en flip-blokker; det er `same_team_top10_share_4plus` som **måleinstrument** der stadig er stumpt. **Holdarbejdet bogføres bevidst ikke i `RiderLoad.work_norm`**: det tal er segment-loopets arbejde i motorens egne enheder, og et holdspils-led ville skulle opfinde en omregning fra "andel af CP". At holdarbejde også skal koste i træningsudbyttet er rigtigt, men det hører i løbsdags-udviklingen ([#4850](https://github.com/NicolaiDolmer/CyclingZone/issues/4850)/D2) sammen med intentionens egen udbytte-multiplikator — ikke i en opfundet enhed i motoren.
...
| **Uheld (M10)** ([#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915)) | samme trappe, risiko og etape-loft som på en vejetape. Holdet er ankomstgruppen, så et tidstab hos en rytter der er med holdet koster **holdets** tid; en droppet rytter taber kun selv. Et alvorligt styrt tager rytteren ud, og holdet kører videre uden ham |
| **Tidsgrænse (M15)** ([#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915)) | en **holdgrænse**: holdets tid mod det vindende holds tid med etapetypens faktor (§2d). Over grænsen er hele holdet ude. Ingen grupetto-redning — et helt hold er altid en "stor gruppe", så redningen ville ophæve grænsen |
| **Point (M9)** ([#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915)) | målpassagen køres på placeringsrækkefølgen med ttt-skalaen og uden bonussekunder — præcis det v3's lag giver (paritet testet i broen). Inden for holdet afgør den faktiske ankomst rækkefølgen, så en droppet rytter står bag holdkammeraterne |
...
**Kalenderen har ingen TTT, heller ikke i S4.** `ttt`-filleren har været slået fra siden [#2411](https://github.com/NicolaiDolmer/CyclingZone/issues/2411) — *"pauset indtil motoren kan simulere ægte hold-TTT"* — og S3 har derfor nul TTT-etaper (de eneste to `ttt`-rækker i basen er S1-rester uden rute). Ejeren valgte 23/9 ([#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915) valg 5 = B): **S4 får ingen TTT**, og holdtidskørslen gøres ægte til S5. S5-sporet står på [#3463](https://github.com/NicolaiDolmer/CyclingZone/issues/3463): formatvalg (klassisk eller nyt, et ejer-valg), egen tid til droppede ryttere ([#2412](https://github.com/NicolaiDolmer/CyclingZone/issues/2412) v2), head-to-head-måling på syntetiske TTT-etaper uden at røre de pinnede proxy-etaper, samt ordning, distance og `tt_finale`.

> **Lukket bag `race_engine_v4` ([#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915)):** de tre huller fra wiringen 6/9 — uheld og tidsgrænse rørte ikke en TTT, en TTT gav ingen point, og `same_team_top10_share_4plus` målte strukturelt 100 % på en holdtidskørsel — er bygget som i tabellen ovenfor. Ankret udelader nu tidskørsler (generatorens `isTimeTrial`: `itt`, `itt_hilly`, `ttt`); værdien på de pinnede proxy-etaper er uændret, kun antallet af målte etaper falder ved næste baseline-regenerering.
>
> ✅ **Ejer-valgene er afgjort (23/9, [#4915](https://github.com/NicolaiDolmer/CyclingZone/issues/4915)):**
> - **TTT i S4-kalenderen: nej.** Den gøres ægte til S5 (se ovenfor). `TTT_ENGINE_SUPPORTED` og filleren forbliver slået fra.
> - **Passage-rækken for en rytter der vinder en indlagt spurt og senere udgår eller bliver OTL: behold** (§2g). Pointene blev vundet på vejen.
> - **v3's manglende bonusloft: gør intet.** v3 går på pension ved flippet. Glider flippet, bygges loftet også i v3.
> - **Et uheld hos en rytter der er med holdet: behold det byggede** (holdet venter). Inert i S4, gøres ægte sammen med formatet til S5.
>
> 🟡 **Bygges til S5, ikke før:** juryen (§2d) gælder også holdtidskørslen (ejer 23/9). Holdets tid dømmes minus den tid holdets uheld kostede, så ét uheld ikke kan sende et helt hold ud på tid. I dag har holdtidskørslen kun holdgrænsen; det er inert i S4, fordi kalenderen ingen TTT har. Enkeltstarten har juryen allerede (§2d, [#5515](https://github.com/NicolaiDolmer/CyclingZone/issues/5515)).
...
| 9 | **Prod har evnerne, gaten har dem ikke endnu (#5572, 24/9).** `positioning` og `tactics` står i `AbilityKey` og vægter i finalens demand-vektorer. I prod har stort set alle ryttere begge evner. Den pinnede gate-population (`backend/scripts/baselines/population-snapshot-2026-09-07.json`, 5.955 ryttere) har dem ikke, fordi eksporten kun skrev klassifikatorens 13 evner — i §7b falder vægten derfor stadig tavst på gulvet. Eksporten skriver nu alle registry-evner, og en **ny pin findes**: `population-snapshot-2026-09-24.json` (hold-id'er pseudonymiseret, sha256 i `backend/scripts/baselines/README.md`). Målt side om side i tre kolonner på samme motor-sha (gammel pin / ny pin med de to evner sat til 0 / ny pin): evnerne alene skifter ingen v4-ankerdom, mens populationsskiftet alene vender bjergetape-top-10-ankerets dom (samme fælde som række 20-21). **Gaten er uflyttet** (§7b, `v4-anchor-baseline.json` og `POPULATION_FILE`-default er uændrede) og venter på ejer-go. Tal: `balance-internals/` (hard rule 17) | [#4604](https://github.com/NicolaiDolmer/CyclingZone/issues/4604) · [#4936](https://github.com/NicolaiDolmer/CyclingZone/issues/4936) · [#5572](https://github.com/NicolaiDolmer/CyclingZone/issues/5572) |
| 10 | **Felt-favoritters win-rate er stadig rød, tallet genmålt mod main `0daee3c21` (#4988+#4998+#4975 merget ind).** Pinnet 3-seed-måling (§7b): **62,6 % (58,9-66,0 spænd)** mod båndet 25-40 %, orders=none (ordre-effekten er en anden akse, se punkt 7 i denne tabel). Stigningen fra 54,1 % er holdspils-kalibreringens egen effekt (#4988, §9 punkt 14: beskyttelses-gabet løftet til v3-paritet), ikke en ny kalibrering af dette anker. Ankeret er ejer-gated: forfølges ved at koble flere mekanikker på og lade AI-ordrer skabe reel variation, **aldrig ved at straffe styrke** (ejer 4/8). **Genmålt 23/9 på 5 seeds (#4914, §7b):** 61,6 % (58,9-64,5) — stadig FAIL, rapporteret, ikke løst. **#5583 (24/9), målt side om side, gaten og båndet urørt:** gatens v4-favorit er næsten cirkulær (finalen sorterer på samme evne-score), så scorecardet viser nu også v4 målt med v3's egen favorit (højeste v3-terrainScore mod etapens v3-kravvektor; v3 og v4 peger så på samme rytter på stort set alle etaper). Kvalitativt: den ikke-cirkulære v4-rate ligger mellem v3's og gatens, og er stadig over båndet. Afstanden til virkeligheden er størst på fladt, hvor favoritten også med den nye definition vinder langt de fleste etaper; kuperede etaper og bjergetaper ligger også over, mens enkeltstarten er på niveau med virkeligheden. Virkelighedens målestok (research med kilder: Frontier Economics' udbrudsanalyse af Touren og en optælling af seks grand tours 2022-2025 fra Wikipedia/PCS' etapelister): feltets stærkeste vinder et mindretal af etaperne på alle vejetapetyper, mest på fladt og i enkeltstart, klart mindre på kuperede etaper og bjergetaper, hvor udbrud tager en stor del. Forbehold: målt på den pinnede population uden taktik/positionering (#5572 ikke merget), så tallene er foreløbige; harnessens felt er en tilfældig stikprøve af hele populationen, ikke et WorldTour-felt, hvilket formentlig i sig selv gør den stærkeste mere dominerende (antagelse, ikke målt). Tal og kilder: `balance-internals/5583-favorit-definitioner/`. Gate-skiftet (A/B) er ejerens | [#4604](https://github.com/NicolaiDolmer/CyclingZone/issues/4604) · [#5583](https://github.com/NicolaiDolmer/CyclingZone/issues/5583) |
| 11 | **Bjerg-top-10-ankeret er kun meningsfuldt ved realistisk feltstørrelse.** Samme kode scorer ~132 s ved 180 ryttere (§7b, genmålt 7/9 mod den nye pinnede population) og 19 s ved hele populationen ved den gamle **5.650**-population (rettet fra 5.938) — målet er sekundbaseret og skalerer med feltet, fordi en stor peloton giver en stor frontgruppe. Populationen er siden #4936 re-eksporteret til **5.955** ryttere (`population-snapshot-2026-09-07.json`); hele-populationen-tallet (19 s) er IKKE genmålt mod den nye fil i denne session (kørslen tager markant over harnessets normale 2-5 sek og blev ikke ventet ud) — flag til en opfølgende session, ikke en påstand her. Det pinnede 180-rytters felt er gaten (§7b's baseline), jf. scorecard-metodologien 23/8; hele-populationen-tallet er uanset population ikke et mål for dette anker | [#4604](https://github.com/NicolaiDolmer/CyclingZone/issues/4604) · [#4936](https://github.com/NicolaiDolmer/CyclingZone/issues/4936) |
...
| 13 | **RETTET (#4975, 7/9): sprinter-ankeret og felt-sammenhængen trækker ikke længere mod hinanden.** Rodårsagen var at jagt-modellen ikke talte feltets antal med; rettelsen gater nu på gruppestørrelse. Pinnet 3-seed-måling efter merge (§7b, main `0daee3c21`): felt-sammenhængen er **88,0 % (85,9-91,4)** mod bånd 80-95 % PASS, sprinter-ankeret er **100,0 % (100,0-100,0)** mod bånd ≥ 90 % PASS. Begge grønne | [#4615](https://github.com/NicolaiDolmer/CyclingZone/issues/4615) · [#4975](https://github.com/NicolaiDolmer/CyclingZone/issues/4975) |
| 14 | ~~**To led i jagt-modellen er absolutte konstanter målt mod en evne-relativ skala**~~ **RETTET bag flag ([#4707](https://github.com/NicolaiDolmer/CyclingZone/issues/4707), 22/9).** Oprindelig modsigelse (samme fejlfamilie som #4604/#4606): sen-etape-uroen og udbruddets størrelses-bonus stod som absolutte konstanter, mens de evne-afledte led skalerede med populationen, så samme relative scenarie gav en anden jagt ved median-evne 11 end ved 60. En naiv relativisering blev prøvet og **målt 3/9: den forværrede bjerg-ankeret** (207 → 247 s mod bånd 180-240) og blev rullet tilbage. Rettelsen: de evne-afledte led måles nu relativt til feltets EGEN evne-reference, og kalibrerings-referencen sidder ved den pinnede populations niveau — netto-jagten er dermed identisk på evne-niveau 5/11/30/60/99 for samme relative felt (`mechanics/breakaway.test.ts`, #4707-testene), og et prod-felt ligger på samme sted som før, så bjerg-ankeret ikke flyttes. Målt 5-seed-middel før/efter: bjerg-top-10 197 → 197 s PASS, sprinter-vinderrate 98,9 → 98,9 % PASS, nedkørsel/summit 0,40 → 0,39 PASS, felt-sammenhæng 88,1 → 88,1 % PASS, felt-favoritter 61,7 → 61,6 % (stadig rød, ejer-gated, række 10). At flytte referencen mod "midt-skala" (den naive retning) blev målt igen og skubber bjerg-ankeret op mod loftet og favorit-raten op — derfor ikke valgt | [#4615](https://github.com/NicolaiDolmer/CyclingZone/issues/4615) · [#4707](https://github.com/NicolaiDolmer/CyclingZone/issues/4707) |
| 15 | ~~**v4 komprimerer feltet så hårdt at tidsgrænsen (§2d) aldrig bider.**~~ **Løst 7/9 ([#4885](https://github.com/NicolaiDolmer/CyclingZone/issues/4885)).** Målt 6/9: 0 OTL og 0 grupetto-redninger på 984 etapekørsler, største spredning 6,1 % på bjerg mod en 15 %-grænse. Grænsen blev ikke trimmet; motoren blev rettet. Rod-årsagen var **ikke** gruppe-låsning men fart-modellen: styrke-leddet målte gruppens CP som en *absolut* difference mod en konstant kalibreret for et midt-skala felt (samme fejlfamilie som #4604 rettede på krav-siden og som overlevede på fart-siden), hvilket låste hele hale-spændet på ~4 % uanset mekanik — og W' blev aldrig læst tilbage i den bæredygtige tærskel, så en kørt-i-sænk rytter kunne ikke blive langsommere, kun mindre. Efter rettelsen (genmålt 7/9 mod main `d7db448ac`, dvs. oven på #4886 og #4905): hale-p90 på bjerg 2,7 % → 9,1 %, på fladt uændret 0,2 % (feltet skal netop *ikke* splittes dér), og M15 fyrer (0,06-2,0 % OTL på bjerg/kuperet/rullende/klassiker, grupetto-redninger på 7 etaper). **Hale-båndet er nu ejer-låst og håndhæves som gate** (§9 række 13, ikke længere et startgæt): bjerg/højbjerg 6-12 %, fladt 0-2 %, `backend/scripts/v4TailSpread.js --gate`. **Prisen er bjerg-top-10-ankeret:** det er nu grønt på 1 af 3 seeds (242 ❌ / 165 ❌ / 228 ✅ mod båndet 180-240) — men s2 var allerede rødt på main efter #4886 (177), og resten er en åben kalibrering af overskuds-grenen, se række 16. Måling: [`audits/v4-tail-spread-2026-09-07.md`](audits/v4-tail-spread-2026-09-07.md) | [#2582](https://github.com/NicolaiDolmer/CyclingZone/issues/2582) · [#4604](https://github.com/NicolaiDolmer/CyclingZone/issues/4604) · [#4885](https://github.com/NicolaiDolmer/CyclingZone/issues/4885) |
| 16 | **Bjerg-top-10-ankeret (180-240 s) og halens længde trækker mod hinanden** (historisk 7/9: grønt på 1 af 3 seeds; aktuel status nederst i rækken). Genmålt 7/9 mod main `d7db448ac` (historisk måling): 242 / 165 / 228 s. To uafhængige bidrag, målt hver for sig: rng-segment-nøglingen ([#4886](https://github.com/NicolaiDolmer/CyclingZone/issues/4886)) flyttede main selv fra 200/182/216 til 220/**177**/212 — s2 var altså rødt før #4885 — og hale-rettelsens overskuds-gren lægger yderligere 10-22 s på. Ankerets margin er dermed opbrugt i begge ender: s1 er 2 s over loftet, s2 15 s under gulvet. At dæmpe overskuds-grenen yderligere koster hale-længde og er en **kalibrering med ejer-go** (§4 *"Simulér før ship"*), ikke en wiring-ændring. **Status 22/9 (genmålt under #4707):** efter overskuds-grenens kalibrering 7/9 (#4914) er ankeret grønt på alle tre pinnede seeds i §7b (spænd 200-220 s) og i 5-seed-middel (197 s); på 5 seeds ligger det laveste enkelt-seed (164 s) stadig under gulvet, jf. række 8. **Status 23/9 (#4914):** §7b er nu selv 5-seed-gaten — 197 s (164-220) PASS på middel; det laveste enkelt-seed under gulvet er synligt i tabellens spænd i stedet for kun i denne række | [#4885](https://github.com/NicolaiDolmer/CyclingZone/issues/4885) · [#4886](https://github.com/NicolaiDolmer/CyclingZone/issues/4886) · [#2415](https://github.com/NicolaiDolmer/CyclingZone/issues/2415) |
...
| 18 | ~~**NY (#4911): hvert v4-løb rapporterer stadig samme sejrstype.**~~ **RETTET ([#5577](https://github.com/NicolaiDolmer/CyclingZone/issues/5577), spor M2, 26/9).** Finalen (`finale.ts`) afgør sejrstypen én gang via `winType.ts` ud fra motorens egen viden: hvor mange der kæmpede om sejren, feltets størrelse, om finalen er et samlet feltopgør, og om puljen kun var et udbrud. `solo_win` = én mand i puljen; `sprint_win` = massefinale hvor puljen er feltet (samme "feltet"-definition som finale-jagten); ellers `close_win`. `itt_win`/`ttt_win` som før. `sprint_decided` udsendes kun ved `sprint_win`; ellers et `finale_attack` (kind `stage_decided`). `buildFinishEvent` læser finalens afgørelse fra tidslinjen, broen giver vinderens række sejrstypen og motorens udbrudsdom, og `raceNarrative` bruger dem i stedet for gap-tærsklerne (som fortsat gælder v3). Ingen placering ændret: kun stemplerne i de golden fixtures er regenereret. Oprindelig modsigelse: `buildFinishEvent`'s `PLACEHOLDER_WIN_TYPE = "group_finish"` (`index.ts:159,168`) stemples ubetinget på ALLE massestarts-etaper uanset om det var massespurt, solo eller udbrud — kommentaren derover ("F2-placeholder... uden en reel finale-mekanik") er selv forældet, for finalen (M4) ER koblet ind siden F2. De fire golden fixtures (`fixtures/*/expected.json`) fryser fejlen fast. TTT er undtaget: `mechanics/teamTimeTrial.ts:402` sætter korrekt `win_type: "ttt_win"`. Ejer-valg om finale-klassificering, ikke rettet her | [#3855](https://github.com/NicolaiDolmer/CyclingZone/issues/3855) |
| 19 | ~~**NY (#4911): aggregerings-koden modsiger stadig sin egen beskrivelse.**~~ **RETTET ([#4947](https://github.com/NicolaiDolmer/CyclingZone/issues/4947), PR [#4966](https://github.com/NicolaiDolmer/CyclingZone/pull/4966), 7/9).** `bandById` i `aggregateScorecards` læser nu `AGGREGATION_BAND_BY_ANCHOR_ID`, som har en indgang for hvert anker `buildScorecard` returnerer, så alle ankre dømmes på seed-middelværdien mod båndet. En forward-guard i `headToHeadAnchors.test.js` fejler hvis et nyt anker tilføjes uden indgang. Oprindelig modsigelse: `aggregateScorecards` sagde at domme afgøres på middelværdien mod båndet, men `bandById` havde kun 10 opslag mens `buildScorecard` returnerede 13 ankre, så `descent_attack_gain_bounds`, `breakaway_rate_per_terrain` og `bonus_seconds_bounded` faldt tilbage på `measured[0].verdict` (dommen for det FØRSTE seed, ikke middelværdien). Maskeret i §7b's tal dengang, fordi alle tre ankre var seed-uafhængige konstanter i den kørsel (spænd 0) | [#4604](https://github.com/NicolaiDolmer/CyclingZone/issues/4604) |
| 20 | **NY (#4936): to §7b-ankre skiftede dom rent af populationsskiftet** (juli-snapshot 5.650 ryttere → re-eksporteret 5.955 ryttere 7/9, samme motor-sha, samme etaper, samme seeds). Sprinter-vinderrate på flat gik **FAIL → PASS** (88,6 % (80,0-94,3) → 96,2 % (91,4-100,0) mod bånd ≥ 90 %). Fundet er en genmåling af en skæv/forældet population, ikke en motor-ændring | [#4936](https://github.com/NicolaiDolmer/CyclingZone/issues/4936) |
| 21 | **NY (#4936): samme populationsskift vendte bjergetape-top-10-ankeret modsat vej.** Gik **PASS → FAIL** (212 s (165-242) → 132 s (121-148) mod bånd 180-240 s). Sammen med række 20 bekræfter det at ankertabellen (§7b) var kalibreret mod en for lav-skæv population — begge tal er nu tættere på den ægte prod-fordeling (se §7b's pin-linje), men selve dommene skifter som en direkte konsekvens. Ingen tuning-ændring lavet her, kun genmåling | [#4936](https://github.com/NicolaiDolmer/CyclingZone/issues/4936) |
| 22 | **NY (#4914, 23/9): indsatsvalget er usynligt for alle 13 ankre, og på den pinnede population er et LAVERE valg ofte bedre end `normal`.** Ankertabellen kører `orders=none` (alle på `normal`), og `--orders=ai` sætter aldrig `all_out` eller `grupetto`, så ingen anker kan fælde eller bekræfte en M12-ændring — målingen er `backend/scripts/v4EffortTwinMeasure.js` (tvillinger i samme løb, 5 seeds, pinnede filer). Kvalitativt fund: på kuperede og bjergetaper slår en tvilling på `grupetto` eller `save` i langt de fleste løb sin tvilling på `normal` for en rytter midt i feltet, fordi `normal`-rytteren kører sig i sænk (W'-udmattelsen fra #4885) og falder af, mens den sparende bliver i gruppen. `save` giver samme beskyttelse, så årsagen er kollaps-modellen, ikke grupetto alene (PR #4909's tvivlspunkt 2, nu målt på den ægte population). Det strider mod §9 punkt 3 ("grupetto er ikke et frikort"). **Grupetto-tempo er en ejer-valgt kontakt** (`GROUP_TEMPO_EFFORT_EXTRA_TUNING.model`; ejeren valgte `effort_weighted` 23/9, default siden [#5581](https://github.com/NicolaiDolmer/CyclingZone/issues/5581); `cp_only` er den gamle regel): `effort_weighted` er modellen "grupettoen er den sidste gruppe på vejen" i to led der kun virker sammen — gruppens tempo følger dens indsats (`segmentLoop.groupEffortTempo`), og en grupetto-rytter falder tilbage på stigninger fra en gruppe hvor andre kører (`climbSelection.grupettoDropBackForced`). Tempo-leddet alene flytter målt næsten intet; tilbagefaldet alene er #4909's tilbagerullede forsøg. Målt sammen (5 seeds): grupetto slår ikke længere `normal`, en stærk grupetto-rytter vinder ikke, ingen af ankrene skifter dom, hale-gaten er PASS, og færre ryttere ender uden for tidsgrænsen. **#5581 (ejer 23/9, valg 1b):** grupettoen regner på tidsgrænsen (§2d) — en stærk rytter på grupetto ryger ikke længere ud uden et uheld, en svag rytter alene kan stadig. Tal: `balance-internals/` (hard rule 17) | [#4914](https://github.com/NicolaiDolmer/CyclingZone/issues/4914) · [#4909](https://github.com/NicolaiDolmer/CyclingZone/pull/4909) |
| 23 | **NY (#5578, 25/9): de to umålte ankre har nu en måling, men udbrudsbåndet er en KANDIDAT, ikke et ejer-godkendt mål.** (1) **Udbrud pr. terræn:** v4 afgør selv om etapen blev vundet fra udbruddet (`groups.isBreakawayWin`, eksponeret via `simulateStageV4WithTrace`): vinderen kørte i en gruppe med udbrudsoprindelse (art breakaway/solo), dannet før finale-segmentet og aldrig hentet, heller ikke i selve finalen. Oprindelsen er et additivt `origin`-felt på `RaceGroup`: M5-formationen sætter den, et split arver den, et merge med en ikke-udbrudsgruppe fjerner den (indhentet), og et nedkørselsangreb ud af feltet får sin egen oprindelse og tæller aldrig. Tidskørsler er udeladt. Dommen er pr. terræn mod **kandidatbåndene fra #1021** (v3-kalibreringen, `BREAKAWAY_TARGETS` i `simulateSeasonDryRun.js`): flad 1-10 %, rullende 4-15 %, kuperet 15-45 %, bjerg 15-50 %, højbjerg 0-15 %, brosten 2-15 %; PASS kun når alle målte terræner ligger inden for. Issuet og spec'en siger at det endelige bånd sættes fra virkelige udbrudsrater (research med kilder) og godkendes af ejeren; indtil da er en FAIL et fund, ikke en gate. (2) **GT-vindermargin:** `backend/scripts/v4GcMargin.mjs` kører de pinnede grand tours med ét fast felt i begge motorer (tid minus bonussekunder, udgåede og OTL ude) og dømmer middelmarginen mod #2415's 1-8 min. Ankertabellen (§7b) og flip-klar-rapporten opdateres først i B13 (#5515). Tal: `balance-internals/5578-v4-ankre/` | [#5578](https://github.com/NicolaiDolmer/CyclingZone/issues/5578) · [#1021](https://github.com/NicolaiDolmer/CyclingZone/issues/1021) · [#2415](https://github.com/NicolaiDolmer/CyclingZone/issues/2415) |
| 24 | ~~**NY (26/9): løbsfilmen kunne skrive "udbruddet holdt", når det blev hentet i finalen.**~~ **RETTET bag flag ([#5515](https://github.com/NicolaiDolmer/CyclingZone/issues/5515)).** `mechanics/breakaway.ts` udsender `breakaway_survived` på sidste segment, før finalen, og dér betyder det kun at udbruddet stadig er sin egen gruppe. Filmen (`stageTimelineFilm.js`) oversatte det til "udbruddet holder feltet fra livet helt til stregen", også når udbryderen blev hentet (golden fixture bjerg-selektion: han blev nr. 5). Nu gør `simulateStageV4` udfaldet op efter finalen (`settleBreakawaySurvivedEvents` i `groups.ts`): eventet bliver kun stående, når motorens udbrudsdom (`isBreakawayWin`, samme som etape-fortællingen bruger, #5577) siger at udbruddet vandt, og gruppen kom i mål foran alle uden for udbruddet. Ellers skrives det om til `breakaway_caught` på målstregen med samme gruppe og de af dens ryttere der kom i mål. Er hele gruppen udgået, udelades udfaldet. Ingen ny event-type, ingen ny copy, ingen placering ændret; kun bjerg-selektions stempel er regenereret. En fixture-test låser at filmen og fortællingen aldrig er uenige | [#5515](https://github.com/NicolaiDolmer/CyclingZone/issues/5515) |
...
Tabellen herunder er IKKE håndskrevet. Den er renderet af `backend/scripts/renderV4AnchorTable.mjs` fra `backend/scripts/baselines/v4-anchor-baseline.json`, som `backend/scripts/buildV4AnchorBaseline.mjs` producerer ved at køre `headToHeadV4.js` på to PINNEDE, committede filer (population + proxy-etaper) over fem låste seeds (s1-s5; udvidet fra tre i #4914, jf. §7 række 8's 5-seed-gate — s1-s3 er de samme som før). Det retter audit-modsigelsen "gaten er ikke pinnet nogen steder" — samme to filer og samme fem seeds hver gang, så et fremtidigt kalibrerings-PR kan sammenligne mod netop denne baseline i stedet for en tilfældig kørsel. Hale-gaten (`v4TailSpread.js --gate`) er fortsat ejer-låst på tre seeds (§9 række 13).
...
**Hvad tabellen IKKE dækker:** `orders=none` (ingen AI-taktik/leadout-ordrer i dette scorecard — det er en anden akse, se §7 punkt 7's note om "wiret ≠ virker"), GT-vindermargin (kræver akkumuleret GC over et helt etapeløb, ikke etape-for-etape; måles siden #5578 af `backend/scripts/v4GcMargin.mjs`, se §7 punkt 23, og kommer ind i tabellen i B13), og de tre ankre nævnt i §7 punkt 19 (aggregerings-bug, pt. ufarlig fordi deres værdier er seed-konstante i denne kørsel).

> ⚠ **Tallene i tabellen er fra 23/9 og ældre end main-motoren (26/9, [#5515](https://github.com/NicolaiDolmer/CyclingZone/issues/5515)).** Den gældende dom pr. anker mod main-motoren står i [flip-klar-rapporten 26/9](audits/2026-09-26-v4-flip-klar-rapport.md#målingerne) (måling 1): 12 PASS, 2 FAIL (felt-favoritternes vinderrate og udbrud pr. terræn), GT-vindermarginen målt for sig og bestået. Den rapport har de nye ankre (enkeltstart, udbrud pr. terræn for v4), som tabellen her mangler. Tabellen er **ikke** regenereret: det ville skrive nye målte værdier ind i et offentligt dokument (hard rule 17), og siden 26/9 står middel, spænd og rater kun i den private fil. At rendere tabellen som dom uden tal (samme form som rapporten) er et lille script-skift i `renderV4AnchorTable.mjs` og afventer et go. Et skift til den nye population ([#5572](https://github.com/NicolaiDolmer/CyclingZone/issues/5572)) venter også på dit go.
...
>
> Pinnet: motor-sha `ee5b304a33` · population `backend/scripts/baselines/population-snapshot-2026-09-07.json` (5955 ryttere, sha256 d59b8cf3d03b2d71) · etaper `backend/scripts/baselines/v4-proxy-stages-2026-09-06.json` (sha256 bbf6e20ce5293280) · seeds s1, s2, s3, s4, s5 · feltstoerrelse 180 · genereret 2026-09-23T00:49:34.520Z.
>
...
|---|---|---|---|
| Felt-sammenhaeng, flade etaper | 80.0%-95.0% (#3917-maalingen (mor-spec §5: "80-95% af feltet paa vinderens tid")) | 2.1 % (1.8 %-2.5 %) [FAIL] | 88.1 % (85.9 %-91.4 %) [PASS] |
| Nedkoersels-gaps vs. summit-gaps (ratio) | <= 0.5 (#3426-maalingen (mor-spec §5: nedkoersels-gaps vs. summit-gaps, ratio <=0,5 ved p5-p10)) | 0.58 (0.47-0.62) [FAIL] | 0.39 (0.24-0.58) [PASS] |
...
| Punch-korrelation (punch-evne vs. placering paa punch-etaper) | spearman > 0.2 (#3965-harnesset (mor-spec §5)) | 0.75 (0.74-0.76) [PASS] | 0.83 (0.82-0.84) [PASS] |
| Brostensevnens loeft paa brosten/grus (spearman-forskel vs. flad) | loeft >= 0.03 (FORSLAG, ikke ejer-godkendt) (FORSLAG (M8-wiring 6/9, #2789/#4105) — regressionsvagt for ejer-reglen 3/9 "brostensevnen taeller kun paa etaper med brosten/grus"; taersklen er valgt af denne harness, ikke ejer-godkendt) | 0.295 (0.277-0.324) [PASS] | 0.062 (-0.010-0.143) [PASS] |
| Felt-favoritters win-rate | 25.0%-40.0% (v3-spec §2 + ejer-valg 20/8 (mor-spec §5: "i dag 80-88%")) | 38.4 % (34.8 %-44.0 %) [PASS] | 61.6 % (58.9 %-64.5 %) [FAIL] |
...
| 13 | **Hale-bånd: bjerg/højbjerg 6-12 %, fladt 0-2 %, p90 uden uheldsramte; begrundelse: 20-30 min på 5-6 timer = 6-10 %.** Håndhæves som gate af `backend/scripts/v4TailSpread.js --gate` (`LOCKED_TAIL_BANDS`, [#4885](https://github.com/NicolaiDolmer/CyclingZone/issues/4885)). Andre etapetyper (hilly, rolling, classic, cobbles, itt m.fl.) har ingen ejer-bånd og rapporteres kun. Se §2d's opfølgende note og §7 række 15 | 7/9 |
