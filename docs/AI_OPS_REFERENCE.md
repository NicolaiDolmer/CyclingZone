...
- **Sikkerhedskontrakt (læring 24/9):** hver serie kører i en KOPI af profilen, som slettes bagefter (kopier efterladt af en dræbt kørsel ryddes ved næste shoot). Alle skrivende kald (API, Supabase REST/RPC, PostHog, Sentry) besvares lokalt med 204, undtagen Supabase' token-refresh; et `signOut` tælles i rapporten uden at ramme mesterprofilen; service workers blokeres. Login-proben (`/dashboard` lander ikke på `/login`, intet 401 fra API'et) kører før serien. Scriptet læser aldrig tokens, cookies eller storage og skruer aldrig browser-uret (`--shot-at` afvises); en anden tilstand fremkaldes med `--mock=<sti>=<json-fil>` (kun GET) eller `--mock-rpc=<funktion>=<json-fil>` (se næste punkt); begge markeres "MOCK" i rapport, log og før/efter-billede.
- **RPC-mocks (`--mock-rpc`, 26/9):** supabase-js' `.rpc()` sender ALTID POST, også for læsende (STABLE) funktioner som `get_season_recap`, `get_season_honours`, `get_season_documentary_facts` og `dashboard_rider_ranking`. En almindelig `--mock` matcher kun GET, så kaldet ramte skrive-vagten og fik et tomt 204: siden viste sin "ingen data"-tilstand, og "har data"-renderingen kunne ikke fotograferes (set 26/9 på SeasonEndPage, #5390: `POST .../rpc/get_season_recap` stod under `blockedWrites` trods en registreret `--mock`). `--mock-rpc=get_season_recap=recap.json` svarer på `POST|GET /rest/v1/rpc/get_season_recap` med filen (eller `=status:<kode>`) FØR vagten ser kaldet; kaldet når aldrig prod. Vagten selv er uændret (`isWriteRequest` siger stadig "skrivning"); rækkefølgen af Playwright-routes (mocks registreres sidst, sidste vinder) er mekanismen. Tag kun funktionsnavnet, ikke stien. JSON-filen skal have PostgREST-svarets form (typisk et objekt for `RETURNS json`, en liste for `RETURNS TABLE/SETOF`); tag den fra et rigtigt svar i Network-fanen, aldrig opfundet data. Build-miljøet renses for `VITE_*`/`SENTRY_*`, så worktreets egne `.env`-filer afgør API'et, og intet PR-build uploader til Sentry.
- **Valg:** `--widths=1440,390`, `--click=<tekst>` eller `--click=css:<selector>` (rapporten skriver hvad hvert klik ramte), `--wait-for=<selector>`, `--mock=<sti>=<json-fil>`, `--mock-rpc=<funktion>=<json-fil>` (begge gentagelige, begge også `=status:<kode>`), `--no-build`, `--headed`, `--dry-run` (plan uden build/browser; viser mocks som `MOCK GET <sti>` / `MOCK RPC <funktion>`). Kør fra PowerShell (Git Bash omskriver `/ruter`; scriptet reparerer det med en advarsel).
