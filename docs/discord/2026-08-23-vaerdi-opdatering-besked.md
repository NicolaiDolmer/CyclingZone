# Spillerbesked · Værdi-opdateringen 23/8 (ejeren poster selv, FØR kl. 18)
...
**Rider values: corrected today (Sunday 23 Aug, before 18:00)**
...
What it means for you: every rider's value drops, yours and everyone else's, by the same factor. Rankings between riders of the same type do not change. Salaries are recalculated from the corrected values at the season switch tonight, so nobody's contract is frozen on the old numbers.

From now on values are reviewed every Sunday. If the market keeps moving, the values follow.

## DA

**Rytterværdier: rettet i dag (søndag 23/8, før kl. 18)**
...
Hvad det betyder for dig: alle rytteres værdi falder, dine og alle andres, med samme faktor. Rangordenen mellem ryttere af samme type ændrer sig ikke. Lønnen genberegnes ud fra de rettede værdier ved sæsonskiftet i aften, så ingen kontrakter fryses på de gamle tal.

Fra nu af gennemgås værdierne hver søndag. Flytter markedet sig, følger værdierne med.
