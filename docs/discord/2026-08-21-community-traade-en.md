# Community-tråde og forum-opslag (EN), ejeren poster SELV

> Skrevet 21/8. Tone-valg (ejer 21/8): **blandet.** Opslag 3 (taktik) og 5 (roadmap) er skrevet i din
> stramme #the-roadbook-stemme, fordi de indeholder forpligtelser. Opslag 1, 2, 4, 6 og 7 er skrevet i
> din løsere #general-stemme: emoji, sidebemærkninger, tænker højt. Stavningen er korrekt overalt
> (dine egne "Im"/"Ill" er charme i chatten, men ser sjusket ud i et opslag du paster ind).
> Ingen em-dash. Ingen løfter ud over det besluttede.
>
> **FAKTA-ANKRE:** 232 konti, 67 Discord-medlemmer (målt 20-21/8) · S2-finale søn 23/8 · S3 første løbsdag tir 25/8, 27 løbsdage
> · taktik-ordrer v1 låst 21/8 (T1-T4: stance chase/neutral/let_go, per-rytter try_break, effort protect/normal/save,
> lås ved etapestart kl. 11, neutrale defaults) · race engine v4 flip-gate mandag aften 24/8 (fallback v3 er låst)
> · dashboard-redesign #4070 + manager-indstillinger #4071 er ejer-noter, køet EFTER cutover-ugen
> · tre-tier Senior/U23/Junior er ejer-låst retning 16/7 (#2492), ikke bygget.
>
> **Discord-grænser:** 2.000 tegn pr. besked, forum-titel maks 100 tegn. Alle opslag ligger under, undtagen 5 (opdelt i 2 beskeder).

---

## Anbefalet rækkefølge (ikke alle på én gang, 67 medlemmer drukner i 7 tråde)

| Dag | Opslag | Kanal |
|-----|--------|-------|
| FRE 21/8 | 4 · September-afstemningen | #feedback-and-ideas |
| FRE 21/8 aften | 1 · Marketing | #feedback-and-ideas |
| LØR 22/8 | 6 · Dashboard | #feedback-and-ideas |
| MAN 24/8 | 5 · Roadmap 3 måneder | #the-roadbook (+ kort version i #annoncements) |
| TIR 25/8 efter første løbsdag | 2 · Race engine-drømmen | #feedback-and-ideas |
| ONS 26/8 (KUN hvis v4 er flippet) | 3 · Taktikskærmen | #feedback-and-ideas |
| TOR 28/8 | 7 · U23 og junior | #feedback-and-ideas |

---

# 1 · Marketing

**Kanal:** #feedback-and-ideas · **Titel:** `If you were me: how would you market this game?` · **Tone:** løs

Okay, honest question, and I mean it completely literally 😄 If you had my keyboard for a week, what would you actually do to get more managers in here?

Where I'm at right now: 232 accounts, 67 of you in this Discord, 0 kroner spent on ads, and one guy building all of it. Everyone who is here found us through word of mouth, or a link somebody dropped somewhere. That's genuinely it. There is no growth plan, I have never made one.

So I would rather ask the people who already chose to stay than sit here guessing ✌️

**1. Where did YOU first hear about Cycling Zone?** Be specific. Which forum, which subreddit, which Discord, which friend at work.

**2. Where do cycling manager people actually hang out?** I know the obvious places. It is the ones I do not know that I need. Non English communities count double.

**3. If you had to sell this game to a friend in one sentence, what would the sentence be?** I am honestly bad at this part. The words you use are worth more than the words I invent sitting alone at my desk.

**4. What is the ONE screenshot that would make you click?** Race results? An auction going completely crazy? A rider profile? Your season calendar?

And then the uncomfortable one, which is probably the most useful: **is there anything in the game right now that you would be a little embarrassed to show a friend?** Better that I hear it from you than that I find out by sending strangers in there 🙂

I read every single reply in this thread.

---

# 2 · Race engine-drømmen

**Kanal:** #feedback-and-ideas · **Titel:** `The race engine: what is your dream version of it?` · **Tone:** løs

The race engine is the heart of this whole thing. Transfers, training, economy, none of it matters except because of what happens between 11:00 and the finish line. So this is the part I care about getting right more than anything else in the game.

Where it is heading: a rewritten engine with a real energy model per rider, proper group dynamics, a descent model where a good descender can actually gain something realistic (and a bad one can never take time from a better one in the same group), and breakaways that behave like actual breakaways.

That is the boring technical answer though. Now I want the greedy version 😄

**What is your dream race engine?** And please do not filter it for "what is realistic for one guy". I will do that filtering myself, that is my job. Tell me what you would want if I had ten people.

Some prompts, if they help:

- **What should a race report tell you?** Right now you get a result list. What do you actually want to know about what happened out there, and in what shape? A timeline? A story? A graph of the gaps?
- **What moment do you want to be possible?** The 60km solo that somehow sticks. The teammate who blows himself up covering a move. The sprinter dropped on the last climb who claws his way back. Name the moment you want to happen to YOUR team one day.
- **What annoys you most about race days as they are today?** Be blunt about it. Way more useful to me than praise ✌️
- **How much should you be able to steer, and how much should the race just decide?** I have a fairly strong opinion here, but I want yours before I say mine.

Nothing is too big for this thread. I will tell you honestly which ones are years away.

---

# 3 · Taktikskærmen

> **⚠ Post KUN dette hvis v4 er flippet (mandag aften-gaten er grøn).** Beskriver du taktik-kortet
> før flippet og motoren falder tilbage til v3, lover du noget der ikke er der tirsdag morgen.
> Bemærk også: v1-ordrerne er allerede låst (T1-T4, 21/8). Derfor spørger opslaget om HVAD DER MANGLER,
> ikke om et blankt stykke papir. Ellers samler du ønsker ind som v1 med sikkerhed ikke opfylder.

**Kanal:** #feedback-and-ideas · **Titel:** `The new tactics card: what is missing from version 1?` · **Tone:** stram

With the new engine you get a Tactics card on every stage, sitting under your lineup. Version 1 is deliberately small, and I want to know what you need next.

**What version 1 gives you, per stage:**

- **Breakaway stance for the team:** Chase it down, Neutral, or Let it go.
- **Try the break, per rider:** a flag that raises the chance he goes up the road. It raises it. It never guarantees it.
- **Effort, per rider:** Protect, Normal, or Save.
- Orders lock when the stage starts at 11:00, and the card shows you the lock time.
- Doing nothing is a valid choice. No orders means neutral: your lineup roles, normal effort, neutral on the break. You are never punished for not opening the card.

That is version 1. It is small on purpose, because I would rather ship three orders that genuinely change a race than twelve that quietly do nothing.

**So: what is the next order you want, and what should it actually do?**

Things already on my own list, so you know what you are adding to:

- Leadout trains for a sprint finish
- Chase duties: which of your riders is allowed to burn himself in the chase
- GC protection: keep these riders with my leader at all cost
- Time targets: "do not lose more than a minute today"
- Attack triggers: attack on the final climb, attack if the gap goes under X

Two harder ones I would like your take on:

**Should orders ever be changeable mid race?** Right now they lock at the start. Live orders are a much bigger build.

**How much should a good order be worth?** A tactic that wins you a race you had no business winning is fun once and broken forever. Where is that line for you?

---

# 4 · September-afstemningen

**Kanal:** #feedback-and-ideas · **Titel:** `Season 3 is here: pick the 3 things I should build in September` · **Tone:** løs

Season 3 starts Tuesday 🙂 That gives me roughly a month of building before the season gets deep enough that big changes start being risky, and I would much rather spend that month on what you want than on what I assume you want.

So here is the honest list of what I can realistically pick from in September. Everything on it is real and scoped, and something I could actually start on Monday.

**Reply with your top 3 by number.** One line of "why" is worth more to me than the vote itself.

**Racing**
1. More tactics: leadout trains, chase duties, attack triggers
2. Race reports that explain what happened, not just who won
3. A proper race centre: today's stages, live updates, results that are easy to find

**Your team**
4. Dashboard rebuild: compact, and you decide what is on it and in what order
5. A manager settings area: your country, your team country, your preferences in one place
6. Training: weekly templates and a lot less clicking per rider
7. Deeper scouting: real capacity per scout, better reports, less noise

**Market and money**
8. Rider values that follow the actual market instead of a formula
9. Contract length matters: longer deals cost more per year
10. Loans, buyout clauses and swaps in the transfer system
11. Notifications so you never miss an auction on a rider you want

**The world**
12. U23 teams with their own calendar
13. A press engine: automatic stories about your season and your rivals
14. Rivalries between teams fighting over the same riders and the same places
15. Hall of fame, records and club history that the game actually remembers

I am not going to build all 15 in September, obviously 😄 I will build 3, properly, and I will come back and tell you which ones and why. And if your idea is not on the list, say it anyway. The list is what I have thought of, not what is possible ✌️

---

# 5 · Roadmap, næste 3 måneder

> **⚠ To ejer-valg i dette opslag, markeret i teksten:** (a) om /pro-afsnittet skal med,
> (b) om du vil nævne v4-fallbacken åbent. Slet de blokke du ikke vil have.
> Resten står uden datoer med vilje.

**Kanal:** #the-roadbook · **Første linje:** `Where this game is going for the next three months` · **Tone:** stram

Season 3 starts Tuesday, so this is a good moment to say out loud where I am taking this game, and just as importantly where I am not.

One rule before the list: **I do not promise dates.** I have learned the hard way that a date I miss costs more trust than a feature I ship late gains. The order below is roughly the order I expect to work in, and it can change if something breaks or if you tell me I have it wrong.

**Right now, this week**

Season 3 starts Tuesday with 27 race days. The rewritten race engine goes live with it if it passes my own check on Monday night. **[VALGFRI SÆTNING]** If it does not pass, the current engine keeps running and the new one comes a bit later. I would rather run one more week on the old engine than launch a season on something I am not sure about.

**Next, roughly September**

- Calibrating the new engine on real season 3 races, not on my test data. Results have to look like cycling, not like a spreadsheet.
- Race reports that tell you what actually happened to your riders.
- More tactical orders per stage.
- Rider values driven by what the market actually pays, instead of a formula.
- The dashboard rebuilt: much more compact and customisable, because right now you scroll too far to find anything.

**After that, roughly October**

- The youth side: U23 as a real tier with its own races, fed by your academy.
- Contracts and the transfer market with more depth.
- Results, records and history the game remembers, so a season leaves a trace.

**Later, roughly November and beyond**

- The world around the racing: press, rivalries, fans, reputation. The things that make a season feel like a story instead of a table.

_(Opslaget er over Discords 2.000-tegns-grænse. Post resten som besked 2, direkte under.)_

**BESKED 2:**

**What I will not do**

- I will not punish you for being good. No handicaps for strong teams, no rubber banding. Balance gets fixed by structure, never by slowing the leader down.
- I will not sell power. Anything paid stays optional and never makes a rider faster.
- I will not lock a date and then ship something half finished to hit it.

**[VALGFRI BLOK, slet hvis du ikke vil nævne /pro nu]**
On the money side, since people have asked: there is an optional supporter tier coming back online. It is optional, it does not make your riders faster, and the game stays fully playable without it. I will post properly about it when it is actually open.

That is the map. Tell me where I have the order wrong.

---

# 6 · Dashboard

**Kanal:** #feedback-and-ideas · **Titel:** `The dashboard is too long. Help me rebuild it.` · **Tone:** løs

Let me start by just agreeing with the complaint: the dashboard is too long. You scroll most of the page to find the one thing you came for, and that means I prioritised the content wrong. It is not you scrolling wrong 😄

So I am rebuilding it. But before I do, I would really like to know how you actually use it.

**1. What do you check FIRST when you log in?** Be honest, even if the answer is "I skip the dashboard completely and go straight to Training". That is genuinely useful to know.

**2. What would you delete from it entirely?** No feelings involved, I promise. If a card has never once been useful to you, say so.

**3. What do you open another page for every single day, that should honestly just be on the front page?**

**4. Mobile or desktop?** And if you use both: what breaks when you switch between them?

Where my own head is at, so you have something to push back on:

- Much more compact. Fewer cards, tighter cards, less air between everything.
- You decide what is shown and in what order. Drag it, hide it, keep it.
- Things that belong together should sit together, instead of the same topic showing up in three different spots down the page.

And one last one, which is the one I most want answered: **if the dashboard could only show you FIVE things, which five?** That answer is worth more to me than a long list of nice-to-haves ✌️

---

# 7 · U23 og junior

**Kanal:** #feedback-and-ideas · **Titel:** `U23 and junior teams: what should they actually be?` · **Tone:** løs

Real cycling has senior teams, U23 teams and junior teams, and I want that in this game. Your academy produces riders, they race at their own level, and the good ones come up to your senior squad. That direction is decided. Basically everything about HOW it works is not, and that is where I need you 🙂

The picture in my head: three tiers per club, each with its own calendar, and riders moving up through them across a career instead of just appearing fully formed in your senior squad one day.

The open questions, and I genuinely have not settled on any of them:

**1. Should U23 races pay prize money, or only develop riders?** Prize money makes them matter. It also turns youth into yet another income optimisation, which might ruin the whole point of it.

**2. How big should a U23 squad be?** Small enough that selection is a real choice, or big enough that everybody gets a race?

**3. Should you be able to promote a rider mid season?** Or is a promotion window part of the drama?

**4. Should there be a U23 transfer market?** Can I sign someone else's talent, or is your academy yours alone?

**5. Junior level: real racing, or purely development?** I lean towards junior being about development and identity rather than results. Convince me otherwise ✌️

**6. How much attention should it cost you per week?** This is my biggest worry by far. Three tiers can easily turn into three times the clicking, and that kills it for anyone playing this on a phone during a lunch break.

**7. What would make you actually care about a 19 year old in your academy?** Naming him? Watching him win something? A story about him? A scout who turned out to be right about him two seasons ago?

That last one matters most to me. A youth system that is only numbers is a spreadsheet. I want it to be the part of the game you get properly attached to.

---

# Korte DA-versioner til #dansk-snak

> Poster du på engelsk i forummet, så smid disse i #dansk-snak, så danskerne ved at tråden findes.
> De peger ind i den engelske tråd, så al feedback samles ét sted.

**Til opslag 4 (September-afstemningen):**
> Jeg har lagt en liste med 15 ting op i #feedback-and-ideas, som jeg kan bygge i september. Vælg dine 3 (skriv bare numrene). Én linje om hvorfor er mere værd end selve stemmen. Det er ikke en ønskeliste jeg kigger på engang, jeg bygger 3 af dem 🙂

**Til opslag 6 (Dashboard):**
> Dashboardet er for langt, og det er min skyld, ikke jeres. Jeg bygger det om. Der ligger en tråd i #feedback-and-ideas med fire spørgsmål. Det vigtigste: hvis dashboardet kun måtte vise FEM ting, hvilke fem så?

**Til opslag 2 (Race engine):**
> Ny tråd i #feedback-and-ideas om løbsmotoren. Ikke "hvad er realistisk", men hvad I ville ønske jer hvis jeg havde ti mand. Fortæl mig hvilket øjeblik I gerne vil se ske for jeres eget hold ✌️

**Til opslag 7 (U23 og junior):**
> Tråd om U23- og juniorhold i #feedback-and-ideas. Retningen er besluttet (tre niveauer pr. klub), men næsten alt andet er åbent: præmiepenge på U23-løb eller ej, oprykning midt i sæsonen, eget transfermarked. Sig din mening.

---

# Forslag: parallelle tråde du kan køre oveni

> Mine egne forslag, ikke bestilt. Rangeret efter hvad der giver dig mest.

**A · "What nearly made you quit?"** (#feedback-and-ideas, efter S3-start)
Den mest værdifulde tråd du kan åbne lige nu. 232 konti, 67 her: forskellen er folk der stoppede. Spørg dem der BLEV, hvad der var tættest på at få dem til at lukke fanen. Svarene peger direkte på fastholdelse, som er dit største håndtag før du overhovedet markedsfører.

**B · "Your first week: what confused you?"** (#feedback-and-ideas)
Onboarding-gæld gjort synlig. Kobler direkte til om det giver mening at sende nye folk ind ad døren. Kør den FØR marketing-tråden bærer frugt.

**C · "Season 2 in one screenshot"** (#team-showcase, søndag efter finalen)
Fejrer sæsonafslutningen, koster dig intet, og du får en stak ægte screenshots du kan bruge til præcis det marketing-materiale du mangler. To fluer.

**D · "The money: what feels wrong?"** (#feedback-and-ideas, ca. 10 dage inde i S3)
Økonomien er halvt genkalibreret, og kalibrerings-sessionen ligger efter cutover. En struktureret tråd giver dig spillernes oplevelse SAMTIDIG med at du har rigtige S3-tal at holde den op imod.

**E · "What should the Help section explain that it does not?"** (#questions-and-answers)
Billig at køre, og hvert svar er en konkret rettelse i help.json. Fanger også de misforståelser der ellers lander som bug-rapporter.

**F · "Predict season 3"** (#general, mandag 24/8)
Ren stemning før første løbsdag. Hvem vinder D1, hvem rykker op, hvilken rytter overrasker. Koster nul udvikling, giver liv i serveren, og du har en sjov tråd at grave op igen når sæsonen slutter.
