...
5. REHEARSAL GRØN på post-#1701-kode: backend/scripts/dev/run-relaunch-rehearsal.mjs (nu MED AI-fyld-acceptance-tjek, #1688 trin 5.6) kørt end-to-end mod en frisk disposabel prod-klon → ALLE PASS. AI-fyld blev wired ind som apply-trin 5.6 EFTER sidste grønne rehearsal; uden denne kørsel er reset-kæden end-to-end uverificeret (dry-run i step 3 springer 5.6 over).
...
4. DESTRUKTIV RESET: infisical run --env=prod -- (RELAUNCH_1101_CUTOVER_ACK=true sat) node backend/scripts/relaunchSeason1.js --apply --target-prod --confirm "RELAUNCH SEASON 1". (PowerShell: $env:RELAUNCH_1101_CUTOVER_ACK="true"; infisical run --env=prod -- node backend/scripts/relaunchSeason1.js --apply --target-prod --confirm "RELAUNCH SEASON 1"). Reset-kæde: retire legacy → full beta-reset → fiktiv population → backfill → startholds → allocateLeaguePools (pulje-spredning, ægte hold i div 3 = MANAGER_ENTRY_DIVISION; div 4 tom headroom, ejer-besluttet 22/6) → clearAllAiTeams (ryd pre-eksisterende phantom AI-hold) → AI-fyld (trin 5.6) → sæson 0+transition 0→1 → board-oplåsning (pending_5yr) → akademi → kontrakter → founder-badges.
5. AI-FYLD (VERIFICÉR — kører ALLEREDE i step 4): AI-fyld er apply-trin 5.6 i relaunchOrchestrator (#1701), så div 1+2 AI-fyldt + div 3+4 AI i pulje-huller (≥1 manager) er allerede udført af step 4's --apply. Det standalone backend/scripts/generateAiTeams.js hard-denier prod — kør det IKKE mod prod. Verificér blot AI-feltet i step 8.
...
8. POST-VERIFY mod prod (alle grønne): pyramide (league_divisions=15; ægte hold league_division_id ikke null + division=3; div 1+2+3 har AI-fyld, **div 4 tom** = 0 race-hold) · 0 phantom AI-hold (alle AI-hold har 8 ryttere + pulje) · 0 aktive ryttere uden rider_derived_abilities/base_value · ægte hold balance=800000 (ingen dobbelt sponsor/upkeep) · sæson-1-vindue board_negotiation_state='pending_5yr' · nyt signup → div-3-pulje + 8-rytter-trup · founder-badges · egne løbsnavne i kalenderen (ikke Czech Tour/Giro). Brug rehearsal-harnessens acceptance-checks som skabelon (25/25 PASS verificeret mod disposabel prod-skema-klon 22/6).
