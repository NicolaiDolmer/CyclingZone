...
> **📊 Status-snapshot (2026-06-17, audit-verificeret):** Race Engine v2 **LIVE** (PR #1428, bag `RACE_ENGINE_V2_ENABLED`) · abilities v2 + egen værdimodel **LIVE + prod-backfilled** (PR #1434/#1435, 8994 ryttere, R² 0.959) · E1 værdi-gate merged (#1397). **Eneste ægte launch-blocker: E2-økonomi (sponsor+løn) er IKKE i kode** — sponsor stadig 240k flat, løn 0.10; gældsloft {1.2M/900k/600k} ER på plads. Fuld motor-modenhed: backend-motorer (økonomi, auktion, sæson-pipeline, stats) er production-grade; race-motorens *dybde* (form/træthed = neutrale 0-stubs) er det største world-class-gap.
...
1. **E2-økonomi (`strict_fair_v1`) — HÅRD GATE før relaunch-seed.** Verificeret 2026-06-17: sponsor stadig 240k flat (`SPONSOR_INCOME_BASE`), løn 0.10 (DB GENERATED-kolonne); gældsloft {1.2M/900k/600k} ER allerede på plads. **Mangler:** division-skaleret sponsor (600/400/260k) + løn ×0.67. Kræver migration (database/*.sql → **ejer-merger**, auto-applies i prod). Re-kør `simulateSeasonDryRun.js`-scorecard FØR seed (kompetent D1-hold taber ~750k/sæson nu).
2. **Season-transition + relaunch-orchestrator: server-side gate + dry-run-preview + undo-scaffolding.** Ingen server-gate på transition pt. + one-shot orchestrator mod prod = ét fejlklik kan korrumpere en live sæson.
