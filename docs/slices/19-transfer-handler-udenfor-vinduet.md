...
**Status:** 🟢 Del A + Del B leveret + live 2026-06-01. Del A (offers + swaps + listing — "betal nu, registrér ved åbning"; ingen migration). **Del B merged via PR #891** (squash `a0e3300f`) — loans + buyout bruger `window_pending` på `loan_agreements`; migration `database/2026-06-01-loan-agreements-window-pending.sql` **kørt mod prod 2026-06-01** (CHECK-constraint udvidet med `window_pending`, verificeret). Alle 4 faser leveret. Udestående: ejer-verifikation af loan-accept/buyout end-to-end i lukket prod-vindue før #19 lukkes.
