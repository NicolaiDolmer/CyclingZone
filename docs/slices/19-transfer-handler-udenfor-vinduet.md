...
**Status:** 🟢 Del A leveret 2026-05-31 (offers + swaps + listing — "betal nu, registrér ved åbning"; ingen migration). **Del B implementeret 2026-06-01 i `fix/19-loans-outside-window`** — loans + buyout bruger `window_pending` på `loan_agreements` og migration `database/2026-06-01-loan-agreements-window-pending.sql`. Afventer review/merge/deploy.
