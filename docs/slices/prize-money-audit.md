...
- **[#895](https://github.com/NicolaiDolmer/CyclingZone/issues/895)** — R3: værdi opdateres ved udbetaling. **✅ Design låst + implementeret (i PR).** Design: [`prize-money-audit-r3-design.md`](prize-money-audit-r3-design.md). Progress-vægtet snit (Option A): `round(Σ earnings_s / max(Σ w_s, 1))`, aktiv sæson vejer `race_days_completed/race_days_total`, gulv mod annualisering i anker-løs sæson 1, bagudkompatibelt (ingen aktiv sæson → gammel logik). Trigges nu fra `paySeasonPrizesToDate` + `processDivisionEnd`.
- **[#896](https://github.com/NicolaiDolmer/CyclingZone/issues/896)** — forbedret post-import preview. **Scope låst (ejer 2026-06-02): Kerne** = #1 optjent-vs-udbetalbar (fri/AI-split synlig) + #4 reconciliation-kolonne (Σudbetalbare-results vs Σfinance pr. betalt løb, ⚠️ ved mismatch) + #3 sanity-warnings (`all_free_ai` = løb hvor hele puljen er fri/AI, før droppet stille; `no_prize_results`). **Implementeret (i PR):** `getSeasonPrizePreview` udvidet med `totals`/`reconciliation`/`warnings` (ren additiv beregning, ingen nye queries; data hentes allerede), `AdminEconomyTab.jsx` renderer dem; 4 nye backend-tests grønne. **Prod-grundet (read-only):** sæson 1 = optjent 31,3M / udbetalbar 23,1M / fri-AI 8,18M (26%). #2 forventet-vs-faktisk + #5/#6 udskudt (ikke i Kerne-scope).
