...
**Status:** v0.4 — **alle fire beslutninger i §11.5 er nu låst (A+B 9/8, C+D 11/8)**. Trin 2 er dermed afblokeret på designsiden; D's fulde design står i §11.6. Trin 1-migrationen venter fortsat på PR-kæden (PR-0 → PR-1 → PR-2), som er sekvenseret til EFTER 23/8-cutoveret (se §12). v0.3-grundlaget: harness-runden 9/8 aften (10-agent workflow + egne verifikationer; resultater i §11), alle 15 porte implementeret + negativ-testet (PR [#3567](https://github.com/NicolaiDolmer/CyclingZone/pull/3567) merged), alle 5 ejer-beslutninger fra §8 står.
...
- [x] **A. Remap-afgrænsning (ejer 9/8 aften): variant B — hale-korrigeret remap + fredningsgulv.** Tiers 1,0-4,0 form-bevares (intra-tier-spredning); kun ≥4,5-klassen (961) presses mod planens hale-ankre (≥74,5/≥84,3/≥94,1 = 0,70/0,32/0,11 % pr. aldersbånd). **Ejer-justeringer efter første diff (begge låst 9/8 aften): gammel 6,0'er aldrig under 80 · gammel 5,5'er aldrig under 74,5** (fredningsgulve; accepteret hale-overskridelse ≥74,5: 248 = præcis de gamle 5,5+6,0'ere mod plan 57 — eroderer ved aldring, flow følger geometrien). Endelig dry-run: 470 nedjusterede (kun pot 5,0-6,0), **−6,7 % værdi** (mod bogstavelig variants 7.243/−27 %), 0 rang-brud, ≥84,3/≥94,1/stock-gate på plan, kontinuert gradient uden huller (4,5+5,0 → 65-74,5 · 5,5 → 74,5-80 · 6,0 → 80-99). Rapport: [trin1-remap-dryrun](2026-08-09-3564-trin1-remap-dryrun.md). Migrations-målet for PR-2 = variant B m. begge gulve; T1-N3 omdefineres til hale-ankre + form-bevarelse + frednings-undtagelse.
- [x] **B. §2a vs §5-konflikten (ejer 9/8 aften, co-design-session):** LØST som a+b — (1) **graduerings-niveau for bedste unge: ~15-18** (§2a's 20-21-anker 12/12 GENÅBNET og erstattet; moderat båndudvidelse jf. sim-evidens statCeil ~58 + rekalibreret base/slope/luck — endelige konstanter fastlægges i trin 3 EFTER kurven); (2) **talent-synlighed: gradvist fra 17-18** (skelettets form bevaret; 16-års-søjlen forbliver flad — skjult potentiale-princippet står). §2a omfortolkes til per-potentiale-mål. Trin 2-kalibrering er hermed afblokeret.
- [x] **C. Træningsscorens privatliv (ejer 11/8): variant a — dagsstøjen hæves mod en MÅLSAT gate.** "Talent kræver uger" var modbevist (potMult-spredning ±38 % ≫ dagsstøj ±15 % ⇒ aflæsning på 1-3 dage). Støjniveauet sættes IKKE i hånden: gaten er **median antal dage før potentialet kan aflæses pålideligt ≥ 14**, og harnessen genkøres for at finde det støjniveau der rammer den — FØR konstanterne låses. Følger designprincip 1 (forholds-gate kræver niveau-gate). Beslutning 4 og 5 står uændret; scouting (#1138) beholder sin rolle. Accepteret pris: den daglige score svinger mere, så ugetendensen — ikke enkeltdagen — er det spilleren læser.
- [x] **D. Specialiserings-gab (ejer 11/8, co-design-session): løst som medfødt hældning + fokus, i ligevægt.** Se §11.6 for hele designet. Kort: (1) to ryttere af samme type SKAL kunne blive forskellige; (2) forskellen kommer fra en medfødt hældning pr. evne som spillerens fokus kan forstærke eller udligne; (3) ligevægt — arven peger, fokus afgør (vedholdende fokus over flere sæsoner kan flytte tyngdepunktet); (4) median-gab **~12** ved 28 år (86 mod 74), harness kører 6/12/20 mod ægte population + race-motor før konstanterne låses; (5) scouting afslører **retningen**, aldrig niveauet. Variant b (redefinér målet) blev afvist: den ville bestå porten uden at ændre hvad spilleren oplever — samme fejlklasse som postmortem 11/8.
- [x] **E. Toprytterens form (ejer 11/8 aften): mesterlig i primæren, god i sekundæren, jævn i resten.** Stillet fordi bestanden i dag lader en toprytter blive stærk i mange discipliner samtidig ("det skal være mere gennemskueligt for managers"). Valgt som blanding af specialist og to-toppet: en 6-potentiale-klatrer skal være blandt spillets bedste klatrere, god i sin sekundære type, og kun jævn udenfor — og **stadig opleves stærk**, fordi han vinder de løb han er skabt til. Konkretiserer beslutning 3's otte type-loftprofiler: profilen skal have to tydelige toppe og en lav bund, ikke fire evner på faktor 1,0. Bindende sidebetingelse (ejer): *"hvis de har haft et indtryk af, at de havde en 6-stjernet rytter, så skal han jo stadig opleves god"* — samme princip som fredningsgulvene i beslutning A, nu udvidet fra remappen til loftprofilerne. Kalibrering hører til trin 1 (profilerne) og verificeres af D's gab-harness (6/12/20 mod ægte population + race-motor).

### 11.6 Beslutning D udfoldet — rytter-identitet inden for en type (ejer 11/8)

**Problemet, målt:** 7 af 8 ryttertyper har 2+ evner med rollefaktor 1,0. Med én score på rytter-niveau får de evner identiske baner for evigt ⇒ gab strukturelt 0,0. Kan ikke tunes væk i den låste modelfamilie. Målt i prod: gab 1,25 (16 år) → 0,11 (21 år), 70 % på nul.

**Designet (fem ejer-beslutninger, stillet ét ad gangen):**

1. **To ryttere af samme type skal kunne blive forskellige.** Én sprinter ender med topfart som klart bedste evne, en anden med acceleration. Begrundelse: uden det er en trup en liste sorteret efter tal, og to sprintere med samme potentiale er samme rytter. Konsekvens: gab-målet bliver i modellen — det redefineres ikke væk.
2. **Kilden = medfødt hældning + spillerens fokus.** Hældningen er en vægt pr. evne der ganges ind i loftet og ligger fast hele livet; fokus (findes allerede: 1,15/1,35/1,60 på daglig gevinst) forstærker eller udligner den. To greb, to steder i kæden. Ren tilfældighed undervejs blev afvist: rytteren du købte må ikke skifte identitet ved held EFTER du har betalt.
3. **Ligevægt mellem arv og arbejde.** Rytteren fødes med en tydelig tendens, men vedholdende fokus over flere sæsoner kan flytte tyngdepunktet til en anden af hans signatur-evner. **Kalibrerings-krav:** antal sæsoner et skifte tager SKAL måles og sættes bevidst — ellers bliver et skifte enten gratis eller umuligt.
4. **Median-gab ~12 ved 28 år** (bedste 86 mod næstbedste 74 for pot-6). 6 blev fravalgt fordi 6 point blandt 15 tal er under hvad en spiller opdager uden at regne — ægte i modellen, usynlig på skærmen. 20 blev fravalgt fordi ryttere bliver skrøbelige og små trupper straffes, hvilket kolliderer med "straf aldrig styrke" ad bagvejen. **Verifikations-krav (ejer 11/8): harnessen kører 6/12/20 mod ægte population + race-motoren, og tallet står på målte resultater før konstanterne låses** — en spids på 12 skal give udslag i race-resultater, ellers er den kosmetik. Tallet er en MEDIAN: de otte type-loftprofiler (beslutning 3) må variere spidsen omkring den, så gc er fladere end sprinter.
5. **Scouting afslører retningen, aldrig niveauet.** En scoutet 17-årig viser "hælder mod topfart" uden at røbe hvor højt han når. Bemærk tvangen: en 16-årig har alle evner mellem 1 og 12, så et gab på 12 kan ikke eksistere endnu — spidsen SKAL vokse frem. Men retningen findes fra fødslen og lækker intet. Det giver spejderen en rolle der ikke er et potentiale-spænd — altså præcis det #2798 skal lukke — og gør ungdomshandel meningsfuld før graduering (#3616's anden halvdel).

**Kobling til resten af kæden:** punkt 2+3 hører til trin 2 (kurven); punkt 4's mål-tal er en trin 2-port; punkt 1+4's profil-variation leveres af beslutning 3's type-loftprofiler i trin 1; punkt 5 er en scout-UI-slice der afhænger af C.
...
## 12. Kædens tilstand efter 10.-11/8, og sekvensen frem mod 23/8 (v0.4, 11/8)

Fire ændringer landede 10.-11/8 og flytter hvor kæden står. De rører ALLE led 2 (loftet) og ingen af dem led 6-7.

| Ændring | Hvad den gjorde | Hvad den IKKE gjorde |
|---|---|---|
| **#3570** (lukket 11/8) | 7.998 ryttere repareret; `archetype_draw` forankrer primæren, så klassifikatoren ikke længere kan flytte den natligt. Post-verify 0 afvigelser. | Rørte ikke sekundæren, ikke værdier, ikke løn. |
| **#3632** (PR #3635, v7.114) | Alle NYE ryttere fødes to-delt; `isHybrid` slettet; vagt på skrivestien; bi-typens vægt i fødsels-stats 0,50→0,10 (målt; 0,50 bryder G2). Loft-faktoren 0,82 urørt. | Rørte ikke bestanden. Prod-verifikation udestår til søndag 16/8 (akademi-intake er søndags-gated). |
| **#3618** (v7.115) | Akademi-udløbskvote 30→45/dag (60 ved efterslæb). | Ændrer flow-volumen, ikke kurven. Gør flow-gates statistisk stærkere. |
| **#3598** (#3591, merged 10/8) | Ensrettede kaldformen: `backfillCores.js` sender nu `age` til `buildCapsForRider`, som motoren altid har gjort. Regressionstest med negativ-bevis. | **Muterede ingen data** (PR'ens egne ord). AI-rytternes GEMTE caps stammer stadig fra den gamle kaldform. |

**Konsekvens for led 2 (loftet):** loftet formes af potentiale × rollefaktor — primær 1,0, **sekundær 0,82**. Primæren er nu forankret overalt. Sekundæren er forankret på akademi-stien siden 11/8, men **ikke** for de ryttere der har tomt sekundært anlæg (målt i prod 11/8: **577** levende, ned fra 583 — erosion via udløb/pension), og **ikke** for voksen-generatorens output (`fictionalRiderGenerator` skriver stadig `secondary: null` ⇒ AI-hold + startholds-trupper). For dem udpeger klassifikatoren sekundæren hver nat, og 0,82-faktoren sidder dermed på en type der kan skifte. Målt i #3593: 25,8 % af bestanden ville få en anden sekundær afhængigt af hvornår klassifikatoren kører, caps-forskel median 17 / max 52.

**Kædens bindende egenskab (uændret):** identitet (primær + sekundær) → loft → klassifikation → værdi → løn. Alt der rører identitet flytter værdi. Derfor skal identiteten være færdig FØR markedsmodellen refitter mod bestanden.

**#3591's reelle reststatus:** punkt 1 (kaldformen) og punkt 3 (forward-guard) er leveret i PR #3598. Punkt 2 er åbent og ER 23/8-deadlinen: korrektionen skal tages **kontrolleret** (dateret snapshot → dry-run-diff → ejer-go), ikke som sideeffekt af at `race_day_engine_enabled` tændes og AI-holdene tickes for første gang. Målt på 10/8-snapshottet: 2.139 af 3.473 AI-ryttere (61,6 %) skifter type ved første tick, 38,5 % taber loft (p10 Δ rating −23). Bemærk: `predictBaseValue` læser abilities + `valuation_type`, ikke caps — men klassifikationen læser caps, så type-skiftet kan flytte værdi for de ryttere hvis `valuation_type` ikke er frosset. Skal måles i dry-runnen.

### Sekvensen

| Blok | Hvornår | Indhold | Hvorfor dér |
|---|---|---|---|
| **1. Identiteten gøres færdig** | Nu → 16/8 | #3593 (udfyld anlægget for de 577 fra `secondary_type`-kolonnen — ren forankring, intet synligt skifter) → #3591 pkt. 2 (kontrolleret re-derive af AI-populationen, dry-run + ejer-go) | Begge ændrer hvilken identitet bestanden HAR. Sker de efter sweepen, refitter markedsmodellen mod en bestand der straks flytter sig. |
| **2. Markedssweep** | Søndag 16/8 | #3449-drejebogen uændret + #3632-prod-verifikation (query i issue-tråden) | Ejer-fastlagt. Forudsætter blok 1. |
| **3. Kun FLOW** | 16/8 → 23/8 | #3631 (sekundær-fordelingen udjævnes i ALLE fødselskilder; simulér-før-ship mod ægte population) → #3634 (voksen-generatoren former kroppen efter to anlæg; eget scorecard mod `balanceSnapshot` + `simulateSeasonDryRun` + `raceCompetitionScorecard`) | Rører kun nyfødte, ikke bestanden. Kan derfor ikke forstyrre den netop kørte sweep. |
| **4. Cutover** | 23/8 | markedsvægt→1,0 → #3393 løn → race-day-flip #3459 → mandat-migration #3514 | Ejer-fastlagt. Kræver rollback-plan pr. komponent (§7, stadig uskrevet). |
| **5. #3564-kæden** | Efter 23/8 | PR-0 (#2798-A + `base_value_preview`-lækagen) → PR-1 (1-99-fundament) → PR-2 (remap variant B, −6,7 %) → trin 2 kurve (C+D er nu låst) → trin 3 startniveau (**#3616 falder ind her** — begge fund, bund og fladhed) → trin 4 værdi+løn | Remappen barberer 6,7 % af værdien og trin 2 ændrer hele væksten. Oven i markedsvægt-flippet og lønomlægningen samme uge kan effekterne ikke skelnes — og så kan intet rulles tilbage med viden om hvad der virkede. |

**#3616's placering er ikke en udskydelse:** issuet anbefaler selv at ligge under #3564 frem for at tune `baseStatAt16` isoleret, og beslutning B låste at graduerings-niveauet flyttes til 15-18 med konstanterne fastlagt i trin 3 EFTER kurven. Fund 2 (alle 21-årige på præcis evne 12) er samme fænomen som D's strukturelle nul-gab set fra en anden vinkel — begge løses af den bredere skala plus type-loftprofilerne, ikke af en base-justering.
