...
**Fase-tracker:** Fase A ✅ · Fase B trin 4 ✅ (hybrid-fatigue, PR #1443 merged) · Fase B trin 5 ✅ **P3 OPFYLDT 18/6** — `SUPABASE_DB_URL` wired i prod-Infisical; verificeret prod-backup off-site (OneDrive, 68 tabeller / 225.046 rækker, sha256, 162 kolonne-GRANTs), restore-bevist tolerance 0 + FK-integritet grøn. **Fase C trin 8 (P4 rehearsal) ✅ KØRT GRØN 18/6** — 12/12 PASS mod disposabel preview-branch; fandt+fixede 2 latente bugs (PR #1463 merged + prod-applied: `'upkeep'` finance-type + `academy_intake` FK i betaReset). **P5 #1101-cutover ✅ KVITTERET 18/6** (ejer godkendte værdimodel-scorecard). **Alle hårde pre-reqs P1-P5 ✅. ⏭ NÆSTE = prod-apply (trin 10) — ejer-kørt destruktivt vindue, ingen samtidige sessioner.** Fase C resterende (prod-relaunch → flag-flip) ⬜ · Fase D (post-launch: #1021, PCM-oprydning, #1310) ⬜. Følg §5 i rækkefølge.
...
| P3 | Verificerbar DB-backup/PITR før prod-apply | ✅ **Opfyldt 18/6** — `SUPABASE_DB_URL` (session-pooler) wired i prod-Infisical; verificeret prod-backup off-site på OneDrive (`cz-db-backups/cyclingzone-20260618-090313`, 68 tabeller / 225.046 rækker, sha256 `c02aba57…`, 162 kolonne-GRANTs), `db:verify-restore` grøn tolerance 0 + FK-integritet 0 violations | Genoptag før hver fremtidig destruktiv prod-op: `npm run db:backup` + `db:verify-restore` (BACKUP_DIR→OneDrive) |
| P4 | Orchestrator grøn ejer-verify mod preview-branch | ✅ **KØRT GRØN 18/6** — `run-relaunch-rehearsal.mjs` 12/12 PASS mod disposabel preview-branch; 2 latente bugs fundet+fixet (PR #1463 merged + prod-applied) | — |
| P5 | #1101 base_value-shadow-cutover kvitteret | ✅ **KVITTERET 18/6** — ejer godkendte værdimodel-scorecard (`docs/audits/2026-06-18-valuation-scorecard.md`); værdier glider selv mod data (#1101-markeds-glidning). Sæt `RELAUNCH_1101_CUTOVER_ACK=true` ved apply | Hard-gate (`RELAUNCH_1101_CUTOVER_ACK=true`) |
...
9. **Academy i seed** (beslutning ③ ✅ **AFGJORT 17/6 = kuld dag 1**): orchestrator trin 6.4 læser `isAcademyEnabled` UDEN beta-opts → `"beta"` = false → academy-intake springes over. **Sæt `academy_enabled='on'` FØR `--apply`** så hvert menneske-hold får et kuld (3-5 offered, nul tvungen cost). Rehearsal (trin 8) er nu academy-aware og verificerer kuld + idempotens.
...
- **② Backup-strategi** (P3) — ✅ **AFGJORT: verificeret logisk pg_dump-backup** (`scripts/db-backup.mjs` + `db-verify-restore.mjs`, commit `893fed13`, lokalt-grøn). En frisk restore-testet dump slår både den 24t-gamle daglige backup og en ubetestet PITR; PITR valgfri oveni. Reverse-DELETE-undo forkastet (dækker ikke sæson/race/finance). MCP-stramning (read-only/scope) bevidst FORKASTET — ejer vil maks autonomi; backuppen er det der gør bred adgang sikker. ✅ **Lukket 18/6:** `SUPABASE_DB_URL` lagt i prod-Infisical, verificeret prod-backup taget off-site + restore-bevist (tolerance 0).
- **③ Academy i seed-kørslen** (Fase C trin 9) — ✅ **AFGJORT 17/6: kuld dag 1.** Hvert menneske-hold får et kandidat-kuld på relaunch-dag 1 → sæt `academy_enabled='on'` FØR `--apply`. Nul tvungen cost (kandidater er "offered", ingen team_id/debit); 3-5/hold (⌀4, ~91 for 23 hold), økonomi-valideret (sim PASS, 29/29 unit-tests grøn). Rehearsal gjort academy-aware (`run-relaunch-rehearsal.mjs`: flag sættes on FØR apply + markeds-tælling "~800" ekskluderer kuld-ryttere + 2 nye acceptance-tjek: kuld 3-5/hold + intake-idempotens). PR `feat/rehearsal-academy-awareness-1308`.
