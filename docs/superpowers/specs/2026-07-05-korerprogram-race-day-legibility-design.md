...
- Friisisch-hændelsen (#2195): Classique du Léon = `game_day` 10 (04-07 13:00Z), Gran Premio de Navarra = `game_day` 11 (04-07 16:00Z). Distinkte spil-dage, samme IRL-dato → genbrug korrekt. **Ingen falsk-positiv overlap-bug** i det scenarie; bindingen nøgler rigtigt på `game_day` (jf. kalender-rebuild 2026-06-27).

## Allerede bygget (skal IKKE genopbygges)

Fra #1823/#1925 + #1984's første runde:
- `raceBinding.js` nøgler på `game_day`; afmeldte løb binder ikke; `windowsOverlap` er dag-granulær.
- Puljens chips: låst kun hvis game-dag-bundet i *alle* dagens løb; låste chips er klikbare; inline lås-grund navngiver det bindende løb.
- `AddRiderPopover` grupperer allerede "Ledig til" vs "Optaget i overlappende løb" med navngivet grund.
- Kladde-model: fjern en rytter → straks fri i kladden; `saveAll` 2-faset release/bind håndterer flytning mellem overlappende løb; navngiven overlap-gemfejl.

Det uløste — og kernen i begge issues — er at **in-game løbsdagen er usynlig**.
...
`backend/routes/api.js` — distribution-endpointet (`GET /api/races/distribution`, kolonne-byggeren ~L1757): tilføj pr. kolonne to felter afledt **direkte** af `schedByRace`-rækkernes `game_day` (ikke fra `bindingWindow`, hvis nøgle-rum kan være CET-ordinaler for legacy-løb):
...
Samme felter på løbs-detalje-endpointet der fodrer løbskortet (så "Race day N" også står på selve løbet). Ingen migration — `game_day` findes allerede på `race_stage_schedule`.
