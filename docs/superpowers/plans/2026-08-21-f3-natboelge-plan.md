...
| 9 | Orders-API | Backend: `race_team_orders`-tabel (migration, IKKE applied — post-merge #2642), CRUD-endpoint pr. (team, race, stage), lås ved etapestart (T2), neutral-default (T4), adapter → StageInput.orders. SQL reviewes FØR merge. **EJER-BESLUTNING 21/8: `race_team_orders` bliver ENESTE sandhed for rolle + effort pr. etape.** `race_stage_roles` (6.959 rækker, 54 hold, 189 løb) migreres ind i den nye tabel og udfases efter v4-flippet — ingen dobbeltskrivning, ingen permanent parallel-model. Worker 9 leverer derfor OGSÅ: (a) en idempotent migrations-SQL der kopierer `race_stage_roles(race_id, stage_number, rider_id, race_role, effort)` → `race_team_orders` med `team_id` slået op via `race_entries`, (b) et post-verify-tjek på rækketal, (c) en note i PR-body om at v3-læsestien (`raceStageRoles.js:25`) skal pege på den nye tabel FØR `race_stage_roles` droppes. Selve DROP'et er ejer-gated og sker tidligst efter flippet. |
