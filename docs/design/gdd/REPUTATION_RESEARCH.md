...
| Rytter-omdømme design låst afsnit for afsnit: hændelsesbog, 0-100 = gulv (loft 60, falder aldrig) + form (halveres pr. sæsonskifte), point = løbsklasse × resultattype, ordbånd Ukendt/Kendt/Profil/Stjerne/Legende, flag off→shadow→on. Klub som hybrid (afløst af D-045, 10/9): §6-formlen består som underliggende tal (sponsor, lånekapacitet, AI), men er ikke længere det spilleren ser — spilleren og andre klubber ser offentlige "kendt for"-mærker udledt af handlingsmønstre. Sponsor uændret i S3. Manager: kun karrierehistorik, ingen effekt | 4/9 2026 (aften), klub-delen afløst 10/9 (D-045) | `docs/superpowers/specs/2026-09-04-reputation-system-design.md` §2, `docs/design/gdd/DECISIONS.md` D-045 |
