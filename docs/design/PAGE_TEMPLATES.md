...
- **Sticky first column** (entity name): `position: sticky; left: 0`, opaque cell background, 1px `--border` right rule. Cell = name (13.5/500, jersey/classification dot) + data-font `text-3xs` uppercase subline. **Three layers, never two** (#5060): body cells `z-table-col` (1) < header row `z-table-head` (2) < the pinned column's own header cell, the corner, `z-table-corner` (3). The corner is pinned on both axes, so it has to out-rank the header row too. Sharing one layer with the rest of the header row let DOM order decide during horizontal scroll, and "Salary"/"Wins" painted straight over "Rider" — the name header disappeared exactly when the player scrolled out to read the numbers. Never put two z-index utilities on one cell (`thClass` already emits one; hand-rolled pins pass `thClass({ pinned: true })` and supply only the offset), and never use the page-chrome `z-sticky` on a table cell.
...
- **Mobile ≤640px:** name column pinned (min ~148px), secondary text columns (age, type, contract) fold into the name cell's subline; numeric columns scroll horizontally under the pinned column. Filter bar collapses to search + two half-width selects. **This is the DEFAULT, not an opt-in per page** (TASTE fork 6, #4625): `DataTable`'s horizontal scroller is unconditional; mark the entity column `sticky: true` to pin it — `DataTable` logs a dev-only warning when a table has rows but no `sticky` column, since that silently drops the pinned-name pattern on mobile.
