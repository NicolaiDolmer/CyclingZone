...
- **Section card (the ONE recipe):** `Card` = `--bg-card`, 1px `--border`, **5px radius**, **padding 20px** (16px mobile), no shadow. **No left accent bar** — that is a fifth priority signal on top of the four gold is rationed for (§ below). `Section` throws in dev if `className` carries a `border-l-*` utility (#4625); it has no `accent`/`leftBar` prop and never will.
...
- **Table-only controls live in `DataTable`'s `toolbar` slot** (#4628): a view switcher, a group filter or a column-mode segment that steers *this table and nothing else* renders inside the table's hairline frame, above the header row, separated by the same 1px rule — never as a free-floating row between the page header and the table ("no orphan action rows" above; audit 2026-09 found exactly that on Mit hold). Page-level controls still belong in the `PageHeader` action cluster, and filters still belong in `FilterBar`.
- **Segmented controls use the `Segmented` primitive** (`components/ui/Segmented.jsx`, #4628): hairline group + 5px radius, 12px/500 labels, active segment = gold text on a 10% gold face (TASTE fork 3's third gold slot), and the active segment carries neither click handler nor pointer cursor (#3188). Three hand-rolled copies with three paddings preceded it. Never hand-roll a fourth.
...
- **Mobile ≤640px — D-047 (owner call 2026-09-10, #5102):** the table shows the **name column + exactly three data columns, with no horizontal scroll**. The three are numeric by default; a page may spend one of them on the row's action column (Academy, Staff and Watchlist do — the page's primary action must not sit behind "Full table" on a phone), and the player may swap one in. **My Team / Squad runs three numbers: rating (OVR), value and salary** (owner call 2026-09-10) — its "Sell / Auction" is one chip tap away and also lives on the rider profile the row links to. **No horizontal scroll is enforced, not hoped for:** the default state renders in an `overflow-x: hidden` container and the name cell wraps instead of standing on one nowrap line, so a long rider name or an 8-digit number becomes two lines rather than pushing the table past 375px. A **chip row above the table** swaps which three are shown (`text-2xs` uppercase, hairline, 999px pill — the one place pill radius is allowed; active chip = `--text-1` border). The chip order is frozen while the table is open, so a tap never moves a chip out from under the finger. The choice is remembered per **column set** (localStorage; deliberately *not* the translated `label`, which changes with language and row count and is shared by two lenses on the same page). A **"Full table" chip** — **neutral `--text-1`, never gold**: the page's one gold primary is already spent, and a column control is not the page's primary action — opens every column as **two layers**: the name block is its own table beside a horizontally scrollable data block — **not CSS sticky**, which is the fragile part #5060 broke on. Row heights are synchronised, and a chevron on an opaque card-coloured field with a hairline (no gradient, no shadow) hints that more columns sit to the right; tapping "Full table" again returns to three columns. Sort order and column order are the desktop ones; **no zoom control** (the phone's own pinch-zoom is the answer). Secondary *text* columns (nation, age) still `fold` into the name cell's subline. Filter bar collapses to search + two half-width selects.
  **This is the DEFAULT, not an opt-in per page**: it is built once in `DataTable`, and a page only names its three starting columns — `mobileDefaults={["ovr","value","salary"]}` (without it, the first three numeric columns). Mark the entity column `sticky: true` — that is how `DataTable` knows what the name block is, and it logs a dev-only warning when a table has rows but no `sticky` column. Every `DataTable` call site in the app carries one as of #5102; a table that lacks one keeps the old scroller, which is the one way to opt *out* by accident. (Auctions and the transfer market run their own `<table>` implementations, not `DataTable` — see the comments in `AuctionsPage.jsx` / `TransfersPage.jsx` — and are unchanged by this.) *Replaces the 2026-09-02 rule (TASTE fork 6, #4625: pinned name + horizontal scroll), which is withdrawn: a pinned column over a scroller as the default state is gone.*

**Træningssiden (#3643, mockup 2, ejer-godkendt 18/9) — undtagelsen fra D-047:** D-047 gælder tabeller hvis kolonner er rytterens **egenskaber**. Træningssidens kolonner er en **tidsakse** — dagens løbsdage — og den har derfor sin egen mobilstandard, bygget som egne komponenter i `components/training/mobile/`, ikke som en indsnævret desktop-tabel. Formen: navnekolonne (forkortet fornavn "M. Sørensen" + én dæmpet linje med type · F · T) + **én kolonne pr. løbsdag**, ca. 9 ryttere pr. skærm. **Ingen chip-række og ingen "Fuld tabel"** — der er ikke fem kolonner at bytte imellem. Den rytter man trykker på får sit fulde kort **ÉN gang under tabellen** (form, træthed, alder, fremgang pr. evne, rollens opskrift som chips med loftet altid inkluderet, tempo som hastighed, og "Skift" der åbner det samme dagspanel som desktop) — mobil har sit eget **udvalg** af tal, men intet tal forsvinder helt. Over fanerne står sidens ene gold primary i fuld bredde (`min-h-11`); programmet er et gitter på 7 ugedage × N løbsdage; assistenten er et lukket panel med tælling. Antallet af kolonner er **divisionsafhængigt** (kalendertætheden er låst pr. division i `backend/lib/calendarTierCaps.js`: 5/4/3/3), og træningens egen tick-model (`training_tick_per_race_day`) er OFF i prod — så modellen (`lib/trainingMobileModel.ts`) bærer 1-5 kolonner uden at skifte form og viser aldrig et hårdkodet sæson-tal; med flaget OFF er der præcis én kolonne, "I dag". "Gruppér efter type" er væk på mobil, uændret på desktop. Desktop-fladen er uberørt. **Udrulning (ejer 19/9):** hele denne mobilstandard ligger bag stadie-flaget `training_mobile_table` i stadie **beta** — kun beta-testere ser den, alle andre ser D-047-grenen ovenfor uændret, og den gamle gren slettes først når flaget går til `on`.

**Rytterdatabasen (#5292, design A, ejer-godkendt 16/9):** mobilstandard = navn + rating, værdi og potentiale/scout. Løn vælges via kolonnechip. Desktop placerer værdi og potentiale/scout lige efter rating. Scout-visningen genbruger auktionssidens `ScoutablePotentiale`, inklusive skjulte estimater, kapacitet, ventetid og fejltilstande.
...
- **Hero card (the ONE anatomy):** canonical card — `--bg-card`, 1px `--border`, **5px radius** — with a **2px gold keyline on its top edge** (the T3 signature; the line follows the rounded corner). Padding 20-24px.
  ...
  - **Stat row — use the `HeroStats` primitive** (`components/ui/HeroStats.jsx`, #4628): 1px top rule, `pt-4`; label `text-3xs` uppercase tracking .1em `--text-3`; value data font **20px/650** tabular; optional `text-2xs` delta in success/danger. **Two columns on mobile, the horizontal row from `sm` up** — the hand-rolled `flex ... overflow-x-auto` copy silently clipped the last stat off the card on 375px (audit 2026-09: `/managers/:teamId` lost its fourth number entirely), and P10 forbids dropping numbers on mobile. Fewer stats is never the fix; the geometry is. **Rating renders as a color plate** (statColor fill + contrast ink since #3666 landing 1 — the 16%-tint variant was superseded 2026-08-15). **Potential renders as the lo-hi ceiling band** (`ScoutablePotentiale`/`PotentialBand`, owner call in #3666+#2454 landing 1, PR #3683 — supersedes the original "stars only" rule; stars remain only in the lo===hi fallback).
...
- **Tabs sit BELOW the hero card** on the page background: own 1px `--border` bottom rule; active tab = gold 2px underline overlapping the rule (`margin-bottom: -1px`); 14px/500 labels; horizontal scroll on mobile. **Tabs are underline-tabs, never buttons** — use `Tabs`/`TabList`/`Tab`/`TabPanel` (`components/ui/Tabs.jsx`) wherever a page switches its whole content collection (not a filter, see `FilterBar` above); the primitive owns `role="tablist"`/`role="tab"`, roving tabindex, and Left/Right/Home/End keyboard navigation — never hand-roll a tab row (audit 2026-09, Mit hold: tabs drawn as four angular buttons).
...
The card header and border stay mounted; only the body swaps between loading / empty / error / content. For tables that means swapping the `<tbody>`, not the whole card (converged 2026-07-25 — two patterns were in circulation); `DataTable` takes an `empty` node for exactly this (#4628). Swapping the whole card takes the toolbar with it, so a filter that empties the table can no longer be switched off — the dead end the rule exists to prevent.
