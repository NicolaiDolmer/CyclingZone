...
- **Mobile ≤640px — D-047 (owner call 2026-09-10, #5102):** the table shows the **name column + exactly three numeric columns, with no horizontal scroll**. A **chip row above the table** swaps which three are shown (`text-2xs` uppercase, hairline, 999px pill — the one place pill radius is allowed; active chip = `--text-1` border), and the choice is remembered per table (localStorage, keyed on the table's `label`). A **"Full table" chip** (gold outline; `--text-1` instead when the page already spends its one gold primary in the mobile viewport — pass `mobileFullTableTone="neutral"`) opens every column as **two layers**: the name block is its own table beside a horizontally scrollable data block — **not CSS sticky**, which is the fragile part #5060 broke on. Row heights are synchronised, and a chevron in the header hints that more columns sit to the right; tapping "Full table" again returns to three columns. Sort order and column order are the desktop ones; **no zoom control** (the phone's own pinch-zoom is the answer). Secondary *text* columns (nation, age) still `fold` into the name cell's subline. Filter bar collapses to search + two half-width selects.
  **This is the DEFAULT, not an opt-in per page**: it is built once in `DataTable`, and a page only names its three starting columns — `mobileDefaults={["ovr","value","salary"]}` (without it, the first three numeric columns). Mark the entity column `sticky: true` — that is how `DataTable` knows what the name block is, and it logs a dev-only warning when a table has rows but no `sticky` column. *Replaces the 2026-09-02 rule (TASTE fork 6, #4625: pinned name + horizontal scroll), which is withdrawn: a pinned column over a scroller as the default state is gone.*
