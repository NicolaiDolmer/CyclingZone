...
- **Row density:** default is the 13px vertical cell padding above. `DataTable dense` (cells `py-[7px]`, header `py-2`) is the ONE opt-in exception, for rosters where rows-per-screen is the point — owner call 2026-07-25 (#2906: "rækkerne er for høje", squad = 30 riders ≈ 1450px). Density is vertical only; the horizontal gutter stays `compact`'s job. Never hand-roll a third row height.
