...
**Status: AFVENTER EJER.** Ikke sendt. Ejeren poster selv lørdag eller søndag, før skiftet køres.
EN til #the-roadbook (primær), DA til #dansk-snak. Stemme: `docs/TONE_OF_VOICE.md` (jeg-stemme,
ingen em-dash, ingen tal ud over dem spilleren kan bruge).
...
Et hold uden login i 30 dage, som ikke har tilmeldt sig, parkeres uden for divisionerne ved skiftet. Intet på holdet slettes. Ét tryk på dashboardet henter dig tilbage, placeret efter din Global Rank, og du kører igen i samme sæson.
[EJER TJEKKER: løn løber videre, ingen sponsorindtægt, mens holdet er parkeret?]
...
Én række pr. påstand, i opslagets rækkefølge. EN og DA siger det samme, så kilden gælder begge.

| # | Påstand | Kilde |
|---|---|---|
| K1 | Sidste løbsdag i S3 slutter søndag 27/9 kl. 15 | `frontend/src/data/patchNotes.js` 7.297 ("At least 24 hours to pick your squad"); `docs/NOW.md` ("S3: slutter 27/9") |
| K2 | Skiftet kører efter sidste løbsdag, intet låst klokkeslæt | `docs/SEASON_CUTOVER_RUNBOOK.md` trin 12 (12a → 12c, hvert klik kræver ejer-go) |
| K3 | Sæson 4 starter 28/9 | `docs/SEASON_CUTOVER_RUNBOOK.md` "S3 → S4 (28/9 2026)" pkt. 2 og trin 1 (`--first-day 2026-09-28`) |
| K4 | Første etape tidligst 24 timer efter skiftet, et helt døgn til at sætte trup | `patchNotes.js` 7.297 |
| K5 | Op- og nedrykning som normalt på S3-stillingen | `SEASON_CUTOVER_RUNBOOK.md` trin 12a (`season_end_skip_division_movement` off); `docs/drafts/spec-s4-struktur-2026-09-24.md` "Rækkefølgen ved skiftet" pkt. 1 |
| K6 | Alle D4-managers rykker op i D3, fordelt på fire puljer efter point | `spec-s4-struktur-2026-09-24.md` ejer-beslutning 24/9 pkt. (1); `docs/NOW.md` Standing context "Liga" |
| K7 | D4 går fra 8 til 4 puljer, fyldt med AI og med løb fra dag ét | `spec-s4-struktur-2026-09-24.md` ejer-beslutning 24/9 pkt. (2); runbook trin 1 og 12a+ (#5641, #5642) |
| K8 | Pyramide 1/2/4/4 fra S4 | `docs/NOW.md` Standing context "Liga"; `buildSeasonCalendar.js`-gaten citeret i runbook trin 1 |
| K9 | 140 løbsdage i alle divisioner | `docs/NOW.md` ("140 er LÅST"); runbook trin 14 |
| K10 | Udløbne kontrakter bliver free agents, forlæng før skiftet | `frontend/public/locales/en/help.json` (FAQ om kontraktudløb og pension) |
| K11 | Sponsorens garanterede basis udbetales ved sæsonstart | `help.json` (FAQ "Sponsor money is paid at season start") |
| K12 | En rytter der pensioneres ved skiftet koster ikke næste sæsons løn | #4153 (lukket 24/9 via PR #5553) |
| K13 | Ingen ungdomsdrift ved dette skifte | `patchNotes.js` 7.300; runbook trin 11b; #5741 |
| K14 | Værdikørslen (usikker) | #5443 seneste kommentar 25/9 ("klar til søndagskørslen 27/9 ... ejer-go på dagen"); værdi-opslaget: `docs/drafts/2026-09-20-vaerdiskifte-udmelding-og-patch-note.md`. **[EJER TJEKKER]** |
| K15 | Loft U23 12, junior 10 | `patchNotes.js` 7.297 ("Squad caps"); `docs/NOW.md` Standing context "Trupper" (#5626) |
| K16 | Egne løb i egne grupper i S4 | `docs/drafts/spec-ungdomslob-2026-09-24.md` (ejer 24/9 + "Form besluttet 24/9: ca. 9 grupper pr. trup i S4"); `docs/NOW.md` løfte-tavle ("ungdomsløb · merget") |
| K17 | U23 1-2 løb om ugen, junior 1 | `docs/CALENDAR_RULES.md` §1f; `docs/YOUTH_RULES.md` §2.3 |
| K18 | Juniorer må køre fra 16 | `docs/NOW.md` ("Junior må køre fra 16 (ejer 24/9)"); `YOUTH_RULES.md` §2.1 |
| K19 | Ingen præmiepenge i ungdomsløb endnu | `YOUTH_RULES.md` §2.3 ("Ingen i v1") |
...
| K29 | Ét tryk henter holdet tilbage efter Global Rank, kører igen i samme sæson | `frontend/public/locales/en/dashboard.json` `seasonSignup.bodyParkedComeback` / `comebackConfirmed`; `spec-s4-struktur-2026-09-24.md` spor A4; `docs/NOW.md` løfte-tavle ("comeback · merget") |
| K30 | Løn uden sponsor mens parkeret (usikker) | `spec-s4-struktur-2026-09-24.md` ejer 23/9 ("A med løn"). Ikke verificeret bygget. **[EJER TJEKKER]** |
| K31 | Ny løbsmotor ikke med i skiftet; S3 færdig på den nuværende | `docs/NOW.md` Standing context "Race engine" (`race_engine_v4` OFF, v3 kører S3 færdig); runbook "Uafklaret" pkt. 5 |
| K32 | Ungdomsdivisioner fra S5, S4 i grupper | `spec-ungdomslob-2026-09-24.md` Y3 ("Divisioner fra S5") |
| K33 | Program pr. løbsdag ikke bygget (usikker) | runbook trin 14 ("Stadig ikke bygget: program pr. løbsdag"), målt 23/9. **[EJER TJEKKER]** om det er ændret |

## Bevidst udeladt

- **Tal fra prod** (hvor mange managers der rykker op, AI-pladser, parkerede hold): fra read-only målinger, ikke spillerregler, og repoet er offentligt.
- **Op- og nedrykningens detaljer** (top 2 op, bund 4 ned, faldskærm): står i Hjælp og er uændrede.
- **Klokkeslæt for træningen om aftenen:** reglen findes (runbook trin 14), men er ikke et løfte til spillerne. Teksten siger "om aftenen".
- **Comeback-grænserne pr. Global Rank:** står i spec'en, ikke i Hjælp. Spilleren får sin division vist, når han kommer tilbage.
