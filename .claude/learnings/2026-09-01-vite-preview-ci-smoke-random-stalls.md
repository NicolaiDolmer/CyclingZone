...
## Hvad skete der?
På `feat/2960-react19-rr8` fejlede 1-2 tilfældige tests i 4 CI-kørsler i træk, altid
~20-25 min inde i den 25-minutters fulde Playwright-suite på windows-latest, altid en
FORSKELLIG spec. Traceanalyse af CI-run 33498888228 viste mobile-webkit navigerede til
`/login` og hængte fast på app-spinneren: `load` fyrede aldrig, og netvaerkstracen
viste `assets/webVitalsIntegration-*.js` med status -1 (afsluttede aldrig), mens 31/32
andre requests var 200. Lokalt: 607/607 groent på 8 kerner på 23 min.

## Root cause
Ikke endeligt bekraeftet ved direkte lokal reproduktion (se "Forsoegt reproduktion"
nedenfor) — men staerk indicie-kaede peger på `vite preview` som Playwright's
`webServer` under vedvarende belastning på en CPU-svag (2-kernet) Windows-runner:
`vite preview` er en tungere, langsommere statisk-fil-server end nødvendigt (se
måling nedenfor), hvilket giver mindre margin før en enkelt request staller når
CPU'en samtidig deles med Playwright's browserprocesser i 20+ minutter. Moenstret
(altid samme fase i koerslen, altid forskellig spec, aldrig lokalt på kraftig
...
og `clarityIntegration` er `lazy()` på BEGGE grene og indgaar i INGEN af de to grenes
...
asset-requests i bolger mod en kørende server i 6 min. Resultat på 8-kernet lokal
hardware:
...
- `sirv` (samme dist, samme port-moenster): ~305.000 requests / 246s, 0 slow, 0
  never-completed — **~4x så mange requests i samme tidsrum**.
...
reproducerbart på en ledig 8-kernet maskine alene med HTTP-last. Men gennemsnits-
...
`hydratingLanding`-gate haandterer allerede "forkert" landing-markup på app-ruter,
se #418/#422 — ingen ændring i adfaerd). Kaldes via `npm run` (ikke `npx`) for at
...
så `worktreeIdPlugin` i `vite.config.js` (der servede `WORKTREE_ID_PATH` dynamisk)
...
haardt ved første lokale kørsel — fanget FOER push. Fix: nyt script
...
ligger i det gitignorede `dist/`, så den påvirker ikke "clean working tree"-tjekket
i CI.
...
HMR-hooks osv.), så hele klassen af "vite preview under load"-bugs er fjernet fra
webServer'en. Lokalt bruges samme kommando (`npm run build && npm run preview:e2e`),
så CI og lokal miljø ikke driver fra hinanden igen (jf. #3429's laering om at
CI/lokal-divergens selv er en fejlkilde).

## Læring
Når en flaky-fejl kun viser sig på CI's svage hardware og aldrig lokalt, er en
lokal stress-test ikke altid nok til at REPRODUCERE den — men den kan stadig give
målbar evidens (her: 4x throughput-forskel) der understoetter en infrastruktur-fix
uden at kraeve et 100% bekraeftet root-cause-bevis. Når den mistaenkte komponent er
...
udskiftning en legitim fix selvom den præcise fejlmekanisme i den gamle komponent
forbliver ubevist.
