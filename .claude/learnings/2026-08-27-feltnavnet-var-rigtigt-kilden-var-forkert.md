...
**7. Halvdelen af feltet var forkert på en helt anden måde.** `raceDays` blev rettet til distinkte `game_day`, men `races` og `raceDays` blev begge talt over ALLE holdets entries, uden sæsonfilter, mens chippens copy sagde "tilmeldt denne sæson". Målt read-only mod prod 27/8: 68.661 af 94.184 entries var fra tidligere sæsoner (73 %), 3.483 af 5.194 ryttere med entries fik et for højt tal (snit 18,1 løb vist mod 4,9 sande, værst 56 for meget), og 2.364 af dem har slet ingen løb i den aktive sæson. Den rettede halvdel var et no-op mod prod; den urettede halvdel løj højlydt hver dag. Når man retter et felt, så tjek HELE feltet, ikke kun det led issuet peger på.
