...
**Multi-slice partial-guard (lektion 2026-06-18):** Når et K-/done-issue er eksplicit opdelt (Slice A/B/C, Track A/B/C, Fase 1-N) og kun NOGLE slices er merged, er det **KEEP-TODO-PARTIAL — ALDRIG MOVE-TO-DONE**, selv om den merged slice i sig selv er dev-komplet. MOVE-TO-DONE kræver at issuets HELE resterende scope er dev-færdigt (kun ejer-verify mangler). 2026-06-18 returnerede scope-verify-agenter `MOVE-TO-DONE` for #1421 (kun Slice A leveret; B-D = ægte resterende dead-clicks) og #1301 (dev-del af SEO-epic done, men ejer-setup-børn #1406/#1407 ej kørt) — begge måtte korrigeres manuelt til KEEP-TODO. Tilføj derfor til scope-verify-prompten: "hvis issuet har flere slices/faser og kun nogle er leveret → KEEP-TODO-PARTIAL, ikke MOVE-TO-DONE; 'den leverede slice er færdig' ≠ 'issuet er færdigt'."

**Billig K-triage: split på PR-TITEL-konventionen før du dispatcher agenter (lektion 2026-08-15 — K var udskudt 2 auditter i træk):** Kategori K's fulde scope-verify kræver agenter, så den bliver konsekvent udskudt når kørslen er agent-fri — og dermed vokser repoets største backlog-kilde uhindret. Men repoet skriver issue-nummeret i PR-TITLEN ved bevidst leverance (`fix(3707):`, `feat(3586):`, `chore(3681):`, `(#3459 fase 2)`), mens kun-body-omtaler typisk er incidentelle (close-out-noter, dependency-noter, talkollisioner). Split derfor K i to spande FØR du beslutter metode:
- **titel-match** (`fix(N)` / `feat(N)` / `(#N` i titlen) — højt signal, verificerbart uden agenter ved at læse issue-body + sidste kommentar mod PR-titlen. Kør altid denne del, også i en agent-fri sweep.
- **kun body-omtale** — lavt signal, kræver agent-scope-verify. Udskyd uden dårlig samvittighed; markér ikke som legit-open uden verify.

2026-08-15: 16 af 44 kandidater var titel-match. Forvent LAVT udbytte selv der — partial-guarden greb 13 af de 16 (multi-slice `trin 2-5`, `Del A/B`, `Rest: 23/8-flip`, `fase 3`). Kun 2 closes (#3601, #3682) + 1 move-to-done (#3498). Det lave udbytte er signalet, ikke en fejl: titel-match beviser at PR'en var *tiltænkt* issuet, ikke at den dækkede hele scopet.

**Direkte commits på main er også leverancer (lektion 2026-08-15):** `crossref.py` matchede oprindeligt KUN issues mod PR'er. Men repoets hard rule tillader `chore`/`docs` direkte på main, så alt leveret den vej var **usynligt for Kategori K**. #3662 (masterplan-synk, ejer-direktiv) blev leveret i commit `91646dff` og blev kun fundet ved manuel efterprøvning af en agents begrundelse. Scriptet læser nu `git log origin/main` ved siden af PR-listen. Tre filtre gør det brugbart frem for støjende:
- **Trailing `(#N)` eller `Merge pull request #N` → spring over.** Repoets squash-konvention hænger altid PR-nummeret bagpå, så det er PR-afledt uanset om PR'en ligger i 200-vinduet.
- **Scan kun SUBJECT, ikke body.** Close-out-commits opremser 5-15 issuenumre i deres body som status. Body-scanning gav 63 kandidater, subject-scanning 9.
- **`docs(now*)` → spring over.** NOW.md-close-outs navngiver 3-8 issues pr. commit uden at levere nogen af dem; 22 af 26 kun-commit-kandidater kom derfra.

Efter filtrene: 4 kandidater, hvoraf 1 var ægte handlingsværdig. Cachen bærer nu både `prs` og `commits`; legacy-entries uden `commits`-nøgle flagges IKKE stale ved første kørsel (ellers ryger hele cachen til re-verify på én gang).
...
**Citeret PR er CLOSED uden merge → tjek om samme branch har en nyere merget PR, før issuet parkeres (lektion 2026-09-22, ejer-godkendt):** GitHub lukker automatisk PR'er hvis base-branch slettes (stablede PR'er, `--delete-branch` i merge-køen). 22/9 blev #5470 lukket og genoprettet som #5478 med samme commits; #4759 og #2491 citerede #5470 og ville efter 2026-05-28-reglen være parkeret som "claims delivered but PR still open", selv om leverancen var på main. Regel: ved `state: CLOSED` + `mergedAt: null` kør `gh pr list --state merged --head <branch>` (branch fra `gh pr view N --json headRefName`) og brug den nyere merged PR som evidens; findes ingen → parkér som før.
...
**Sidste kommentar er EVIDENS, ikke LEVERANCE → done→todo-kandidat (lektion 2026-08-15 — 4 fund på én kørsel):** Et `claude:done`-issue hvis seneste kommentar tilføjer **ny evidens FOR problemet** i stedet for at rapportere en leverance, bærer et falsk done-flag. Scoringsscriptet ser kun efter prod-evidens/PR-referencer og fanger ingen af dem — de er ofte de mest værdifulde issues i backloggen, fordi de er aktive spillerønsker der ligger gemt. Detektion: kommentaren indeholder **ingen** merged-PR-leverancepåstand (`shipped i PR #N`, `merged`, `leveret i`, `landede`) OG matcher mindst ét af: `Ny evidens`, `Evidens fra Discord`, `ønsket gentaget`, `efterspørges stadig`, `Symptomet er nu synligt`, `nyt krav`, `> **@`-citatblok, `Ny brugsbegrundelse`, `sweep <dato>`. → **DONE→TODO**, med en kommentar der siger præcis hvad der mangler. 2026-08-15: #2700 (nyt krav 9/8: varslet skal ses på rytteren), #2721 (@jeppek gentog ønsket 11/8), #3328 (nye fordelings-forslag 6/8), #3345 (spillerevidens på symptomet, ingen PR). Skeln fra work-pending-guarden: den fanger "arbejdet fortsætter"; denne fanger "arbejdet er aldrig begyndt, men nogen satte et flag".

**Kryds-issue-konflikt om samme PR → luk ingen af dem (lektion 2026-08-15):** Når ≥2 `claude:done`-issues citerer den SAMME PR og deres kommentarer er uenige om hvad der landede, er mindst én af dem forkert — og du kan ikke afgøre hvilken fra kommentarerne alene. Flag begge som **uafklaret leverance** til ejeren; luk ingen. 2026-08-15: #2454's kommentar sagde "Denne del er **ikke** bygget endnu", mens #3666's kommentar samme døgn sagde "#2454 potentiel rating i rating-point på alle flader" leveret i PR #3683 — samme PR, modstridende påstande. Uden reglen var #2454 blevet lukket på #3666's ord. Detektion i praksis: byg PR→issue-mappingen begge veje under Trin 2 og flag PR'er der optræder i flere done-issues.

**Flag-gated dev-done = `claude:done`, IKKE hard-close (lektion 2026-06-13):** Når launch-/relaunch-kritisk arbejde er merged men lever bag et feature-flag, hvis flip er en fremtidig **dateret begivenhed** (fx 20/6-relaunch, `academy_enabled`/race-motor-flag OFF indtil da), er korrekt state `claude:done` (gated) — IKKE hard-close — selv når arbejdet er backend-verificerbart. Hard-close kun når issuets EGET scope er prod-live (migration anvendt + prod-verificeret) OG ikke afventer en flag-flip. Skelner #1309 (kontrakt-seed: migration prod-live → **luk**) fra #1308/#1102 (bag `academy_enabled`/race-flag OFF → **claude:done**). Begrundelse: hard-close mister launch-dags-sporingen; et flag-gated issue er "done men gated", og flag-flippet trackes typisk af et separat orchestrator-/checkliste-issue (her #1103).

**Natbølge-done-flip = luk direkte i sweepen (lektion 2026-09-03, ejer-godkendt):** Efter en natbølge flipper orkestratoren issues til `claude:done` med kommentaren `Shipped i PR #N (squash <hash>), natboelge <dato>. Done-flip af orkestratoren; ejeren lukker per label-state-maskinen.` — 34 af 70 done-issues 3/9 havde præcis den. Mønsteret er nu STRONG i `score_done.py`. Sweepens verifikation: (1) `git fetch origin main` + `git merge-base --is-ancestor <hash> origin/main` for HVER hash (ét Bash-loop, alle 34 var på main 3/9), (2) spillervendte → tjek at den nyeste patch note dækker PR'en (grep `patchNotes.js` efter PR-nummeret eller titlen; 4 af 34 manglede og ventede på 7.240), (3) forbidden labels som normalt (`needs-decision` der er BESVARET af PR'en tæller ikke, fx #2405/#4288). Er alle tre opfyldt → luk med generisk kommentar + evidens på #627, uden ejer-kort pr. issue. Begrundelse: ejer-mandatet for done-sweeps (19/7, 3/9) dækker "leveret + på main + patch note"; at lade 34 verificerede issues stå til et kort pr. stk. er den pukkel der voksede 10 → 70 på tre dage.

**Samme-dags K-titel-match = luk i sweepen, og kravet gælder også enkelt-merges (lektion 2026-09-06, ejer-godkendt):** Runbookens 5b-krav (done-flip PR-for-PR) gælder kun natbølger. Men triage-sessioner (Sentry/Supabase-triage, ops-fixes) merger 3-6 PR'er med issuets nummer i titlen uden at flippe issuet — 6/9 stod #4896/#4899/#4900/#4903 som `claude:todo` samme aften som deres fix var på main. Samme fejlklasse som 15/8. To konsekvenser: (a) i sweepen: en K-kandidat hvis PR-titel matcher issuets titel OG er merget inden for 48 timer lukkes direkte efter `merge-base`-verify, ingen agent; (b) i prompter til ENHVER worker/session der merger (også uden for bølger): "når en PR med `#N` i titlen merges, flippes #N til `claude:done` i samme tur" — skrevet ind i `NIGHT_WAVE_RUNBOOK.md` 5b som gældende for alle merges.
...
**Kilde-tjek FØR enhver lukkeliste (lektion 2026-08-31):** Won't-do-/kill-kandidater (også fra klassifikations-agenter) SKAL kilde-tjekkes før de foreslås: indeholder body et Discord-citat, et spillernavn eller et ejer-løfte ("Loefte D/M", "ejeren skrev", "@spiller") → ALDRIG på lukkeliste uden at ejeren har set issuet med den kontekst fremhævet. 31/8: 3 af 17 i bundt 1 var ægte spillerønsker/ejer-løfter (#1837 @jeppek-ønske, #3726 ejer-løfte 3/8, #2723 @thelamba-forvirring) som klassifikations-agenten havde stemplet "ingen har bedt om det".

**Dublet-krydstjek mod scope-verify-resultater (lektion 2026-08-31):** Før en dublet-close eksekveres: tjek at dup-issuet ikke er udpeget som SPORINGS-STED for et andet issues restarbejde i K-/scope-verify-verdikterne fra samme kørsel. 31/8: dublet-agenten foreslog at lukke #2259 som dup af #4333 — mens K-verdiktet for #4333 samtidig sagde at #4333's ejer-gatede restpunkter netop trackes i #2259. Kun main-loop-krydstjek fangede det.
...
**Listen er turens SIDSTE besked (SKÆRPET 2026-09-05, bidt 3. gang):** Tekst der står FØR et tool-kald i samme tur (også før et `AskUserQuestion`) når ikke ejeren. Skriv listen som den afsluttende besked, afslut turen, og stil spørgsmålet i prosa; ejeren svarer i næste tur. 5/9: hele godkendelseslisten (4 grupper, 31 issues) lå mellem et Bash-kald og spørgsmålskortet, og ejeren svarede "Jeg kan ikke se den besked du siger du har sendt."

**Godkendelses-lister SKAL være synlige for ejeren (lektion 2026-06-11 — bidt 2x samme dag; SKÆRPET 2026-08-31):** Chat-tekst mellem tool-kald vises IKKE pålideligt i ejerens UI. Enhver liste der skal godkendes (kill-liste, dublet-batch, priority-sweep) præsenteres I CHATTEN som afsluttende besked FØR spørgsmålet: grupperet efter årsag, med kort begrundelse ("hvorfor lukke") pr. punkt, i klart sprog uden intern jargon (aldrig "spor B"/"D1"). #627-posting er audit-trail, IKKE præsentation — ejeren 31/8: "Jeg skal ikke åbne github for at læse informationen" + "Ikke bare en stor pærevælling ... skriv hvorfor noget skal lukkes efter din mening." Ét spørgsmål ad gangen, også for mandat-spørgsmål.
...
1b. **Done-verify ved 0 kommentarer: SØG PR-TITLER før DONE-TO-TODO (lektion 2026-07-26).** Repoets konvention er issue-nr i PR-titlen (`fix(...): ... (#N)`), ikke kun i kommentarer/body. Grep merged-PR-titlerne i `audit-pr-merged.json` (eller `gh pr list --state merged --search "#N in:title"`) FØR et done→todo-flip. 26/7: 3 af 11 DONE-TO-TODO-verdicts var falske — #2907/#2908/#2909 havde merged P0-PR'er (#2931/#2930/#2927) med nummeret i titlen, som agenterne aldrig søgte.
...
Mellem-tier mellem daglig routine (rører ikke puklen — alt scorer T3) og fuld-backlog-varianten (52 agenter, ~6,3M tok). Dagens facit 19/7: **46 closes + 5 done→todo for ~1/50 af fuld-variantens tokenforbrug, 0 subagenter.** Opskrift:
...
Kadence: ugentligt, eller når done-puklen > ~20. Fuld-backlog-varianten reserveres til kvartals-/præ-milestone-audits.
...
- **Workflow-budget-guard er obligatorisk på store audits (lektion 2026-06-11):** 11/6-fuld-backlog-auditten estimerede 2-3M subagent-tokens men brugte ~6,3M — der var INTET budget sat, selvom Workflow-værktøjet understøtter `budget.remaining()`-guards. Fremover: indhent token-target fra ejeren i mandat-spørgsmålene og skriv `while (budget.total && budget.remaining() > X)`-guards / agent-caps ind i scriptet. Dyreste poster at cappe først: klassifikations-chunks med bodies + Discord/docs-sweeps.
...
- **Slice/milestone-label-hygiejne (lektion 2026-06-13):** Ved en fokuseret deadline-/slice-audit (ejer beder om fx `slice:tdf-launch`-gennemgang) er den HØJESTE-værdi-handling at afstemme den åbne slice-label mod faktisk merged arbejde og **fjerne labelen** fra issues der er verificeret post-deadline — ikke kun close/done. Det gør et milestone-board ærligt: kun ægte blockers bliver tilbage. 2026-06-13: launch-blocker-audit fjernede `slice:tdf-launch` fra 9 post-launch-issues (#673/#961/#1140/#1235/#1237/#1280/#1357/#1294/#677) → board 38→27, hvoraf kun 12 var ægte todo-blockers. Handlinger: `gh issue edit N --remove-label "slice:..."` + kort comment med begrundelse; issue forbliver åbent i backloggen. Kombinér med move-to-done (dev-færdig/gated) + close (prod-live) for et komplet board-snit.
...
- **2026-09-06 — Done-sweep på ejer-mandat "luk de done-gated nu" (ejer-godkendt retro, 1 edit).** 25. kørsel, billig sweep (0 agenter): **33 closes** (28 done-sweep + 4 K-titel-match samme dag + #4418 efter ejer-beslutning), **3 done→todo** (#4376 forward-fix mangler, #4595 gate permanent rød, #4423 én rytter stadig is_academy i prod via SQL), 8 done beholdt (gated), 4 needs-decision fjernet ved close, nyt opfølger #4927, K-cache +20. `claude:done` 40 → 8, åbne 593 → 561. Accepteret edit: **samme-dags K-titel-match lukkes i sweepen + done-flip-kravet gælder alle merges, ikke kun bølger** (runbook 5b udvidet). Observation: `score_done.py` scorede 37/40 som T3 selv om 28 var rene "PR på main"-closes — mønstret `Merget som #N` / `Landet i PR #N` uden squash-hash er ikke STRONG i scriptet; overvej at tilføje det næste gang hvis det gentager sig.

- **2026-09-05 — Sweep efter natbølge + merge-runde (ejer-godkendt retro, begge edits).** 24. kørsel, billig ugentlig sweep (0 agenter): **30 closes** (12 backend/docs, 12 spillervendte med note 7.254, 5 ældre med prod-verify, #3422) + **2 todo→done** (#4557 gated, #4098 post-verify) · K: 16 titel-match-kandidater, 0 closes, 9 legit-open i cachen · luk-intent-grep 5 hits, alle uopfyldte. `claude:done` 52 → 21, åbne 582 → 550. Edit A: `score_done.py` regner nu `Shipped <dato> i PR #N` / `Leveret <dato> … PR #N` som STRONG når PR-nummeret står i kommentaren (alle 24 friske flips scorede WEAK/T3 og måtte verificeres manuelt). Edit B: Trin 6 skærpet: listen er turens sidste besked, ingen tool-kald mellem liste og spørgsmål. Observation: dagens done-flips havde PR-nummer men ingen squash-hash; verifikationen var `gh pr view --json state` pr. PR i ét loop, hvilket er billigt nok til at være standard.

- **2026-09-03 — Done-sweep + K-verify efter natbølge (ejer-godkendt retro, 1 af 3 edits).** 23. kørsel, input = kandidatlisten fra PR #4696. **63 closes** (50 done-sweep: 29 natbølge-flips med squash-hash verificeret på origin/main + 21 med prod-/CI-evidens, heraf #4644 via SQL mod `growth_metric_snapshots` og #4507 via natlig kalender-audit; 4 efter patch note 7.240; 6 K-verificeret af tre Sonnet-agenter; 1 dublet; #4262), **2 done→todo** (#3494/#4377: sidste kommentar var spiller-evidens), 1 todo→done, 4 relabels, masterplan-drift rettet som status-only efter ejer-kort. `claude:done` 70 → 14, åbne 644 → ~579. Accepteret edit: **Trin 3 "natbølge-done-flip = luk direkte"** + STRONG-mønster `Shipped i PR #N (squash <hash>)` i `score_done.py` — puklen voksede 10 → 70 på tre dage fordi bølgens done-flips ventede på et kort pr. issue. Observation: titel-match-triagen af K gav 16/40, agent-verify af 30 gav 6 closes + 3 BLOCKED-OWNER (#4485 #4576 #4278) — K er stadig lavt udbytte, men BLOCKED-OWNER-verdikterne er nyttige som ejer-liste. #4000's 31/8-flip til todo var falsk (konstanten var flippet 23/8, PR #4135) — se Rejected for hvorfor reglen ikke blev skrevet ind.

- **2026-08-31 — Fuld-backlog-audit retro (ejer-godkendt, alle 3 edits).** 22. kørsel: 54-agent workflow (~6,0M tok) over 602 åbne — K-verify m. hård refute (66 kandidater → 5 close + 7 done + 54 cache), 42 dublet-par (20 lukket), fuld klassifikation (8 buckets, penge/infra-linse), 80 beslutningsark, NUA-check, marketing-inventar. **I alt 41 closes + 7 todo→done + 2 done→todo**; 602→561 åbne. Ejer-mandater opnået undervejs: spor D (penge) aktiveret i S3-vinduet (skridt-for-skridt-go), won't-do-bundt 1 gennemgået i chat (12 lukket, 5 reddet). 3 accepterede edits: (1) **kilde-tjek før lukkeliste** — 3/17 bundt-kandidater var ægte spillerønsker/ejer-løfter som klassifikations-agenten havde stemplet lav-værdi; (2) **godkendelses-lister i chatten, grupperet m. hvorfor pr. punkt, jargon-frit** (#627 er audit-trail, ikke præsentation); (3) **dublet-krydstjek mod K-verdikter** (#2259-parret ville have lukket et udpeget sporings-sted). Observation: Kategori K gav 12/66 handlinger (bedre end 15/8's 5/48) — agent-scope-verify af hele puljen betaler sig ved fuld-audit, titel-match-triagen er stadig rigtig til billige sweeps.

- **2026-08-15 — Done-sweep retro (ejer-godkendt).** 21. kørsel, billig ugentlig sweep (0 subagenter): **33 closes** (31 done-sweep + 2 K-titel-match), **6 done→todo**, 1 move-to-done. `claude:done` 51→14, åbne 598→574. Puklen var vokset 19→51 på fire dage. Label-hygiejne ren i begge ender. 3 accepterede edits: (A) **Trin 3 — "sidste kommentar er evidens, ikke leverance"**: 4 af de 6 falske done-flag havde en sidste kommentar der tilføjede ny evidens FOR problemet (Discord-citater, nye krav, forslag) i stedet for at rapportere en leverance; scoringsscriptet fangede ingen af dem, og de var de mest værdifulde issues i puklen (#2700/#2721 er aktive spillerønsker der lå gemt bag et done-flag). (B) **Trin 3 — kryds-issue-konflikt om samme PR**: #2454 sagde "ikke bygget endnu", #3666 sagde "#2454 leveret i #3683" — samme PR, modstridende påstande; reglen forhindrede en forkert lukning. (C) **Trin 2 — billig K-triage via PR-titel-konventionen**: K var udskudt 2 auditter i træk fordi fuld scope-verify kræver agenter; titel-match-spanden (16 af 44) er verificerbar agent-frit. **(D) `crossref.py` udvidet til direkte commits på main** (ejer-godkendt senere samme dag, efter at agent-verifikationen af body-kun-spanden afslørede hullet): #3662 var leveret i commit `91646dff` og usynlig for Kategori K, fordi scriptet kun matchede mod PR'er — subject-only-scanning + trailing-`(#N)`-skip + `docs(now*)`-filter skærer 63 støj-kandidater ned til 4. **Facit for hele Kategori K denne dag: 44 + 4 kandidater gav 5 handlinger.** Kategorien er ikke den guldmine den er beskrevet som; kør titel-match + commit-spanden hver gang, og brug kun agenter på body-kun-spanden kvartalsvis. Observation til næste kørsel: 6 af 51 done-issues (12 %) bar et falsk done-flag mod 1 af 51 (2 %) 11/8, og fem af dem fik flaget fordi en PR *nævnte* issuet — samme fejlklasse som læringen fra 14/8, gentaget fem gange i samme døgn. Tilvæksten (+74 nye issues på fire dage) overstiger lukketakten; done-sweepen bør køre oftere end ugentligt så længe natbølgerne producerer i dette tempo.
...
- **2026-07-19 — Billig-ugentlig-sweep-variant (ejer-godkendt).** 17. kørsel, kombineret med masterplan-resync: done-pukkel 61→13 via **46 closes + 5 done→todo, 0 subagenter** (scripts + spot-PR-verify + #627-liste + AskUserQuestion-batches). Ejer-læringer fra kørslen: (a) ejer accepterede bulk-close af user-features når merged+deployet+patch note — men stoppede selv #671 (multi-slice: "flere flader tilbage" i sidste kommentar = partial-guard virkede den anden vej); (b) done→todo er en lige så vigtig output-klasse som close (#2449 stod done med u-genereret S2-kalender og ejer-deadline 26/7 — done-labelen SKJULTE ugens vigtigste opgave); (c) tjek nyeste Discord-evidens i score-output før close (#2526/#2581 så leverede ud men spillere meldte stadig fejl 19/7). Ny sektion "Billig ugentlig sweep" tilføjet som default-mode for pukkel-lukning.

- **2026-06-18 — Komplet done-audit retro.** 16. kørsel: ejer bad om komplet audit — find issues 100% færdige men ikke lukkede + luk optimalt. Forrige (13/6) var fokuseret launch-audit → 29 done-issues ophobet (21→29). `score_done` (29 done, alle T3 — AI-author/WEAK) + `crossref` (39 K-kandidater) + 5 parallelle Explore-agenter scope-verificerede 23 ægte K-kandidater (FP-clustre #1415/#1295 frafiltreret). **20 closes** (ejer-godkendt): 9 backend/bugfix/docs/security/a11y + 3 K-cleanup (#917/#1286/#1207) + 8 dev-verificerede board/UI-features. 12 beholdt claude:done (flag-gatede, sporer 20/6-relaunch). 30 K-kandidater verificeret legitimt åbne → cache-markeret. 3 accepterede edits (ejer: "ingen præference" → implementér efter dømmekraft): (A) scope-verify: **multi-slice partial-guard** — agenter returnerede MOVE-TO-DONE for #1421/#1301 hvor kun én slice/fase var leveret; "den leverede slice er færdig" ≠ "issuet er færdigt" → KEEP-TODO-PARTIAL; (B) `score_done.py`: **keep_done_gated/GATED-KEEP-flag** for aktive launch-slice-labels — adskiller bevidst-beholdte gatede fra close-eligible backend-fixes så manuel triage ikke re-udleder gated-listen; (C) baked-in: **routine auto-lukker IKKE dette repos done-pukkel** (alt AI-author/WEAK → Tier 3; routinen rører dem ikke → periodisk fuld done-sweep nødvendig uanset).

- **2026-06-13 — Launch-blocker-audit retro.** 15. kørsel: fokuseret slice-audit (ejer bad om afstemning af `slice:tdf-launch` mod merged arbejde + destilleret pre-20/6-blocker-liste). 4 parallelle Explore-agenter scope-verificerede alle 38 launch-issues + PR-merge-spot-verify. **20 handlinger:** 7 todo→done (dev-færdig/gated) + 2 closes (#1309 prod-live, #864) + 9 slice-label-fjernelser (post-launch). Board 38→27 (12 dev-færdig/gated, 3 epics, 12 ægte todo-blockers). Blocker-liste leveret til NOW.md + artifact. 2 accepterede edits, 1 afvist: (A) baked-in: **slice/milestone-label-hygiejne** — ved fokuseret deadline-audit er højeste-værdi-handling at FJERNE slice-labelen fra post-deadline-issues, ikke kun close/done (dagens 38→27 kom primært herfra); (B) Trin 3: **flag-gated dev-done = claude:done, ikke hard-close** — launch-arbejde bag et flag hvis flip er en fremtidig dateret begivenhed hører til `claude:done` (gated), hard-close kun ved prod-live-eget-scope (#1309 vs #1308/#1102). (C afvist, se Rejected).
...
- **2026-09-03 — (B) "done→todo kræver kode-grep" + (C) "bar `gh pr merge --admin` / `git --no-pager show` som baked-in".** Ejeren valgte kun edit A. B: #4000 blev 31/8 flippet done→todo på "implementerings-PR aldrig merget", men konstanten var flippet 23/8 (PR #4135, patch note 7.180) — forslaget var en regel om at grep'e koden før et done→todo på "ikke leveret". C: klassifikatoren blokerer `gh pr merge --admin` i sammensatte kommandoer (bar form virker), og `git show` på branch-filer skal have `--no-pager` (hook). Begge er dokumenteret her som observation; genforeslå kun hvis mønstret bider igen.

- **2026-06-13 — "Fokuseret-scope er gyldig success-mode" som baked-in lesson.** Foreslået: tilføj note om at en launch-/slice-specifik audit korrekt scoper til det label-subset (ikke alle 21 done-issues), så "lukkede ikke alle done" ikke fejltolkes som fejlet audit. Ejer afviste: aggressive-close-success-kriteriet (header) gælder fortsat, og en eksplicit "fokuseret mode kan lade done stå"-undtagelse risikerer at give dækning for at springe ægte close-arbejde over. Den daglige routine #627 dækker uanset de done-issues en fokuseret kørsel ikke når. Hvis fremtidige fokuserede kørsler reelt bider på dette (en ægte close bliver sprunget over og kaldt OK), re-introducer med skarpere afgrænsning.
