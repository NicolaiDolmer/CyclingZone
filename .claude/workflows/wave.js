...
function laneBrief(track) {
  ...
  return [
    ...
    track.kind === 'investigate' ? investigateBlok(track) : '',
    '',
...
const stopped = results.filter((r) => r.status === 'frys' || r.status === 'timeout' || r.status === 'investigate-timeout' || r.status === 'doed' || r.status === 'fejl')
