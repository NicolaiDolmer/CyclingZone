...
import { promote as promoteAcademyRider, demote as demoteAcademyRider, demoteContractPatch } from "../lib/academyTransfer.js";
...
async function notify(userId, type, title, message, relatedId = null, metadata = null) {
...
// Fix: denne route kalder LIGE PRÆCIS demoteContractPatch() — den samme funktion
// demote() selv bruger til at sætte den faktiske kontrakt — på en fuldt frisk
// server-side SELECT. Ingen frontend-kopi af formlen tilbage i demote-stien.
// (#4582 udvidede den delte funktion fra lønnen alene til hele kontrakten.)
//
...
router.get("/riders/:id/academy-demote-quote", requireAuth, async (req, res) => {
  ...
  const [racesCleared, racesOngoing, seasonNumber] = await Promise.all([
    countFutureRaceEntries(supabase, rider.id),
    countOngoingRaceEntries(supabase, rider.id),
    getActiveSeasonNumber(),
  ]);

  // #4582: dialogen skal vise DEN kontrakt flyttet rent faktisk skriver — og
  // siden demote nu ARVER en eksisterende kontrakt, må preview'et ikke længere
  // vise en gen-beregnet ungdomsløn. Samme funktion som demote() selv kalder
  // (demoteContractPatch), på den samme friske server-side SELECT — #3784-læringen
  // udvidet fra lønnen alene til hele kontrakten. keepsContract styrer dialogens
  // tekst, så den siger "kontrakten fortsætter uændret" i stedet for "ungdomsløn".
  const patch = demoteContractPatch(rider, seasonNumber);

  res.json({
    currentSalary: rider.salary ?? null,
    newSalary: patch.salary,
    keepsContract: patch.keepsContract,
    contractLength: patch.contract_length,
    contractEndSeason: patch.contract_end_season,
    racesCleared,
    racesOngoing,
  });
});
...
// #2518: Season Planner skal kunne skifte mellem sæson 1 og 2 (og enhver anden
// eksisterende sæson) — planlægger-fladerne (board, peak-plans-CRUD, spiller-
// kalenderen) opløste FØR kun den aktive sæson (activePeakSeason). Et eksplicit
// ?season_number= (eller season_number i body) slår ANY sæson op via sit
// spiller-vendte nummer (uanset status — 'upcoming' inkl., så S2's kalender kan
// ses/planlægges FØR den starter, jf. #2449). Uden parameteren er adfærden
// UÆNDRET (aktiv sæson), så gamle klienter/tests ikke brydes.
async function resolvePlannerSeason(seasonNumberRaw) {
...
// GET /api/peak-plans — holdets peak-planer for EGNE ryttere i sæsonen.
// #2518: ?season_number= vælger sæson (default: aktiv sæson, uændret adfærd).
router.get("/peak-plans", requireAuth, async (req, res) => {
  if (!req.team) return res.status(400).json({ error: "No team found" });
  try {
    if (!(await peakPlannerEnabledFor(req))) return res.json({ enabled: false, plans: [] });
    const season = await resolvePlannerSeason(req.query?.season_number);
...
router.post("/peak-plans", requireAuth, marketWriteLimiter, async (req, res) => {
  if (!req.team) return res.status(400).json({ error: "No team found" });
  const { rider_id: riderId, target_race_id: targetRaceId, season_number: seasonNumber } = req.body ?? {};
  ...
  try {
    ...
    // #2518: season_number lader manageren oprette peaks mod en IKKE-aktiv sæson
    // (fx S2 før den starter) — samme fallback-mønster som GET/board.
...
router.post("/peak-plans/dismiss-suggestions", requireAuth, marketWriteLimiter, async (req, res) => {
  if (!req.team) return res.status(400).json({ error: "No team found" });
  const { rider_id: riderId, season_number: seasonNumber } = req.body ?? {};
  if (typeof riderId !== "string" || !riderId) return res.status(400).json({ error: "invalid_rider_id" });
  try {
    if (!(await peakPlannerEnabledFor(req))) return res.status(404).json({ error: "not_found" });
    const season = await resolvePlannerSeason(seasonNumber);
...
router.get("/peak-plans/board", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // #2518: ?season_number= vælger S1/S2/... — default aktiv sæson (uændret).
    const season = await resolvePlannerSeason(req.query?.season_number);
...
// Range-pagineret + id-chunked load af race_stage_schedule MED game_day + stage_number.
// fetchAllScheduleRows ovenfor henter kun (race_id, scheduled_at) til binding-vinduer;
// kalenderen har brug for game_day (in-game-dag = kalender-sandheden) + stage_number.
// Samme cap-sikre mønster (#1823/#1798) så fuld-sæson-schedule ikke trunkeres tavst.
async function fetchAllScheduleRowsWithGameDay(supabase, raceIds) {
  const PAGE = 1000;
  const ID_CHUNK = 300;
  const rows = [];
  for (let i = 0; i < raceIds.length; i += ID_CHUNK) {
...
// IKKE gated på isRaceEngineV2Enabled. Kalenderen skal kunne renderes selv når
// race_engine_v2_enabled='off' (kill-switch) — den viser hvad der ER planlagt i
// sæsonen, uafhængigt af om simuleringen kører. Gaten her er udelukkende "der findes
// en aktiv sæson med planlagte løb". Selve motorens/schedulerens gates røres ALDRIG.
//
  ...
  try {
    ...
    res.json({
      season: {
        ...
        raceDaysTotal: season.race_days_total ?? null,
...
router.put("/races/:raceId/selection", requireAuth, marketWriteLimiter, async (req, res) => {
  ...
  try {
    ...
    if (bound.length) {
      // #2637 (Discord-opfølgning på #2599): en rytter der er auto-udtaget til et
      // ENDAGSLØB inden for dette (manuelt valgte) etapeløbs vindue blokerede tidligere
      // ALTID gemningen. En assistent-forslag et andet sted må aldrig vinde over et
      // manuelt valg her — så er konflikten en auto-genereret entry i et løb der IKKE
      // er startet endnu, frigiver vi den automatisk (fjerner KUN den ene rytter fra
      // det konfliktende løb) og fortsætter. Er konflikten derimod en MANUEL entry
      // eller et løb der allerede er i gang, kan vi ikke gætte spillerens hensigt —
      ...
      // ikke se HVEM/HVOR uden at rekonstruere det fra klientens egne kladder.
      if (blocking.length) {
        return res.status(409).json({ error: "selection_rider_bound", bound_rider_ids: bound, conflicts: blocking });
      }

      // Alle konflikter er løsbare (auto-genereret + ikke-startet løb): frigiv dem — kun
      // DEN pågældende rytter fjernes fra det konfliktende auto-udtaget løb — FØR vi
      // gemmer denne udtagelse. replace_race_selection-RPC'en gentager binding-tjekket
      // under advisory-lås (#2256), så en samtidig skrivning kan aldrig bryde invarianten.
      for (const r of resolvable) {
        const { error: relErr } = await supabase.from("race_entries").delete()
          .eq("race_id", r.race_id).eq("team_id", req.team.id).eq("rider_id", r.rider_id).eq("is_auto_filled", true);
        if (relErr) {
          captureException(new Error(`race_entries auto-release (${r.race_id}/${r.rider_id}): ${relErr.message}`));
...
router.put("/races/:raceId/stage-roles", requireAuth, marketWriteLimiter, async (req, res) => {
  ...
  try {
    ...
    const result = validateStageRoleOverrides({
      ...
      abandonedRiderIds: ctx.abandonedRiderIds,
    });
    if (!result.ok) {
...
router.get("/races/strategy", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Roster (løbs-berettigede ryttere) + abilities. NB: `riders` har INGEN `overall`-
    ...
    // Sæsonens stage-profiler → bucket-suitability pr. rytter + kommende løb til mål-løb.
    const { data: season } = await supabase.from("seasons").select("id, start_date").eq("status", "active").maybeSingle();
    let suitabilities = {};
    let upcoming = [];
    if (season) {
      ...
      // Kommende = ikke afsluttet. "Completed" = status='completed' ELLER alle etaper kørt.
      // Rå felter returneres; frontend afleder display-status via sin egen deriveRaceStatus (DRY).
      const isCompleted = (r) => r.status === "completed" || (r.stages_completed ?? 0) >= (r.stages || 1);
      upcoming = myRaces
        .filter((r) => !isCompleted(r))
        .map((r) => ({
          id: r.id, name: r.name, race_class: r.race_class,
          status: r.status, stages: r.stages, stages_completed: r.stages_completed ?? 0,
...
// Race Hub S3 — POST /api/races/strategy/preview. Live preview-diff: kør generatoren mod
// holdets kommende, ikke-startede, ikke-manuelle løb under den FORESLÅEDE strategi (i body,
// samme form som PUT) og differ mod nuværende entries. Skriver intet.
router.post("/races/strategy/preview", requireAuth, marketWriteLimiter, async (req, res) => {
  ...
  try {
    ...
    // Normalisér foreslået strategi mod roster + sæson-løb.
...
router.get("/admin/seasons/:id/generate-calendar/preview", requireAdmin, async (req, res) => {
  try {
    ...
    for (const tm of teams || []) {
      if (tm.is_ai === false && !tm.is_bank && !tm.is_frozen && !tm.is_test_account && tm.league_division_id != null) {
        realCountByPool.set(tm.league_division_id, (realCountByPool.get(tm.league_division_id) || 0) + 1);
      }
    }
...
router.post("/admin/seasons/:id/generate-calendar", requireAdmin, adminWriteLimiter, async (req, res) => {
  try {
    ...
    if (!dryRun) {
      await logActivity("season_calendar_generated", {
        meta: {
          season_id: season.id,
          season_number: season.number,
