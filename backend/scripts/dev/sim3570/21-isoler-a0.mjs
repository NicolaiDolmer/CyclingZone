...
const scenarier = [
  ["REN database (backup-tabellen findes IKKE) — PART A's normale førstegangs-kørsel", false, 6],
  ["backup-tabellen findes allerede (tom)", true, 6],
  ["databasen er allerede repareret (8193 draws)", false, 8193],
];

for (const [navn, opretKopi, antalDraw] of scenarier) {
  for (const [variant, sql] of [["GAMMEL form (før)", A0], ["NU, fra filen", A0_NESTET]]) {
    const db = new PGlite();
    const noter = [];
    db.onNotice?.((n) => noter.push(n.message ?? String(n)));
    await db.exec(`CREATE TABLE public.riders (id int primary key, archetype_draw jsonb);`);
    for (let i = 0; i < antalDraw; i++) await db.query(`INSERT INTO public.riders VALUES ($1,'{"primary":"gc"}')`, [i]);
