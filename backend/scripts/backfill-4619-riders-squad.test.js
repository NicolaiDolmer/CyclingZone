...
test("planSquadBackfill: totals + hvilke raekker der aendres", () => {
  const plan = planSquadBackfill({ riders: POPULATION, teams: TEAMS, graduations: [], seasonNumber: SEASON });
...
test("planSquadBackfill: pending graduation kun til den >= 23-aarige MED hold", () => {
  ...
  const plan = planSquadBackfill({ riders: withFreeOldster, teams: TEAMS, graduations: [], seasonNumber: SEASON });
...
test("planSquadBackfill: en rytter med eksisterende pending-raekke faar ikke en ny", () => {
  const plan = planSquadBackfill({
    riders: POPULATION, teams: TEAMS, seasonNumber: SEASON,
...
test("planSquadBackfill: hold over loft rapporteres med navn og antal", () => {
  ...
  const plan = planSquadBackfill({ riders: crowded, teams: TEAMS, graduations: [], seasonNumber: SEASON });
...
test("planSquadBackfill: ukendt hold-id falder tilbage paa id'et, ikke paa undefined", () => {
  const plan = planSquadBackfill({ riders: [rider("x", { age: 20, team: "t-ghost" })], teams: TEAMS, graduations: [], seasonNumber: SEASON });
...
test("IDEMPOTENS: anden koersel aendrer intet", () => {
  ...
  const first = planSquadBackfill({ riders: POPULATION, teams: TEAMS, graduations, seasonNumber: SEASON });
  ...
  const second = planSquadBackfill({ riders: after.riders, teams: TEAMS, graduations: after.graduations, seasonNumber: SEASON });
...
test("IDEMPOTENS: tredje koersel er ogsaa tom", () => {
  ...
  const second = planSquadBackfill({ riders: after1.riders, teams: TEAMS, graduations: after1.graduations, seasonNumber: SEASON });
  const after2 = applyPlanTo(after1.riders, after1.graduations, second);
  const third = planSquadBackfill({ riders: after2.riders, teams: TEAMS, graduations: after2.graduations, seasonNumber: SEASON });
