...
export function formatMarkdown({ before, after, overlapBefore, overlapAfter }) {
  ...
  lines.push(`## Fund og begrænsninger (ærlig rapportering: ikke alt ramte målet)`);
  lines.push(``);
  lines.push(`⚠ **VIGTIGSTE FUND (ejer-review anbefales FØR merge): B's GT-spredning-mål er IKKE nået,`);
  lines.push(`og den FULDE pakke (A+B+...+G sammen) måler et REGRESSIVT resultat på netop dette punkt**`);
  lines.push(`(spredning 4→7 dage, se detaljer nedenfor): se afsnittet "B" for den fulde forklaring`);
  lines.push(`og en anbefaling til næste skridt.`);
  lines.push(``);
  lines.push(`- **B (Giro-spredning): rammer IKKE ±1 dags-målet, og den FULDE pakke måler en**`);
  lines.push(`  **REGRESSION på dette specifikke mål, ikke en forbedring.** Målt spredning (maks−min`);
  lines.push(`  GT-spænd) for den FAKTISK SHIPPEDE kombination: FØR (gammel 21-etapers GT-længde,`);
  lines.push(`  B+C+D+E+F+G aktive) = 4 dage: praktisk talt identisk med rå prod-baseline (også 4).`);
  lines.push(`  EFTER (fuld pakke inkl. A's kortere GT'er) = 7 dage: VÆRRE end både FØR og`);
  lines.push(`  baseline. Isoleret afprøvning under implementeringen viste at B (rebalancerings-`);
  lines.push(`  funktionen) har PRAKTISK TALT INGEN målbar effekt på EFTER-scenariet mod det ægte`);
  lines.push(`  katalog (identiske GT-spænd med og uden B slået til): D1's ikke-GT-etapeløbs-pulje`);
  lines.push(`  ("others") er for lille i det rigtige katalog (typisk 3-4 løb) til at en`);
  lines.push(`  omfordeling flytter noget mærkbart, selvom mekanismen er verificeret korrekt i`);
  lines.push(`  isolerede unit-tests med en større syntetisk pulje. Regressionen (4→7) stammer i`);
  lines.push(`  stedet fra A's kortere GT'er i samspil med layoutStream's target-formel (hver GT's`);
  lines.push(`  EGEN seasonFraction, uændret af denne PR, styrer stadig hvornår dens "Trin 2"-`);
  lines.push(`  fyld-til-target kører): en fuld ±1-dags-garanti ville kræve at ændre selve`);
  lines.push(`  target-formlen (raceCalendarLanePacker.js linje ~365-380), vurderet for risikabelt`);
  lines.push(`  til denne PR (samme algoritme har haft 3 tidligere regressions-runder, #3472 v1-v3).`);
  lines.push(`  **Anbefaling: ejeren bør se dette tal FØR merge og afgøre om A (GT-længde) stadig**`);
  lines.push(`  **ønskes leveret nu, eller om B's rodfix skal udvides FØRST** (opfølgende issue).`);
  lines.push(`- **C (dage uden afgørelse) reducerer problemet markant i BEGGE kolonner, men rammer**`);
  lines.push(`  **ikke 0.** Engangs-diagnostik (C midlertidigt deaktiveret, samme katalog/seed):`);
  lines.push(`  UDEN C ville FØR give 7 dage uden afgørelse og EFTER 10: MED C (de målte tal`);
  lines.push(`  ovenfor) giver FØR 4 og EFTER 5, BEGGE bedre end rå baseline (6). C's placerings-`);
  lines.push(`  prioritet er BEVIDST konservativ (kapitel "Kan positionen ... flyttes" i`);
  lines.push(`  raceCalendarLanePacker.js): den flytter ALDRIG en etapeløbs mellemliggende`);
  lines.push(`  (ikke-slut-)etape, fordi det kan bryde løbets kronologiske rækkefølge: kun`);
  lines.push(`  endagsløb/hviledags-fillere byttes, og kun når et sikkert bytte findes. Nogle dage`);
  lines.push(`  (typisk midt i et GT-vindue hvor ALLE aktive løb er igangværende, ikke-`);
  lines.push(`  afsluttende etaper) har ingen sikker donor og forbliver uden afgørelse: et BEVIDST`);
  lines.push(`  designvalg (aldrig en tvunget/urealistisk fix), ikke en fejl.`);
