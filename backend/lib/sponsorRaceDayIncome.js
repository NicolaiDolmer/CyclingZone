...
export async function payRaceDaySponsorsToDate(seasonId, supabase, opts = {}) {
  ...
  const { data: contracts, error: contractsError } = await supabase
    ...
    .select("id, team_id, per_race_day_rate, bonus_clauses, results_bonus_paid, activated_at")
    .eq("status", "active");
  ...
  for (const race of races) {
    // #3316: imported_at tilføjet — bruges til at bygge resultTimeByTeam
    // (tidligste registrering pr. hold i DETTE løb), så en mid-season-
    // aktiveret kontrakt ikke kan bagudbetale for et løb der reelt blev
    // kørt/completet FØR aktiveringen.
