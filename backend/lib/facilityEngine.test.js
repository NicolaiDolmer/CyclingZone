...
test("effectiveBonus: bagud-kompat — integer staffTier (A1-service/A3-UI rå-tier-sti)", () => {
  ...
  assert.equal(effectiveBonus("training", 5, null), 0.165 * 0.5);       // uden staff: 50% (gulv)
...
test("effectiveBonus: #2216 A4 — ability-drevet display-magnitude (staff-objekt med overall)", () => {
  // Ny model: base × staffEffectFactor(staff) = base × (0.5 + 0.5·overall/99).
  assert.equal(effectiveBonus("training", 5, { overall: 99 }), 0.165 * 1.0);       // overall 99 → faktor 1.0
  assert.equal(effectiveBonus("training", 5, { overall: 0 }), 0.165 * 0.5);        // overall 0 → gulv
  assert.equal(effectiveBonus("training", 5, null), 0.165 * 0.5);                  // ingen staff → gulv
