...
test("gulv-kredit gives kun ved sejr, og aldrig i Class1/Class2", () => {
  ...
  assert.equal(floorCreditFor({ base: EVENT_BASE.ONE_DAY, outcome: EVENT_OUTCOME.WIN, raceClass: "ProSeries" }), 1);
  ...
  // Class1/Class2: nul, uanset hændelse.
  assert.equal(floorCreditFor({ base: EVENT_BASE.GC, outcome: EVENT_OUTCOME.WIN, raceClass: "Class1" }), 0);
...
test("gulvet kappes ved FLOOR_CAP uanset hvor mange sejre der ligger bag", () => {
...
test("form halveres pr. sæson siden hændelsen; gulvet gør ikke", () => {
  ...
  assert.equal(twoLater.reputation, 20);
});
...
test("reputation klemmes til 0-100", () => {
