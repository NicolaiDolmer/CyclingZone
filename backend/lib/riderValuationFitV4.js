...
//   O = fittingOutput(abilities, primary_type, alpha)  (riderValuation.js)
//
...
import { blendedOutput as liveBlendedOutput, meanAbilityScore } from "./riderValuation.js";
// Development fitting only. Do not teach the production valuation path about
// arbitrary experimental tables. The default retains historical behavior.
export function fittingOutput(abilities, type, alpha = 1, weights = null) {
  if (!weights) return liveBlendedOutput(abilities, type, alpha);
  const table = weights[type];
  if (!table) throw new Error('No candidate weights for ' + type);
  let sum = 0, count = 0;
  for (const [key, weight] of Object.entries(table)) {
    if (weight <= 0 || abilities?.[key] == null) continue;
    const value = Number(abilities[key]);
    if (Number.isFinite(value)) {sum += weight * value; count += weight;}
  }
  const mean = meanAbilityScore(abilities);
  return alpha * (count ? sum / count : mean) + (1 - alpha) * mean;
}
...
function fitForAlpha(samples, alpha, weights = null) {
  ...
  for (const s of samples) {
    const O = fittingOutput(s.abilities, s.primary_type, alpha, weights);
    outputs.push(O);
    X.push([1, O, O * O]);
...
export function fitOffsetsForFixedCurve(samples, { alpha, a, b, c = 0, weights = null } = {}) {
  ...
  for (const s of samples) {
    ...
    outputs.push(fittingOutput(s.abilities, s.value_role ?? s.primary_type, alpha, weights));
    y.push(Math.log(Math.max(Number(s.e_prize) || 0, FLOOR)));
  }
...
export function predictProductionLn({ abilities, primary_type }, fit, weights = null) {
  const O = fittingOutput(abilities, primary_type, fit.alpha, weights);
