-- #5051 — compute_daily_growth_snapshot(): halvårs-LTV-prisen var 26500 øre,
-- den faktiske pris er 21200 øre (265,00 kr. inkl. moms, jf. CZ Pro 6 Months
-- i backend/scripts/lib/aluntaPlanCatalog.js og frontend/public/locales/
-- {en,da}/pro.json). 26500 øre => 331,25 kr. inkl. moms — findes ingen steder
-- spilleren ser eller betaler. Samme fejlklasse som #4645 (pris-drift mellem
-- to håndskrevne konstanter), men i LTV-estimatet, ikke selve Alunta-planen.
--
-- Rod-årsag: 2026-08-03-growth-snapshots-3196.sql's CASE-udtryk blev aldrig
-- opdateret da CZ Pro 6 Months' pris blev sat til 21200/265,00 (#4645,
-- 2/9) — hverken denne funktion eller backend/lib/growthSnapshot.js's
-- PLAN_PRICE_CENTS.semiannual blev rettet dengang.
--
-- Berørte historiske rækker: growth_metric_snapshots.ltv_total_cents og
-- .ltv_avg_cents for enhver dag med mindst én semiannual-kunde, dvs. enhver
-- snapshot_date fra og med den dato 2026-08-03-growth-snapshots-3196.sql
-- første gang kørte til og med dagen før denne migration anvendes. Ingen
-- backfill her (ejer-beslutning: kun formel rettes) — LTV genberegnes
-- automatisk næste gang compute_daily_growth_snapshot() kører (dagligt cron,
-- se #3196), og ON CONFLICT (snapshot_date) DO UPDATE overskriver den gamle
-- forkerte værdi for dén dag. Ældre dages allerede-skrevne rækker forbliver
-- forkerte indtil de genberegnes (funktionen genberegner kun p_snapshot_date,
-- typisk "i dag") — ingen automatisk backfill af historikken.
--
-- Alt andet er UÆNDRET fra 2026-09-02-growth-snapshot-paying-only-4636.sql
-- (DAU/WAU/MAU, retention, "har betalt"-partitionering, NPS, upsert). Eneste
-- diff: 26500 -> 21200 i LTV-CASE'en (to forekomster, total + avg).
-- CREATE OR REPLACE = idempotent. RLS-gate uændret: service_role-only.
...
COMMENT ON FUNCTION public.compute_daily_growth_snapshot(date) IS
  '#3196/#4636/#5051 skriver ét vækst-snapshot (DAU/WAU/MAU/D1/D7/D30/abonnementer/LTV-estimat/NPS) for p_snapshot_date, UPSERT-idempotent. paying_customers + LTV tæller kun rækker med betalingsspor (Alunta-id, Pro-relevant status eller dækket periode), ikke rene vilkårsaccepter. Halvårs-LTV-prisen er 21200 øre (265,00 kr. inkl. moms), matcher CZ Pro 6 Months (#5051). service_role-only.';

-- Post-verify (køres af Claude efter merge, #2642): dagens snapshot genberegnes
-- og ltv_total_cents/ltv_avg_cents skal falde for enhver dag med semiannual-
-- kunder (26500 -> 21200 øre pr. periode); paying_customers uændret.
--   SELECT (public.compute_daily_growth_snapshot(current_date))->>'ltv_total_cents';
