// Port 1:1 af frontend/src/pages/LandingPage.jsx (#672-designet, anti-slop-
// hærdet i #671) til marketing-sitet (#4067). Ændringer i porten:
//   - react-i18next → statisk dict + makeT (server-renderet, ingen runtime-i18n)
//   - react-router <Link> → <a> mod app'ens absolutte URL'er
//   - LanguageToggle (klient-state) → rene links mellem / og /da
//   - useDocumentHead/useJsonLd → Next metadata-API + <script> i siderne
// Copy er de UÆNDREDE landing.json-tekster (kopieret fra frontend).
...
type HowRow = { no: string; title: string; body: string; data: string };
...
function Wordmark({ className = "h-7 sm:h-8" }: { className?: string }) {
  return (
    <img
...
// Sprogskifter som rene links (marketing-sitet er statisk pr. sprog-URL).
function LanguageToggle({ lang, label }: { lang: "en" | "da"; label: string }) {
  return (
...
// <details>/<summary> fungerer uden JS — FAQ'en er interaktiv i ren statisk HTML.
...
export default function LandingPage({ lang, dict }: { lang: "en" | "da"; dict: Dict }) {
  const t = makeT(dict);
  const howRows = t.raw<HowRow[]>("how.rows") ?? [];
  const diffCards = t.raw<DiffCard[]>("different.cards") ?? [];
  const faqItems = t.raw<FaqEntry[]>("faq.items") ?? [];
  const privacyHref = `${APP}${t("waitlist.privacyPath")}`;
        ...
        <section className="mx-auto grid max-w-6xl items-center gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14 lg:pb-24 lg:pt-20">
          <div>
            ...
            <p className="mt-3 text-sm">
              <a href="#waitlist" className="font-medium text-cz-2 underline-offset-2 hover:text-cz-1 hover:underline">
                {t("hero.ctaEmail")}
              </a>
            </p>
        ...
        <section id="waitlist" className="border-t border-cz-border">
          <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-8 lg:grid-cols-2 lg:gap-16 lg:py-20">
            ...
            <div className="lg:pt-2">
              <WaitlistForm dict={dict} privacyHref={privacyHref} />
            </div>
          </div>
        </section>
